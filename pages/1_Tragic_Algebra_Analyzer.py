...
if years and ticker and st.session_state.get("tk") == ticker:
    ...
    # ══ Up-C basis (§5.5 G, 15 Sep 2026 — supersedes the queue-G stop) ══
    # CVNA and RYAN: counts fold (queue G) and income now re-bases per year
    # to the filed consolidated figure, verification-gated against the
    # 15 Sep 2026 pastes (UP_C_INCOME). Verified → the basis banner and the
    # full page, one basis on both sides of every per-share figure. Legs
    # unreadable or a verification miss → the surviving stop, with the year
    # record rendered inside it (the GRAB lesson) and the footer before
    # st.stop (Job 5b). Guarded on shares > 0 as before: a discarded COUNT
    # entry leaves counts unread and falls to the ordinary no-share-count
    # stop exactly as pre-route.
    ...
    if any(e.up_c for e in XBRL_REGISTRY.get(tk, ())) and pre.get("shares", 0) > 0:
        if _upcb and _upcb.get("ok"):
            ...
            st.caption(f"ΔE measured all the same — last 3 years: "
                       ...
                       "as the income was read; only the per-share division is refused.")
    ...
    if not pooled.dE_defined:
        ...
        st.warning(f"**About {pooled.dE * 100:.0f} cents of each reported dollar reaches shareholders "
        ...
        st.success(f"**About {pooled.dE * 100:.0f} cents of each reported dollar reaches shareholders "
                   f"at this ΔE.** If the dilution pace behind it persists, 15% reported growth "
