...
if years and ticker and st.session_state.get("tk") == ticker:
    ...
    # ══ Up-C stop (queue G, 9 Sep 2026) ═══════════════════════════════
    # CVNA and RYAN: the XBRL route summed real per-class counts, so every
    # measurement is genuine — and the per-share valuation still cannot be
    # stood behind: the net income read is the parent's slice while the
    # summed count spans everything (Carvana roughly a third outside the
    # parent, Ryan Specialty over half). Refuse BEFORE the inputs (§5.5 G's
    # placement), with the year-by-year record rendered inside the stop
    # (the GRAB lesson: never point at notes a stopped page won't show) and
    # the footer before st.stop (the Job 5b lesson). Guarded on shares > 0:
    # if the route's verification gate discarded the entry, counts stay
    # unread and the ordinary no-share-count stop below handles it exactly
    # as before this route existed. Lifts with HANDOVER §5.5 G's NCI fix,
    # for which these registry entries are the test cases.
    if any(e.up_c for e in XBRL_REGISTRY.get(tk, ())) and pre.get("shares", 0) > 0:
        ...
        st.caption(f"ΔE measured all the same — last 3 years: "
                   ...
                   "of the whole business; only the per-share division is refused.")
    ...
    with st.expander(label):
        ...
        st.dataframe(pd.DataFrame([{
            ...
            "ΔE": _dE_text(dE_cell(y.N, y.dE, _med_N))} for y in years]).style.format({
                "Net income": _mfmt, "GAAP SBC": _mfmt, "Buybacks": _mfmt,
                "Share change": "{:+,.1f}", "Avg price": "${:,.2f}", "True SBC cost": _mfmt,
