...
if years and ticker and st.session_state.get("inf_tk") == ticker:
    ...
    # Up-C stop (queue G, 9 Sep 2026): rendered with the live gates because
    # that is where this page refuses. The XBRL route summed real per-class
    # counts, so the trend table, Ω and ΔE above are genuine measurements —
    # and the IV15 below them cannot be: the net income read is the parent's
    # slice while the summed count spans everything (Carvana roughly a third
    # outside the parent). Lifts with HANDOVER §5.5 G's NCI fix, not before.
    # (No footer call: this page's refusal stack predates the footer port —
    # the queued pages-4/5/6 footer fix picks this stop up with the rest.)
    if any(e.up_c for e in XBRL_REGISTRY.get(tk, ())) and shares > 0:
        st.error("**Refused.** " + up_c_sentence(tk, shares))
        with st.expander("Notes and detail", expanded=False):
            for kind_, msg in alerts:
                getattr(st, kind_)(msg)
            st.write("**What was read from the filings** — every tag, found or missing")
