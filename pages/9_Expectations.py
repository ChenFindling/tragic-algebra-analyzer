"""
Expectations — the reverse DCF (Rappaport & Mauboussin)
=======================================================
This page never says what a company is worth. It solves what today's PRICE
implies: the owners'-earnings growth rate g* at which the Tragic Algebra
Analyzer's IV15 equals the price at the chosen tier and required return, and —
holding growth at the revenue seed — the year-15 exit multiple the price
implies when the cash flows are bought and the business sold in year 15. The
...
--------------------------------------------------------------------------
     ...
     9   Expectations                 (this page)
     99  Return Calculator            (structurally last for the life of the kit)

3 is retired; never reuse a number. The engine span below carries the map
comment at its 12-September vintage, byte-verbatim with the copy; this
docstring is the map of record for this file.

Engine: the Tragic Algebra Analyzer's, copied verbatim (its lines 31-5204,
176 checks) and inverted by page-local code BELOW the span — the growth
solve, the exit solve, the evidence panels and the pinned base-rate data
never touch the shared block. If the shared reader changes in the page
files, it changes here too — this file is a sync target exactly like
pages 2, 4, 5, 6, 7 and the Baselines app.
...
# ══════════════════════════════════════════════════════════════════════
#  EXPECTATIONS — everything below this line is this page's own code.
#  Everything above it is the Tragic Algebra Analyzer's engine and
#  reader, copied verbatim (lines 31–5204 of the deployed file, 176
#  checks; only the module docstring was replaced). First copy 13 Sep
#  2026, at the state the toolkit pass closed on. The doctrine: what
#  this page inverts is what that page runs. The solvers below call the
#  span's intrinsic_value with the required return threaded — the
#  span's own solve_growth stays untouched (it hard-codes 15 and serves
#  that page's calibration box); generalising it in place would be a
#  shared-block change this page must not make (IVParams extraction is
#  the queued home for one shared solver).
# ══════════════════════════════════════════════════════════════════════
...
RR_FLOOR = 8.0        # % — above every tier's terminal growth cap (max 7%),
                      # so the box can never request the engine's NaN. A box
                      # that can ask for a refusal is a refusal designed out.
G_CEILING = 0.60      # implied growth beyond this prints a sentence, not a
                      # number — it matches the Tragic Algebra Analyzer's own
                      # growth-box ceiling, and the base table's record stops
                      # far below it.
EXIT_CEILING = 60.0   # implied exit multiples beyond this print a sentence —
                      # three times the richest tier default (Fortress, 20×).
IDENTITY_TOL = 0.005  # the inversion must reproduce IV15(g*) = price to the
                      # cent, live, on every run.


def solve_price_growth(price: float, base: "IVParams",
                       rr: float) -> tuple[str, float | None]:
    """What growth rate makes IV15 equal the price — or which sentence fires.

    Returns ("below", None) when the price sits at or below the zero-growth
    IV15 (the price implies decline; no negative rate is solved — it would
    print precision the arithmetic does not contain), ("ceiling", None) when
    even 60% a year cannot reach the price, and ("solved", g) otherwise.
    Bisection on [0, G_CEILING]; intrinsic value rises monotonically with
    growth, so the bracket sign test is the existence proof.
    """
    f = lambda g: intrinsic_value(
        IVParams(**{**base.__dict__, "growth": g}), rr) - price
    if f(0.0) >= 0:
        return "below", None
    if f(G_CEILING) < 0:
        return "ceiling", None
...
def identity_residual(price: float, base: "IVParams", g: float,
                      rr: float) -> float:
    """IV15 at the solved rate minus the price. The inversion's own proof,
    run live on every solve: |residual| at or above IDENTITY_TOL refuses."""
    return intrinsic_value(IVParams(**{**base.__dict__, "growth": g}), rr) - price


def implied_path_rates(base: "IVParams", g: float,
                       ks: tuple[int, ...] = (5, 10, 15)) -> dict[int, float]:
    """The demand the implied path places on its first k years, from the
    engine's own stream: (level_k / base)^(1/k) − 1. g is the stage-1 rate;
    the tier's fade makes every longer-span demand lower, which is exactly
    why the output sentence never quotes g as a 15-year rate."""
    s = _stream(IVParams(**{**base.__dict__, "growth": g}), max(ks))
    return {k: (s[k - 1] / base.OE) ** (1.0 / k) - 1.0 for k in ks}


def solve_price_exit(price: float, base: "IVParams",
                     rr: float, held_g: float) -> tuple[str, float | None, float]:
    """The second inversion, on the pure exit leg (Rappaport's price-implied
    terminal): buy the cash flows for 15 years at the held growth, sell in
    year 15 — long-horizon weight 0, cash-flows-plus-exit. IV is affine in
    the exit multiple on that frame, so two engine calls give the exact
    solution and a third proves it; no re-derived closed form that could
    drift from the engine.

    Returns (branch, x_or_None, iv_at_zero_exit): ("zero", …) when the
    discounted flows alone reach the price (the implied exit is at or below
    zero), ("beyond", …) past EXIT_CEILING, ("solved", x, …) otherwise.
    """
    p0 = IVParams(**{**base.__dict__, "blend": 0.0, "m2_style": "dcf",
                     "growth": held_g, "exit_multiple": 0.0})
    iv0 = intrinsic_value(p0, rr)
    iv1 = intrinsic_value(IVParams(**{**p0.__dict__, "exit_multiple": 1.0}), rr)
    slope = iv1 - iv0
    if not (slope == slope) or slope <= 0:
        return "undefined", None, iv0
    x = (price - iv0) / slope
    if x <= 0:
        return "zero", None, iv0
    if x > EXIT_CEILING:
        return "beyond", None, iv0
    return "solved", x, iv0


# ── The pinned base-rate table ────────────────────────────────────────
#
# Captured 13 Sep 2026 by the Baselines app's base-rate capture (an
# app-tail surface; the clean line held verbatim around its deploy) and
# pasted here by the session. Recomputed by session, never fetched live,
# exactly like the registry. The capture's definitions are restated in
# the page caption; the spot fragments at the bottom are raw filed
# series this page's self-tests recompute with the identical
# best_stretch_cagr and must land on the pinned figures exactly — the
# table cannot disagree with the series it was computed from.
#
# Coverage is honest per column, and one asymmetry is a documented
# reader property, not a defect: the reader knows post-2019 revenue
# tags only (mixing SalesRevenueNet eras was rejected on TransDigm —
# a real definitional difference), so many filers' revenue reaches
# ~9–10 years and the ten-year revenue column counts 8 names against
# 21 for owners' earnings.

from dataclasses import dataclass as _dataclass_br


@_dataclass_br(frozen=True)
class BaseRateRow:
    """One name's best endpoint stretches: rate, its window, the span read."""
    ticker: str
    rev5: float | None
    rev5_win: tuple[int, int] | None
    rev10: float | None
    rev10_win: tuple[int, int] | None
    oe5: float | None
    oe5_win: tuple[int, int] | None
    oe10: float | None
    oe10_win: tuple[int, int] | None
    rev_span: tuple[int, int, int] | None
    oe_span: tuple[int, int, int] | None
...
# ── BASE_RATES — pinned data for pages/9_Expectations.py ─────────────
# Captured 2026-09-13 by the Baselines app's base-rate capture.
...
# never fetched live.
BASE_RATES_AS_OF = "2026-09-13"
BASE_RATE_ROWS = [
    BaseRateRow(ticker='AAPL',
                rev5=0.11459147414899729, rev5_win=(2017, 2022),
                rev10=None, rev10_win=None,
                oe5=0.36104877257385204, oe5_win=(2010, 2015),
                oe10=0.165691863720012, oe10_win=(2010, 2020),
                rev_span=(2017, 2025, 9), oe_span=(2010, 2025, 16)),
    BaseRateRow(ticker='ADBE',
                rev5=0.21941629371494487, rev5_win=(2016, 2021),
                rev10=0.17890785243780782, rev10_win=(2014, 2024),
                oe5=1.4563523588497564, oe5_win=(2013, 2018),
                oe10=0.7611576789936536, oe10_win=(2013, 2023),
                rev_span=(2010, 2025, 16), oe_span=(2010, 2025, 16)),
    BaseRateRow(ticker='ADI',
                rev5=0.18022733067057395, rev5_win=(2017, 2022),
                rev10=None, rev10_win=None,
                oe5=0.25532877037620816, oe5_win=(2017, 2022),
                oe10=0.19086757292621126, oe10_win=(2013, 2023),
                rev_span=(2017, 2025, 9), oe_span=(2010, 2025, 16)),
    BaseRateRow(ticker='AMGN',
                rev5=0.07647774623365566, rev5_win=(2020, 2025),
                rev10=None, rev10_win=None,
                oe5=0.3124045110160969, oe5_win=(2017, 2022),
                oe10=0.04703111276525074, oe10_win=(2011, 2021),
                rev_span=(2019, 2025, 7), oe_span=(2010, 2025, 16)),
    BaseRateRow(ticker='AMZN',
                rev5=0.28140729260146746, rev5_win=(2016, 2021),
                rev10=None, rev10_win=None,
                oe5=0.8963301446474026, oe5_win=(2016, 2021),
                oe10=0.6770120520608702, oe10_win=(2011, 2021),
                rev_span=(2016, 2025, 10), oe_span=(2010, 2025, 16)),
    BaseRateRow(ticker='COST',
                rev5=0.11957397552382898, rev5_win=(2017, 2022),
                rev10=None, rev10_win=None,
                oe5=0.16472193445994843, oe5_win=(2016, 2021),
                oe10=0.14370870845957917, oe10_win=(2011, 2021),
                rev_span=(2016, 2025, 10), oe_span=(2010, 2025, 16)),
    BaseRateRow(ticker='CRM',
                rev5=0.2571449046015546, rev5_win=(2017, 2022),
                rev10=None, rev10_win=None,
                oe5=0.2798690605068461, oe5_win=(2021, 2026),
                oe10=None, oe10_win=None,
                rev_span=(2017, 2026, 10), oe_span=(2011, 2026, 16)),
    BaseRateRow(ticker='CSCO',
                rev5=0.04915047605967504, rev5_win=(2021, 2026),
                rev10=None, rev10_win=None,
                oe5=0.07882211519449678, oe5_win=(2011, 2016),
                oe10=0.03735302268498408, oe10_win=(2011, 2021),
                rev_span=(2017, 2026, 10), oe_span=(2011, 2026, 16)),
    BaseRateRow(ticker='GOOGL',
                rev5=0.23336333520406605, rev5_win=(2016, 2021),
                rev10=0.18665746361631075, rev10_win=(2013, 2023),
                oe5=0.46921678540345035, oe5_win=(2017, 2022),
                oe10=0.25379028556266214, oe10_win=(2015, 2025),
                rev_span=(2013, 2025, 12), oe_span=(2013, 2025, 13)),
    BaseRateRow(ticker='INTU',
                rev5=0.1965028658180299, rev5_win=(2020, 2025),
                rev10=None, rev10_win=None,
                oe5=0.5404765939654519, oe5_win=(2015, 2020),
                oe10=0.3420252133301396, oe10_win=(2015, 2025),
                rev_span=(2019, 2026, 8), oe_span=(2011, 2026, 16)),
    BaseRateRow(ticker='META',
                rev5=0.33666569031924, rev5_win=(2016, 2021),
                rev10=None, rev10_win=None,
                oe5=0.650994935746815, oe5_win=(2013, 2018),
                oe10=0.38674726620792854, oe10_win=(2013, 2023),
                rev_span=(2016, 2025, 10), oe_span=(2012, 2025, 14)),
    BaseRateRow(ticker='MSFT',
                rev5=0.15473427181155142, rev5_win=(2017, 2022),
                rev10=0.13792901043139016, rev10_win=(2016, 2026),
                oe5=0.42306972338789506, oe5_win=(2018, 2023),
                oe10=0.2423253798330931, oe10_win=(2015, 2025),
                rev_span=(2016, 2026, 11), oe_span=(2011, 2026, 16)),
    BaseRateRow(ticker='NFLX',
                rev5=0.2981791326099046, rev5_win=(2015, 2020),
                rev10=0.2772885733425925, rev10_win=(2010, 2020),
                oe5=0.8800833833330723, oe5_win=(2016, 2021),
                oe10=0.504038689211759, oe10_win=(2014, 2024),
                rev_span=(2010, 2025, 16), oe_span=(2010, 2025, 16)),
    BaseRateRow(ticker='NVDA',
                rev5=0.6689858183148778, rev5_win=(2021, 2026),
                rev10=0.45696505314008307, rev10_win=(2016, 2026),
                oe5=1.3203440595935483, oe5_win=(2021, 2026),
                oe10=0.7347742248724352, oe10_win=(2016, 2026),
                rev_span=(2010, 2026, 16), oe_span=(2011, 2026, 16)),
    BaseRateRow(ticker='PAYX',
                rev5=0.09528106546435477, rev5_win=(2021, 2026),
                rev10=None, rev10_win=None,
                oe5=0.14778735229442153, oe5_win=(2021, 2026),
                oe10=0.12470778896800461, oe10_win=(2014, 2024),
                rev_span=(2017, 2026, 10), oe_span=(2011, 2026, 16)),
    BaseRateRow(ticker='QCOM',
                rev5=0.18146962836798064, rev5_win=(2010, 2015),
                rev10=0.0874046023515116, rev10_win=(2012, 2022),
                oe5=0.38449082252016353, oe5_win=(2017, 2022),
                oe10=0.19419913421255552, oe10_win=(2011, 2021),
                rev_span=(2010, 2025, 16), oe_span=(2010, 2025, 16)),
    BaseRateRow(ticker='TTWO',
                rev5=0.24439514196475431, rev5_win=(2018, 2023),
                rev10=None, rev10_win=None,
                oe5=0.6422047409034126, oe5_win=(2016, 2021),
                oe10=0.2319099935639115, oe10_win=(2011, 2021),
                rev_span=(2017, 2026, 10), oe_span=(2011, 2026, 16)),
    BaseRateRow(ticker='TXN',
                rev5=0.06530147145019449, rev5_win=(2016, 2021),
                rev10=None, rev10_win=None,
                oe5=0.7395328658380766, oe5_win=(2013, 2018),
                oe10=0.3294375150269435, oe10_win=(2013, 2023),
                rev_span=(2016, 2025, 10), oe_span=(2010, 2025, 16)),
    BaseRateRow(ticker='BBW',
                rev5=0.1572150463549864, rev5_win=(2021, 2026),
                rev10=None, rev10_win=None,
                oe5=0.9654733336689014, oe5_win=(2020, 2025),
                oe10=0.1513831566105981, oe10_win=(2015, 2025),
                rev_span=(2017, 2026, 9), oe_span=(2010, 2026, 15)),
    BaseRateRow(ticker='CLMB',
                rev5=0.18410575005397734, rev5_win=(2009, 2014),
                rev10=0.05497608766906281, rev10_win=(2015, 2025),
                oe5=0.3581302947579712, oe5_win=(2020, 2025),
                oe10=0.16199626908164966, oe10_win=(2014, 2024),
                rev_span=(2009, 2025, 12), oe_span=(2010, 2025, 16)),
    BaseRateRow(ticker='CROX',
                rev5=0.2949361583094736, rev5_win=(2018, 2023),
                rev10=None, rev10_win=None,
                oe5=1.2090600009499628, oe5_win=(2017, 2022),
                oe10=0.6833756177759751, oe10_win=(2014, 2024),
                rev_span=(2016, 2025, 10), oe_span=(2010, 2025, 16)),
    BaseRateRow(ticker='PDEX',
                rev5=0.9888125190341792, rev5_win=(2013, 2018),
                rev10=0.5153191933753787, rev10_win=(2013, 2023),
                oe5=0.5792618091036215, oe5_win=(2016, 2021),
                oe10=0.36026342315887505, oe10_win=(2016, 2026),
                rev_span=(2012, 2026, 14), oe_span=(2011, 2026, 16)),
    BaseRateRow(ticker='PLTR',
                rev5=0.3257731580149814, rev5_win=(2020, 2025),
                rev10=None, rev10_win=None,
                oe5=None, oe5_win=None,
                oe10=None, oe10_win=None,
                rev_span=(2018, 2025, 8), oe_span=(2020, 2025, 6)),
    BaseRateRow(ticker='RDDT',
                rev5=None, rev5_win=None,
                rev10=None, rev10_win=None,
                oe5=None, oe5_win=None,
                oe10=None, oe10_win=None,
                rev_span=(2022, 2025, 4), oe_span=(2024, 2025, 2)),
    BaseRateRow(ticker='TGTX',
                rev5=None, rev5_win=None,
                rev10=None, rev10_win=None,
                oe5=None, oe5_win=None,
                oe10=None, oe10_win=None,
                rev_span=(2021, 2025, 5), oe_span=(2011, 2025, 15)),
    BaseRateRow(ticker='XPEL',
                rev5=0.2923717348681558, rev5_win=(2018, 2023),
                rev10=None, rev10_win=None,
                oe5=0.2772988118375288, oe5_win=(2019, 2024),
                oe10=None, oe10_win=None,
                rev_span=(2018, 2025, 8), oe_span=(2019, 2025, 7)),
]
BASE_RATE_DROPPED = [
    ('GRAB',
     'foreign private issuer reporting under IFRS — the kit routes it to the Non-US Checker; the base table keeps to the names its own frame reads'),
    ('KNSL',
     "insurer — the kit's pages refuse this frame for financials; the base table follows its own doctrine"),
]
BASE_RATE_SPOT_REV = ('ADBE',
    {2010: 3800.0, 2011: 4216.258, 2012: 4403.677, 2013: 4055.24, 2014: 4147.065, 2015: 4795.511, 2016: 5854.43, 2017: 7301.505, 2018: 9030.0, 2019: 11171.0, 2020: 12868.0, 2021: 15785.0, 2022: 17606.0, 2023: 19409.0, 2024: 21505.0, 2025: 23769.0},
    [])
BASE_RATE_SPOT_OE = ('AAPL',
    {2010: 11705.319398472493, 2011: 22846.177649409954, 2012: 37671.04462718083, 2013: 35644.42499735201, 2014: 32932.832859353286, 2015: 54670.331811673685, 2016: 44645.157567096416, 2017: 47433.724681220425, 2018: 58465.648015594474, 2019: 53059.1802513237, 2020: 54227.66701534212, 2021: 82407.17575261278, 2022: 89431.61855267217, 2023: 89072.14449502327, 2024: 89781.14533128738, 2025: 106406.75145144889},
    [])


BASE_RATE_THRESHOLDS = (0.10, 0.15, 0.20, 0.25, 0.30)


def count_names_at(rate: float, measure: str, span: int) -> tuple[int, int]:
    """(names at or above `rate`, names with a record at all) for one column.

    Counted from the pinned per-name records at render time, so the table
    and the sentence can never disagree with their own inputs — and the
    sentence counts at the demand rate exactly, never by nearest threshold.
    """
    have = [getattr(r, f"{measure}{span}") for r in BASE_RATE_ROWS
            if getattr(r, f"{measure}{span}") is not None]
    return sum(1 for v in have if v >= rate), len(have)


# ── The company against itself ────────────────────────────────────────


def full_window_cagr(series: dict[int, float],
                     excluded=frozenset()) -> tuple[float, int, int] | None:
    """Endpoint CAGR from the first to the last admissible year:
    (rate, first fy, last fy), or None. The same endpoint rules as
    best_stretch_cagr — positive, not excluded — applied to the window's
    own ends, so the figure is labelled by its actual span, never called
    a ten-year rate on nine years of filings."""
    ok = sorted(fy for fy, v in series.items() if v > 0 and fy not in excluded)
    if len(ok) < 2 or ok[-1] == ok[0]:
        return None
    n = ok[-1] - ok[0]
    return (series[ok[-1]] / series[ok[0]]) ** (1.0 / n) - 1.0, ok[0], ok[-1]


def own_history(ticker: str, years: list) -> dict:
    """The page's own filed evidence: best 5-year stretch and full-window
    rate on owners' earnings and on revenue, from the same ten-year read
    the page runs on. Owners' earnings drop zero-price years and bar
    excluded years as endpoints; revenue is the reader's own series,
    windowed to its last ten filed years — the identical rules the pinned
    base table was captured under, applied to this one name live."""
    oe = {y.fy: y.OE for y in years if y.price > 0}
    oe_excl = frozenset(y.fy for y in years if y.excluded)
    cmap = _ticker_map()
    facts = _facts(cmap[resolve_ticker(ticker, cmap)])
    rev_raw = _annual(facts, *CONCEPTS["REV"], [], "REV" in FILL_KEYS,
                      "REV" in RECENCY_KEYS, {})
    rev = {fy: rev_raw[fy][2] / 1e6 for fy in sorted(rev_raw)[-10:]}
    return {"oe5": best_stretch_cagr(oe, 5, oe_excl),
            "oe_full": full_window_cagr(oe, oe_excl),
            "rev5": best_stretch_cagr(rev, 5),
...
# in standalone functions, so the self-test can assemble each one with
# figures and hold it to the no-adjective rule — "what wrong looks
# like" item three as a red check rather than a review habit. The
# banned list includes the adverbs "only", "just" and "merely": the
# sneakiest adjectives are adverbs (Chen, 13 Sep 2026).

BANNED_IN_OUTPUT = (
    "reasonable", "unreasonable", "plausible", "implausible", "demanding",
    "undemanding", "modest", "extreme", "heroic", "aggressive",
    "conservative", "rich", "cheap", "expensive", "ordinary",
    "extraordinary", "remarkable", "stunning", "absurd", "staggering",
    "impressive", "lofty", "bargain", "eye-watering", "breathtaking",
    "only", "just", "merely",
)


def scan_for_adjectives(sentence: str) -> list[str]:
    """The banned words present in a sentence, lower-cased, whole words."""
    import re as _re
    low = sentence.lower()
    return [w for w in BANNED_IN_OUTPUT
            if _re.search(r"\b" + _re.escape(w) + r"\b", low)]


def growth_sentence(tk: str, price: float, tier_name: str, rr: float,
                    g: float, rates: dict[int, float]) -> str:
    """The page's signature: the implied path as the tier actually shapes
    it. g is the STAGE-1 rate — quoting it as a 15-year rate would
    overstate the 15-year demand (nine points at Chapel and 24%), which
    is exactly the sentence class this kit deletes — so the fade and the
    per-span demands are stated beside it."""
    t = AICT[tier_name]
    return (
        f"At {d(price)}, {tk}'s price implies owners' earnings growing at "
        f"**{g:.1%} a year for {tier_name}'s first {t.stage1_years} years**, "
        f"fading to {g * t.stage2_multiplier:.1%} a year for the "
        f"{15 - t.stage1_years} after, to a {t.default_exit_multiple:g}× exit "
        f"in year 15 — a 10-year demand of {rates[10]:.1%} a year and a "
        f"15-year flat equivalent of {rates[15]:.1%} a year, all to deliver "
        f"{rr:g}% a year to the buyer."
    )


def own_evidence_sentence(tk: str, own: dict) -> str:
    """The company against itself, spans named — a best stretch can start
    at a trough, so the window rides with every figure."""
    if own["oe5"]:
        v, a = own["oe5"]
        s = (f"{tk}'s own best filed 5-year stretch on owners' earnings is "
             f"{v:.1%} a year (FY{a}→FY{a + 5})")
    else:
        s = (f"No 5-year stretch with positive endpoints exists in {tk}'s "
             f"filed owners'-earnings window")
    if own["oe_full"]:
        v, a, b = own["oe_full"]
        s += f"; across the full window, {v:.1%} a year (FY{a}→FY{b}, {b - a} years)."
    else:
        s += "; the full window has no positive endpoint pair."
    return s


def base_evidence_sentence(g: float, rates: dict[int, float]) -> str:
    """The base table at the demand rates exactly — the counts come from
    the same pinned records the table renders, so the two cannot drift."""
    k10, m10 = count_names_at(rates[10], "oe", 10)
    k5, m5 = count_names_at(rates[5], "oe", 5)
    kr, mr = count_names_at(rates[5], "rev", 5)
    return (
        f"Of the {m10} names in the base table with a ten-year "
        f"owners'-earnings record, {k10} had a stretch at or above the "
        f"10-year demand of {rates[10]:.1%}; of the {m5} with a five-year "
        f"record, {k5} reached the 5-year demand of {rates[5]:.1%}, and "
        f"{kr} of {mr} reached it on revenue."
    )


def below_zero_sentence(tk: str, price: float, iv_zero: float, rr: float) -> str:
    return (
        f"**At {d(price)}, {tk}'s price is at or below its zero-growth IV15 of "
        f"{d(iv_zero)} on this base** — the price implies decline, not growth, "
        f"at a {rr:g}% required return. No rate is solved below zero: a "
        "negative implied figure would print precision this page cannot "
        "stand behind. The evidence panels below still apply to the base and "
        "the record."
    )


def ceiling_sentence(tk: str, price: float, rr: float) -> str:
    return (
        f"**At {d(price)}, no growth rate up to {G_CEILING:.0%} a year makes "
        f"{tk}'s IV15 reach the price** at this tier and a {rr:g}% required "
        "return — the implied rate is beyond any base rate on record. The "
        f"table below shows how many names ever sustained "
        f"{max(BASE_RATE_THRESHOLDS):.0%} for ten years; past "
        f"{G_CEILING:.0%} there is no record to compare against, and no "
        "figure is printed."
    )


def exit_sentence(tk: str, held_g: float, x: float, tier_name: str) -> str:
    t = AICT[tier_name]
    return (
        f"Holding growth at the seeded {held_g:.1%} and buying the cash flows "
        f"for 15 years with a year-15 sale, {tk}'s price implies an exit at "
        f"**{x:.1f}× owners' earnings** in year 15 — against the "
        f"{t.default_exit_multiple:g}× the tier itself assumes."
    )


def exit_zero_sentence(tk: str, held_g: float, iv0: float, price: float) -> str:
    return (
        f"Holding growth at the seeded {held_g:.1%}, the discounted cash "
        f"flows alone come to {d(iv0)} against the {d(price)} price — the "
        f"implied year-15 exit is at or below zero. The price implies a "
        "lower path than the seed, not an exit; the growth solve above is "
        "the reading that carries this page."
    )


def exit_beyond_sentence(tk: str, held_g: float) -> str:
    return (
        f"Holding growth at the seeded {held_g:.1%}, {tk}'s price implies a "
        f"year-15 exit beyond {EXIT_CEILING:g}× owners' earnings — past "
        "every traded multiple the tiers contemplate, and no figure is "
        "printed there. The growth solve above, which lets the whole path "
        "move rather than the exit alone, is the reading that carries this "
        "page."
    )


def identity_refusal_sentence(residual: float) -> str:
    return (
        f"**The inversion failed its own identity.** IV15 at the solved rate "
        f"differs from the price by {d(abs(residual))}, at or above the "
        "half-cent the solve must reproduce. No implied figures are printed. "
        "This is a defect in the page, not in your inputs — paste the "
        "assumptions block."
    )


def financial_stop_sentence(fin_label: str, fin_reason: str) -> str:
    """Page-specific: where the Tragic Algebra Analyzer withholds its
    verdict and shows an indicative ladder, this page prints nothing — an
    implied growth rate solved on the wrong frame is a number it cannot
    stand behind."""
    return (
        f"**No implied growth is printed — {fin_label}.** {fin_reason} This "
        "page's frame prices operating earnings with net cash added, and a "
        "balance-sheet business is priced on tangible book, returns and "
        "payout — the Financials Checker page's job. An implied growth rate "
        "solved on the wrong frame would be a number this page cannot stand "
        "behind. Use the Financials Checker page for this ticker."
    )


def inflection_route_sentence(base_oe: float, median_oe: float) -> str:
    return (
        f"**No implied growth from this base.** The owners'-earnings base is "
        f"{base_oe:,.0f}M and the five-year median is {median_oe:,.0f}M — an "
        "implied growth rate solved from a base at or below zero is noise, "
        "not a measurement. The Inflection Checker page prices loss-to-profit "
        "shapes on an operating basis; use it for this ticker. To solve here "
        "anyway, set a positive base in the box above — the assumptions "
        "block will record it as set by hand."
    )


def foreign_stop_sentence() -> str:
    return (
        "No implied growth is printed on a reading this page cannot stand "
        "behind. Use the Non-US Checker page for this ticker."
    )
...
def expectations_self_test() -> list[tuple[str, bool, str]]:
    ...
    _syn = IVParams(OE=100.0, shares=10.0, tier="Chapel", growth=0.0,
                    net_cash=50.0, exit_multiple=14.5, blend=0.5)

    # 1. The required-return floor clears every tier's terminal cap, so
    #    the box can never request the engine's NaN.
    _max_cap = max(t.terminal_growth_cap for t in AICT.values())
    out.append(("Required-return floor sits above every tier's terminal cap",
                RR_FLOOR / 100.0 > _max_cap,
                f"{RR_FLOOR}% vs cap {_max_cap:.0%}"))

    # 2. The inversion identity at 15%: forward IV at a known rate, solved
    #    back, must recover the rate and reproduce the price to the cent.
    _p15 = intrinsic_value(IVParams(**{**_syn.__dict__, "growth": 0.18}), 15)
    _k, _g = solve_price_growth(_p15, _syn, 15)
    _r = identity_residual(_p15, _syn, _g, 15) if _k == "solved" else float("nan")
    out.append(("Inversion identity at 15%: rate recovered, price to the cent",
                _k == "solved" and abs(_g - 0.18) < 1e-9 and abs(_r) < IDENTITY_TOL,
                f"g*={_g:.9%}, residual ${_r:.6f}" if _k == "solved" else _k))

    # 3. The same identity at a non-default required return (the box).
    _p10 = intrinsic_value(IVParams(**{**_syn.__dict__, "growth": 0.12}), 10)
    _k, _g = solve_price_growth(_p10, _syn, 10)
    _r = identity_residual(_p10, _syn, _g, 10) if _k == "solved" else float("nan")
    out.append(("Inversion identity at 10%: the required return threads through",
                _k == "solved" and abs(_g - 0.12) < 1e-9 and abs(_r) < IDENTITY_TOL,
                f"g*={_g:.9%}, residual ${_r:.6f}" if _k == "solved" else _k))

    # 4. Monotonicity across the bracket — the property bisection stands on.
    _ivs = [intrinsic_value(IVParams(**{**_syn.__dict__, "growth": g}), 15)
            for g in (0.0, 0.20, 0.40, G_CEILING)]
    out.append(("IV15 rises strictly with growth across the solve bracket",
                all(a < b for a, b in zip(_ivs, _ivs[1:])),
                " → ".join(f"{v:,.1f}" for v in _ivs)))

    # 5. The below-zero branch: a price at the zero-growth IV15 exactly, and
    #    one below it, both refuse the negative solve.
    _iv0 = intrinsic_value(IVParams(**{**_syn.__dict__, "growth": 0.0}), 15)
    out.append(("Price at or below zero-growth IV15 → the decline sentence, no solve",
                solve_price_growth(_iv0, _syn, 15)[0] == "below"
                and solve_price_growth(_iv0 * 0.9, _syn, 15)[0] == "below",
                f"zero-growth IV15 {_iv0:,.2f}"))

    # 6. The ceiling branch: a price past IV15 at the 60% ceiling.
    _iv60 = intrinsic_value(IVParams(**{**_syn.__dict__, "growth": G_CEILING}), 15)
    out.append(("Price beyond the 60% ceiling → the ceiling sentence, no number",
                solve_price_growth(_iv60 * 1.01, _syn, 15)[0] == "ceiling",
                f"IV15 at 60% is {_iv60:,.2f}"))

    # 7. The exit solve: affine, exact, proved by a third engine call.
    _base_g = IVParams(**{**_syn.__dict__, "growth": 0.10})
    _pk = 20.0 * _base_g.OE / _base_g.shares
    _kx, _x, _iv0x = solve_price_exit(_pk, _base_g, 15, 0.10)
    _chk = intrinsic_value(IVParams(**{**_base_g.__dict__, "blend": 0.0,
                                       "m2_style": "dcf",
                                       "exit_multiple": _x}), 15) if _kx == "solved" else float("nan")
    out.append(("Exit solve: affine solution reproduces the price to the cent",
                _kx == "solved" and abs(_chk - _pk) < IDENTITY_TOL,
                f"x*={_x:.4f}×, IV {_chk:,.6f} vs price {_pk:,.2f}" if _kx == "solved" else _kx))

    # 8. The exit solve's own branches: flows alone past the price → zero;
    #    a price demanding more than 60× → beyond.
    out.append(("Exit solve branches: at-or-below-zero and beyond-the-ceiling",
                solve_price_exit(_iv0x * 0.9, _base_g, 15, 0.10)[0] == "zero"
                and solve_price_exit(_pk * 50, _base_g, 15, 0.10)[0] == "beyond",
                f"flows-alone value {_iv0x:,.2f}"))

    # 9. The path demands, derived a second way: Chapel at 24% — five years
    #    at g, ten at 0.45g — recomputed from the tier's own parameters
    #    rather than through _stream.
    _rates = implied_path_rates(IVParams(**_syn.__dict__), 0.24)
    _t = AICT["Chapel"]
    _lvl10 = (1.24 ** 5) * ((1 + 0.24 * _t.stage2_multiplier) ** 5)
    _lvl15 = (1.24 ** 5) * ((1 + 0.24 * _t.stage2_multiplier) ** 10)
    out.append(("Path demands (Chapel, 24%): 5y = stage-1 rate; 10y and 15y "
                "match a second derivation",
                abs(_rates[5] - 0.24) < 1e-12
                and abs(_rates[10] - (_lvl10 ** 0.1 - 1)) < 1e-12
                and abs(_rates[15] - (_lvl15 ** (1 / 15) - 1)) < 1e-12,
                f"5y {_rates[5]:.4%} · 10y {_rates[10]:.4%} · 15y {_rates[15]:.4%}"))

    # 10. The same on a tier whose stage 1 is shorter than five years, so
    #     even the 5-year demand mixes stages (Wood: 2 at g, 3 at 0.25g).
    _rw = implied_path_rates(IVParams(**{**_syn.__dict__, "tier": "Wood"}), 0.30)
    _w = AICT["Wood"]
    _lvl5 = (1.30 ** 2) * ((1 + 0.30 * _w.stage2_multiplier) ** 3)
    out.append(("Path demands (Wood, 30%): the 5-year demand mixes stages",
                abs(_rw[5] - (_lvl5 ** 0.2 - 1)) < 1e-12 and _rw[5] < 0.30,
                f"5y {_rw[5]:.4%} against a 30.0000% stage-1 rate"))

    # 11–12. The spot fragments: the pinned figures must be reproduced from
    #     the raw filed series by the identical function — the table cannot
    #     disagree with the series it was computed from.
    _adbe = next(r for r in BASE_RATE_ROWS if r.ticker == BASE_RATE_SPOT_REV[0])
    _sr = best_stretch_cagr(BASE_RATE_SPOT_REV[1], 10,
                            frozenset(BASE_RATE_SPOT_REV[2]))
    out.append(("Spot check, revenue: the pinned ten-year figure recomputes "
                "exactly from its raw series",
                _sr is not None and _sr[0] == _adbe.rev10
                and (_sr[1], _sr[1] + 10) == _adbe.rev10_win,
                f"{BASE_RATE_SPOT_REV[0]} {_sr[0]:.6%} FY{_sr[1]}→FY{_sr[1] + 10}"
                if _sr else "no stretch"))
    _aapl = next(r for r in BASE_RATE_ROWS if r.ticker == BASE_RATE_SPOT_OE[0])
    _s5 = best_stretch_cagr(BASE_RATE_SPOT_OE[1], 5,
                            frozenset(BASE_RATE_SPOT_OE[2]))
    _s10 = best_stretch_cagr(BASE_RATE_SPOT_OE[1], 10,
                             frozenset(BASE_RATE_SPOT_OE[2]))
    out.append(("Spot check, owners' earnings: both pinned figures recompute "
                "exactly from the raw series",
                _s5 is not None and _s10 is not None
                and _s5[0] == _aapl.oe5 and (_s5[1], _s5[1] + 5) == _aapl.oe5_win
                and _s10[0] == _aapl.oe10
                and (_s10[1], _s10[1] + 10) == _aapl.oe10_win,
                f"{BASE_RATE_SPOT_OE[0]} 5y {_s5[0]:.6%}, 10y {_s10[0]:.6%}"
                if _s5 and _s10 else "no stretch"))

    # 13. The columns' coverage, pinned from the capture of record — a
    #     mangled paste of the data block fails here by name.
    _ms = {(m, sp): count_names_at(-10.0, m, sp)[1]
           for m in ("rev", "oe") for sp in (5, 10)}
    out.append(("Column coverage matches the capture of record: rev 24/8, "
                "oe 23/21, of 26 rows",
                len(BASE_RATE_ROWS) == 26 and _ms[("rev", 5)] == 24
                and _ms[("rev", 10)] == 8 and _ms[("oe", 5)] == 23
                and _ms[("oe", 10)] == 21,
                f"rows {len(BASE_RATE_ROWS)}, M {_ms}"))

    # 14. The counting rule, recounted independently, boundary included: a
    #     name AT the probed rate counts (the sentence says "at or above").
    _probe = _adbe.rev10
    _k, _m = count_names_at(_probe, "rev", 10)
    _k2 = sum(1 for r in BASE_RATE_ROWS
              if r.rev10 is not None and r.rev10 >= _probe)
    out.append(("Counting rule: independent recount agrees, and a name at "
                "the probed rate itself is counted",
                _k == _k2 and _k >= 1, f"{_k} of {_m} at {_probe:.4%}"))

    # 15. Row shape: every stated window is exactly its span wide, and
    #     every figure has its window (and the reverse).
    _shape = all(
        (getattr(r, f"{m}{sp}") is None) == (getattr(r, f"{m}{sp}_win") is None)
        and (getattr(r, f"{m}{sp}_win") is None
             or getattr(r, f"{m}{sp}_win")[1] - getattr(r, f"{m}{sp}_win")[0] == sp)
        for r in BASE_RATE_ROWS for m in ("rev", "oe") for sp in (5, 10))
    out.append(("Row shape: figures and windows travel together, windows "
                "exactly span-wide", _shape, "all rows"))

    # 16. The no-adjective rule, held on every assembled output sentence —
    #     including the adverbs only/just/merely (13 Sep 2026).
    _rates_d = implied_path_rates(_syn, 0.24)
    _own_d = {"oe5": (0.112, 2019), "oe_full": (0.087, 2016, 2025),
              "rev5": (0.14, 2019), "rev_full": (0.10, 2016, 2025)}
    _sentences = [
        growth_sentence("SYN", 61.20, "Chapel", 15, 0.243, _rates_d),
        own_evidence_sentence("SYN", _own_d),
        own_evidence_sentence("SYN", {"oe5": None, "oe_full": None,
                                      "rev5": None, "rev_full": None}),
        base_evidence_sentence(0.243, _rates_d),
        below_zero_sentence("SYN", 10.0, 12.0, 15),
        ceiling_sentence("SYN", 500.0, 15),
        exit_sentence("SYN", 0.08, 41.2, "Chapel"),
        exit_zero_sentence("SYN", 0.08, 25.0, 20.0),
        exit_beyond_sentence("SYN", 0.08),
        identity_refusal_sentence(0.02),
    ...
    out.append(("No adjective — and no only/just/merely — in any output sentence",
                not _hits, "clean" if not _hits else "found: " + ", ".join(sorted(_hits))))

    # 17. Route sentences name their destination pages by menu name and
    #     carry no numeric page reference (the sweep's _route_ok).
    _fin = financial_stop_sentence("a bank", "Deposits were read.")
    _inf = inflection_route_sentence(-1.0, -1.0)
    _for = foreign_stop_sentence()
    out.append(("Routes: Financials Checker, Inflection Checker and Non-US "
                "Checker named, _route_ok on all three",
                _route_ok(_fin) and "Financials Checker" in _fin
                and _route_ok(_inf) and "Inflection Checker" in _inf
                and _route_ok(_for) and "Non-US Checker" in _for,
                "three routes"))

    # 18. The identity guard's sentence exists and names the tolerance —
    #     the refusal that fires if the inversion ever fails live.
    _idr = identity_refusal_sentence(0.02)
    out.append(("Identity refusal names the residual and the half-cent tolerance",
...
def _page_footer() -> None:
    """The glossary, the self-test button and the disclaimer. Called before
    every st.stop and once at the bottom — a refused ticker's reader is
    exactly the reader who may want to run the checks (the GRAB lesson)."""
    st.divider()
    _r1, _r2 = st.columns(2)
    with _r1:
        with st.expander("What the numbers mean", expanded=False):
            st.markdown(
                "**g\\*** — the stage-1 owners'-earnings growth rate at which the Tragic "
                "Algebra Analyzer's IV15 equals today's price. The tier fades it after "
                "stage 1, so the sentence also states the 10-year demand and the 15-year "
                "flat equivalent — the same path, quoted over each span it is asked of.\\n\\n"
                "**Implied exit** — holding growth at the revenue seed, the year-15 "
                "multiple of owners' earnings the price implies when the cash flows are "
                "bought and the business sold in year 15. Rappaport and Mauboussin's "
                "price-implied terminal.\\n\\n"
                "**Base-rate table** — of the names this kit already reads, how many ever "
                "sustained a given rate for 5 and for 10 years, on revenue and on owners' "
                "earnings. Pinned data with its as-of date, recomputed by session, never "
                "fetched live. A best stretch can start at a trough — the window rides "
                "with every figure.\\n\\n"
    ...
    with _r2:
        with st.expander("Verify this page"):
            st.caption(
                "Two suites. **Engine** is the Tragic Algebra Analyzer's own self-test, "
                "running here on the verbatim copy — identical code, identical results, "
            ...
            if st.button("Run checks"):
                for _label, _fn in (("Engine (the copied suite)", self_test),
                                    ("Expectations", expectations_self_test)):
                    _results = _fn()
                    _sev, _line = test_summary(_results)
                    getattr(st, _sev)(f"{_label}: {_line}")
                    for _name, _ok, _got in _results:
                        st.write(("✅ " if _ok else "❌ ") + f"{_name} — {_got}")
    st.caption(
        "Research aid, not financial advice. This page prints what a price implies on "
        "stated assumptions, never what a company is worth. The framework follows Alfred "
        "Rappaport and Michael Mauboussin's published *Expectations Investing*, run on "
        "Michael Burry's owners'-earnings engine as this kit implements it; the "
        "inversions, refusals, evidence panels and base-rate table are this project's "
        "own design. Independent — not affiliated with or endorsed by any of them."
    )


st.set_page_config(
    page_title="Expectations — what the price implies, a reverse DCF on owners' earnings",
    page_icon="🔭",
    layout="centered",
...
st.caption(
    "What today's price implies — never what the company is worth. "
    "After Rappaport and Mauboussin's *Expectations Investing*, on the "
    "Tragic Algebra Analyzer's engine run backwards."
)
...
if "exp_years" not in st.session_state:
    st.info(
        "**A price is a claim about the future.** This page solves the claim out loud: "
        "the owners'-earnings growth it takes for the price to deliver the required "
        "return at your tier, and the year-15 exit it implies at the seeded growth — "
        "then puts that implied path against the company's own filed record and a "
        "base-rate table of the names this kit already reads. You judge plausibility; "
        "the page never will."
    )

with st.form("exp_lookup"):
    ticker = st.text_input("Stock ticker",
                           placeholder="MRVL · NVDA · ADP — press Enter").upper().strip()
    submitted = st.form_submit_button("What does the price imply?", type="primary")

tier_name = st.selectbox("Moat tier", list(AICT), index=2,
                         format_func=lambda t: f"{t} — {TIER_BLURB[t]}",
                         help="Sets stage lengths, how far growth fades in stage 2, the "
                              "terminal cap and the exit multiple — the frame the price is "
                              "inverted against. The biggest lever on this page, exactly as "
                              "on the Tragic Algebra Analyzer.")
_t = AICT[tier_name]
st.caption(
    f"**{_t.horizon}-year horizon · {_t.default_exit_multiple:g}× exit · "
    f"{_t.terminal_growth_cap:.0%} terminal.** A softer tier makes the same price imply "
    "a lower growth rate — set it deliberately, and read the implied rate as conditional "
    "on it."
)
required_return = st.number_input(
    "Required return (%)", min_value=RR_FLOOR, max_value=30.0, value=15.0, step=0.5,
    help="The return the buyer demands, which the implied path must deliver. 15% is the "
         "kit's standard and Burry's frame; 8–10% is the market's cost of capital and the "
         "Rappaport reading — what the MARKET expects rather than what a 15% buyer needs. "
         f"Floored at {RR_FLOOR:g}% so it always clears every tier's terminal growth cap.")
...
if submitted:
    if not ticker:
        ...
        try:
            ...
            st.session_state.update(exp_years=yrs, exp_notes=notes, exp_pre=pre,
                                    exp_tk=ticker)
...
years = st.session_state.get("exp_years", [])
if years and ticker and st.session_state.get("exp_tk") == ticker:
    notes, pre, tk = (st.session_state["exp_notes"], st.session_state["exp_pre"],
                      st.session_state["exp_tk"])
    pooled = pool(years)
    recent = pool_recent(years, 3) if len(years) >= 3 else pooled
    alerts: list[tuple[str, str]] = [("info", n) for n in notes]

    def _record_inside_stop():
        """The year-by-year record, rendered inside a refusal (the GRAB
        lesson: never point at notes a stopped page won't show)."""
        with st.expander("Notes and the year-by-year record", expanded=True):
    ...
    # an indicative ladder, this page prints nothing: an implied growth rate
    # solved on a zeroed-net-cash operating frame for a balance-sheet
    # business is exactly a number this page cannot stand behind.
    if pre.get("financial"):
        _label = {"bank": "a bank", "insurer": "an insurer", "reit": "a REIT",
                  "broker": "a broker",
                  "refused": pre.get("sic_desc") or "a financial company"}.get(
                      pre.get("fin_class", ""), "a financial company")
        st.error(financial_stop_sentence(_label, pre.get("fin_reason", "")))
        st.caption(f"ΔE measured all the same — last 3 years: "
                   f"{dE_caption(recent.dE, recent.dE_defined, False)} · full period: "
                   f"{dE_caption(pooled.dE, pooled.dE_defined, False)}.")
        _record_inside_stop()
        _page_footer()
        st.stop()

    # ══ foreign stop — the reader's own note, upgraded to a refusal ══════
    # The Tragic Algebra Analyzer prints the IFRS note and proceeds with its
    # figures on display; this page's whole output is one solved number, so
    # a reading the note itself calls unverified cannot feed it.
    _FOREIGN_MARK_LOCAL = "foreign private issuer reporting under IFRS"
    _foreign_notes = [n for n in notes if _FOREIGN_MARK_LOCAL in n]
    if _foreign_notes:
        st.error(_foreign_notes[0])
        st.error(foreign_stop_sentence())
        _record_inside_stop()
        _page_footer()
        st.stop()

    # ══ no share count — the Tragic Algebra Analyzer's stop, unchanged ═══
    if pre.get("shares", 0) <= 0:
        st.error(
            "**No share count was read from any tag this reader knows** — the notes below "
            "say which years. Nothing per share can be computed, and an implied growth "
            "rate is a per-share claim. The usual shape is a dual-class filer: per-class "
            "counts carry a class dimension that EDGAR's companyfacts API strips. Type "
            "the diluted count from the 10-K cover page into the Tragic Algebra Analyzer "
            "to study the name; this page seeds from the filings alone.")
        _record_inside_stop()
        _page_footer()
        st.stop()

    # ══ inputs — the Tragic Algebra Analyzer's seeding, verbatim rules ═══
    st.markdown("---")
    st.subheader("The base the price is inverted against")
    ...
    use_recent = st.radio(
        ...
        help="ΔE is the share of reported profit that actually reaches shareholders. The "
             "long window is the diagnostic; where capital policy has changed, the recent "
             "one is what will apply going forward.") == "Last 3 years"
    ...
    fwd_N = c1.number_input("Forward net income ($M)",
    ...
    OE = c1.number_input("Owners' earnings ($M)", value=float(round(derived, 1)), step=1.0,
                         help="The base the implied growth compounds from. Seeded from "
                              "forward net income × ΔE, the Tragic Algebra Analyzer's "
                              "rule; adjust for maintenance capex, working capital and "
                              "anything non-recurring.")
    shares = c2.number_input("Diluted shares (M)", value=float(round(pre["shares"], 1)),
                             step=1.0)
    price = c3.number_input("Price", value=float(current_price(pre.get("ticker", tk))
                                                 or 100.0), step=0.01)
    cash = c3.number_input("Cash & investments ($M)",
                           value=float(round(pre.get("cash", 0.0), 1)), step=10.0,
                           help="Only what is freely deployable. Restricted, regulated "
                                "and operationally-tied cash funds the business.")
    debt = c2.number_input("Total debt ($M)", value=float(round(pre.get("debt", 0.0), 1)),
                           step=10.0, help="Short-term plus long-term borrowings.")
    net_cash = cash - debt
    seeded_growth = pre.get("growth", 0.08)
    c1.caption(f"Net cash {d(net_cash, 0)}M · seeded growth for the exit solve "
               f"{seeded_growth:.1%} (the most recent year of revenue growth, the "
               "Tragic Algebra Analyzer's seed)")

    if seed_source == SEED_FROM_MEDIAN_LOSS:
        st.error(
            f"**The forward year is a loss on a profitable record.** Net income for the "
            f"latest year is {d(fwd_N, 0)}M, while the five-year median of owners' "
            f"earnings is {d(median_OE, 0)}M. A single loss year on a record like that is "
            "usually a non-cash charge, and an implied growth rate measured against it "
            "would describe the charge, not the business. Owners' earnings above are "
            f"therefore seeded from the median, {d(median_OE, 0)}M; set the box to what "
            "the business earns in a normal year.")
    if dE_capped:
        alerts.append(("warning",
            f"ΔE measured {use_dE:.1%} over this window — a real reading, left as filed "
            "in the record below. It is not projectable: the base above is seeded at "
            f"{DE_SEED_CEILING:.0%} of forward net income rather than {use_dE:.1%}, the "
            "Tragic Algebra Analyzer's ceiling rule. Raise the box by hand if you can "
            "say why owners should keep more than the company reports earning."))
    if not dE_ok:
        alerts.append(("error",
            "ΔE cannot be projected on this window — the seeding fell back per the "
            "Tragic Algebra Analyzer's rules, and the base box above is the number to "
            "set. The seed line in the assumptions block records where it came from."))

    # ══ the negative-base route — this page's own refusal ════════════════
    if OE <= 0:
        st.error(inflection_route_sentence(OE, median_OE))
        _record_inside_stop()
        _page_footer()
        st.stop()

    base = IVParams(OE=OE, shares=shares, tier=tier_name, growth=0.0,
                    net_cash=net_cash,
                    exit_multiple=AICT[tier_name].default_exit_multiple,
                    blend=0.5, m2_style="dcf", stage0_years=0, stage0_growth=0.0)

    # ══ the solves ═══════════════════════════════════════════════════════
    st.markdown("---")
    st.subheader(f"What the price implies · {tk}")
    st.caption(
        f"Frame for the growth solve: the Tragic Algebra Analyzer's IV15 at its page "
        f"defaults — long-horizon weight 0.5, cash-flows-plus-exit leg, the tier's "
        f"{AICT[tier_name].default_exit_multiple:g}× exit, no stage 0 — with net cash of "
        f"{d(net_cash, 0)}M added, at a {required_return:g}% required return. The exit "
        "solve states its own frame beside its sentence; both are in the assumptions "
        "block."
    )

    branch, g_star = solve_price_growth(price, base, required_return)
    rates = None
    if branch == "below":
        _iv0 = intrinsic_value(IVParams(**{**base.__dict__, "growth": 0.0}),
                               required_return)
        st.error(below_zero_sentence(tk, price, _iv0, required_return))
    elif branch == "ceiling":
        st.error(ceiling_sentence(tk, price, required_return))
    else:
        resid = identity_residual(price, base, g_star, required_return)
        if abs(resid) >= IDENTITY_TOL:
            st.error(identity_refusal_sentence(resid))
            _record_inside_stop()
            _page_footer()
            st.stop()
        rates = implied_path_rates(base, g_star)
        v1, v2, v3 = st.columns(3)
        v1.metric("Implied growth g*", f"{g_star:.1%}",
                  f"stage-1 rate, {tier_name}", delta_color="off")
        v2.metric("10-year demand", f"{rates[10]:.1%}",
                  "the path's first decade", delta_color="off")
        v3.metric("15-year flat equivalent", f"{rates[15]:.1%}",
                  "the whole path, one rate", delta_color="off")
        st.caption(
            f"Identity, run live: IV15 at the solved rate is {d(price + resid)} against "
            f"the {d(price)} price — residual within the half-cent the solve must "
            "reproduce."
        )
        own = own_history(tk, years)
        st.markdown(
            growth_sentence(tk, price, tier_name, required_return, g_star, rates)
            + " " + own_evidence_sentence(tk, own)
            + " " + base_evidence_sentence(g_star, rates)
        )

    # ══ the second solve — the price-implied exit ════════════════════════
    xbranch, x_star, iv_flows = solve_price_exit(price, base, required_return,
                                                 seeded_growth)
    if xbranch == "solved":
        st.info(exit_sentence(tk, seeded_growth, x_star, tier_name))
    elif xbranch == "zero":
        st.info(exit_zero_sentence(tk, seeded_growth, iv_flows, price))
    elif xbranch == "beyond":
        st.info(exit_beyond_sentence(tk, seeded_growth))
    st.caption(
        "Frame for the exit solve: long-horizon weight 0 — the cash flows for 15 years "
        f"at the seeded {seeded_growth:.1%} on the tier's stage shape, plus the year-15 "
        "sale the solve prices; net cash added. Rappaport and Mauboussin's price-implied "
        "terminal."
    )

    # ══ evidence panel — the company against itself ══════════════════════
    if branch != "solved":
        own = own_history(tk, years)
    st.markdown("---")
    st.subheader("The evidence")
    st.write(f"**{tk} against itself** — best filed stretches, windows named")
    _fmt_b = lambda b, sp: "—" if b is None else f"{b[0]:.1%} (FY{b[1]}→FY{b[1] + sp})"
    _fmt_f = lambda f: "—" if f is None else f"{f[0]:.1%} (FY{f[1]}→FY{f[2]}, {f[2] - f[1]}y)"
    st.dataframe(pd.DataFrame([
        {"Series": "Owners' earnings", "Best 5-year stretch": _fmt_b(own["oe5"], 5),
         "Full window": _fmt_f(own["oe_full"])},
        {"Series": "Revenue", "Best 5-year stretch": _fmt_b(own["rev5"], 5),
         "Full window": _fmt_f(own["rev_full"])},
    ]), width='stretch', hide_index=True)
    st.caption(
        "From this page's own ten-year read, the same endpoint rules as the base table: "
        "endpoints positive, excluded years barred, owners' earnings only where a year "
        "has a price. A best stretch can start at a trough — the window says where. A "
        "dash means no pair of admissible endpoints exists for that cell."
    )

    # ══ evidence panel — the base rates ══════════════════════════════════
    st.write(f"**The base rates** — {len(BASE_RATE_ROWS)} names this kit reads, "
             f"as of {BASE_RATES_AS_OF}")
    _grid = pd.DataFrame([{
        "Sustained": f"{th:.0%}",
        f"Revenue, 5y (of {count_names_at(-10.0, 'rev', 5)[1]})":
            count_names_at(th, "rev", 5)[0],
        f"Revenue, 10y (of {count_names_at(-10.0, 'rev', 10)[1]})":
            count_names_at(th, "rev", 10)[0],
        f"Owners' earnings, 5y (of {count_names_at(-10.0, 'oe', 5)[1]})":
            count_names_at(th, "oe", 5)[0],
        f"Owners' earnings, 10y (of {count_names_at(-10.0, 'oe', 10)[1]})":
            count_names_at(th, "oe", 10)[0],
    } for th in BASE_RATE_THRESHOLDS])
    st.dataframe(_grid, width='stretch', hide_index=True)
    st.caption(
        "How many names had a best endpoint stretch at or above each rate, for each span "
        "— counted from the pinned per-name records below, on a sixteen-year read. "
        "\"Of N\" is the column's own coverage: revenue history under the current tags "
        "reaches ten years for many filers (a documented reader property), and a "
        "ten-year owners'-earnings stretch needs eleven priced years, so the ten-year "
        "columns are thinner than the five-year ones. The output sentence above counts "
        "at the implied demand exactly, from the same records — never by nearest "
        "threshold."
    )
    with st.expander("The names behind the table"):
        st.dataframe(pd.DataFrame([{
            "Ticker": r.ticker,
            "Rev best 5y": _fmt_b((r.rev5, r.rev5_win[0]) if r.rev5 is not None else None, 5),
            "Rev best 10y": _fmt_b((r.rev10, r.rev10_win[0]) if r.rev10 is not None else None, 10),
            "OE best 5y": _fmt_b((r.oe5, r.oe5_win[0]) if r.oe5 is not None else None, 5),
            "OE best 10y": _fmt_b((r.oe10, r.oe10_win[0]) if r.oe10 is not None else None, 10),
        } for r in BASE_RATE_ROWS]), width='stretch', hide_index=True,
            height=min(38 * len(BASE_RATE_ROWS) + 40, 1000))
        for _t_, _why in BASE_RATE_DROPPED:
            st.write(f"· **{_t_}** — dropped: {_why}")
        st.caption(
            "Captured by the Baselines app's base-rate capture and pinned here as data "
            "with its date. Two raw series fragments ride with the pins, and this page's "
    ...
    with st.expander(label):
        ...
        _med_N = median_positive_N([y.N for y in years])
        _mfmt = money_fmt([v for y in years for v in (y.N, y.G, y.T, y.omega, y.OE)])
        st.write("**Year by year** — identical to the Tragic Algebra Analyzer for the "
                 "same ticker")
        st.dataframe(pd.DataFrame([{
            "FY": f"{y.fy}*" if y.excluded else str(y.fy),
            "Net income": y.N, "GAAP SBC": y.G, "Buybacks": y.T,
        ...
        _g_line = (f"{g_star:.6%}   identity residual {resid:+.6f}"
                   if branch == "solved" else branch.upper())
        _x_line = (f"{x_star:.4f}x" if xbranch == "solved" else xbranch.upper())
        st.code(
            ...
            f"tier                {tier_name}   required return {required_return:g}%\n"
            f"frame, growth solve blend 0.5   leg dcf   exit "
            f"{AICT[tier_name].default_exit_multiple:g}x   stage 0 none\n"
            f"frame, exit solve   blend 0.0   leg dcf   growth held "
            f"{seeded_growth:.2%} (revenue seed)\n"
            f"implied growth g*   {_g_line}\n"
            f"implied exit        {_x_line}\n"
            f"base table          {BASE_RATES_AS_OF}   {len(BASE_RATE_ROWS)} names",
            language="text")
