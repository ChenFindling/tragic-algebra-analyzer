...
def load(ticker: str, n_years: int = 10):
    ...
    return years, notes, {"up_c_basis": _upcb, "tags": tags, "net_cash": net_cash, "cash": cash_total, "debt": debt_total,
                          ...
                          "sic_desc": sic_desc,
                          "financial": fin_class in ("bank", "insurer", "reit", "broker", "refused"),
...
def up_c_sentence_dcf(ticker: str, total_m: float) -> str:
    """The surviving Up-C refusal in this page's own terms (§5.5 G,
    15 Sep 2026). The shared up_c_sentence says "the table and the
    measurements above" — this page's evidence is its cash-flow legs, and
    a sentence may only point at evidence the page shows: the same
    page-local-truth doctrine as the 10 Sep 2026 sentence this supersedes
    (Chen's decision of record — no false sentence at any price)."""
    return (
        f"**Valuation withheld — Up-C structure, consolidated income unverified.** "
        f"{ticker}'s per-class share counts were read from its filings' own XBRL "
        f"instances and summed ({total_m:,.1f}M across the classes), so the free "
        "cash flow history and the per-year Ω column above are real measurements. "
        "This page prices umbrella-partnership C-corps on the as-exchanged basis — "
        "the SBC-corrected leg's whole-company cash flows over that full count — "
        "but the filed consolidated income legs that verify the basis could not be "
        "read for this filer, and per-unit claims on an unverified basis are "
        "exactly what this page refuses to print.")


def up_c_banner_dcf(ticker: str, total_m: float, basis: dict) -> str:
    """The as-exchanged basis statement in this page's own terms — the
    shared banner says per-share figures divide consolidated EARNINGS,
    true on the earnings pages and false here: this page's legs are
    whole-company cash flows already. Same page-local-truth doctrine as
    up_c_sentence_dcf, carried into §5.5 G (15 Sep 2026)."""
    spec = UP_C_INCOME[ticker]
    return (
        f"**Priced on the as-exchanged basis — Up-C structure.** {ticker}'s "
        f"per-class share counts were read from its filings' own XBRL "
        f"instances and summed ({total_m:,.1f}M across the classes). Both "
        "legs on this page are whole-company cash flows already — CFO minus "
        "capex, and the Ω stream — so the basis change here is the count: "
        "every per-unit figure divides those consolidated flows by the full "
        "as-exchanged count, at the Class A price the exchange right gives "
        "every unit. " + basis.get("identity", "")
        + "Two real limits are named here, not adjusted for: the **Tax "
        f"Receivable Agreement** (`{spec.tra_tag}`, \\${spec.tra_val_m:,.0f}M "
        f"at FY{spec.tra_fy}, from \\${spec.tra_prior_m:,.0f}M a year earlier) "
        "transfers value to pre-IPO holders outside this arithmetic — "
        "direction: flattering. And the **tax-status difference**: the "
...
def self_test() -> list[tuple[str, bool, str]]:
    ...
    out.append(("F1 amended: an episodic line is a capital event whatever its size (KNSL)",
                ...
                "note claims"))
    out.append(("Page-local Up-C pair: legs named on both sentences, no ΔE-pool claim, banner never withholds",
                ...
                and "unverified" in up_c_sentence_dcf("CVNA", 1091.7)
                and "cash flows" in up_c_banner_dcf("CVNA", 1091.7, {"identity": ""})
                and "withheld" not in up_c_banner_dcf("CVNA", 1091.7, {"identity": ""})
    ...
    from pathlib import Path as _Pg2
    _sp = _Pg2(__file__).read_text(encoding="utf-8")
    out.append(("Gate 2: the note names the years and the old hedge is retired",
...
if years and ticker and st.session_state.get("dcf_tk") == ticker:
    ...
    # ══ Up-C basis (§5.5 G, 15 Sep 2026 — supersedes the queue-H stop) ══
    # Verified consolidated income → the page-local basis banner (this
    # page's legs are whole-company cash flows already, so the count is the
    # only basis change — the banner says so in this page's own terms);
    # unverified → the page-local surviving stop. Placement unchanged:
    # after nothing-to-discount (the RIVN ordering), before the share-count
