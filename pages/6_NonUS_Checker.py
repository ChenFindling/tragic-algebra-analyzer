...
def load(ticker: str, n_years: int = 10, price_symbol: str = "", ads_ratio: float = 1.0):
    ...
    _px_ccy = ""
    try:
        closes, splits, _px_ccy = _monthly_closes(_px_sym, _px_start)
    except Exception:
        closes, splits = {}, {}
...
def self_test() -> list[tuple[str, bool, str]]:
    ...
    out.append(("F1 amended: an episodic line is a capital event whatever its size (KNSL)",
                ...
                "note claims"))
    out.append(("Sweep: the router sentence names the menu page, no numeric reference",
                _route_ok(sent_to_tool1(_shop)), sent_to_tool1(_shop)[-70:]))
    _swf = nonus_financial_refusal("6331", "Fire, Marine & Casualty Insurance")
    out.append(("Sweep: the financial refusal names the menu page, no numeric reference",
                _route_ok(_swf), _swf[:70]))
    ...
    from pathlib import Path as _Pc6
    _sc6 = _Pc6(__file__).read_text(encoding="utf-8")
...
if PASTE:
    ...
    if st.button("Compute", type="primary"):
        ...
        _px_err = ""
        if _p_sym:
            # Same defect, paste route: the averages are over CALENDAR years,
            # so the request starts at Jan 1 of the earliest typed FY.
            _p_fys = [int(_cell(_r.get("FY"))) for _r in _rows
                      if _cell(_r.get("FY")) is not None]
            _p_start = f"{min(_p_fys):04d}-01-01" if _p_fys else None
            try:
                _cl, _sp, _pxc = _monthly_closes(_p_sym, _p_start)
            except Exception:
                _cl, _pxc = {}, ""
