"""
Magic Formula — Joel Greenblatt's two legs
==========================================
Earnings yield (EBIT / enterprise value) and return on capital
(EBIT / (net working capital + net fixed assets)) — his definitions, his
exclusions, computed honestly for ONE ticker at a time from audited SEC
filings.

THE REFUSAL THE PAGE IS BUILT ON
--------------------------------
The Magic Formula is a market-wide RANKING, and a ranking needs a
universe this kit does not fetch. This page computes the two legs and
shows where the ticker sits against a small pinned table of the names
this kit reads — as-of dated, honest N — context, never a rank, never a
buy signal, never one combined score. Beside each leg, the kit's
signature: the same leg on SBC-corrected EBIT (EBIT + G − Ω), the gap
exact by linearity, because a formula screened on pre-SBC earnings is
exactly where the hidden cost hides.

Where no operating-income subtotal is filed, EBIT is derived as revenue
minus the filed all-in expense total, the subtraction printed as
arithmetic on the page (the H&R Block shape). Financials refuse with
Greenblatt's own exclusion as the stated reason — the rare case where
the author's rule and this kit's gate agree exactly — except managed
care at exactly SIC 6324, which his own screen publishes and this page
therefore computes, with the boundary decision stated in a banner.
Utilities refuse with his other exclusion, SIC-range honest.

The reader below is the Tragic Algebra Analyzer's, per the kit's
per-page copy pattern, byte-verbatim except ONE dated additive
FROZEN_MENU entry: this page adds no concept lines — every line it
needs is read page-locally through the reader's own machinery, exactly
as the Baselines app's capture read them. Reader fixes land in every
carrier file, this one now included.

THE MENU MAP OF RECORD FOR THIS FILE (18 Sep 2026)
--------------------------------------------------
    Home (entrypoint)
    ...
    5   Financials Checker           10  EPV
    6   NonUS Checker                11  Magic Formula (this file)
                                     99  Return Calculator (last, permanently)

3 is retired and is never reused. The in-span header comment below stays
at its 12-Sep vintage on purpose: refreshing it is a fleet-wide wording
ride, queued, and editing it here alone would fork the copies.
...
FROZEN_MENU = ("Tragic Algebra Analyzer", "100-Bagger Checker", "Inflection Checker",
               ...
               "Return Calculator",
               # Magic Formula page, 18 Sep 2026: this page routes to the
               # EPV page (the no-normalization sentence), which postdates
               # the fleet's tuple. The fleet-wide tuple/map refresh is a
               # queued wording ride; this entry is page-local and additive.
               "EPV")
...
# ══════════════════════════════════════════════════════════════════════
#  MAGIC FORMULA — Greenblatt's two legs (this page's own code; the reader
#  above is the Tragic Algebra Analyzer's)
# ══════════════════════════════════════════════════════════════════════
#
# Everything from here down is this page's own. The span above is
# byte-verbatim except one dated, additive FROZEN_MENU entry — this page
# adds NO concept lines to the reader: every line it needs (EBIT ladder,
# balance lines, interest) is read page-locally through the span's own
# _annual and _instant with the MF_TAGS lists below, exactly as the
# Baselines app's capture read them, so the page and the capture cannot
# drift.
#
# Whose method: Joel Greenblatt ("The Little Book That Beats the
# Market") — earnings yield (EBIT / enterprise value) and return on
# capital (EBIT / (net working capital + net fixed assets)), his
# definitions, his exclusions. What is ours: every refusal, the
# refusal to RANK (the formula is a market-wide ranking and a ranking
# needs a universe this kit does not fetch), the SBC-corrected second
# reading of each leg with the gap exact by linearity, the derived-EBIT
# ladder with its arithmetic printed, and the cash waterfall convention
# shared with the 100-Bagger Checker.

MF_MIN_CAPITAL_EV = 0.01     # the near-zero-capital caption's stated trigger
MF_UTILITY_SIC = (4900, 4999)  # Greenblatt's other exclusion, SIC-range honest
...
# ── The pinned reference table — captured data, recomputed by session ──
#
# Cells from records: every displayed cell recomputes from the raw
# components below at render time, so the table cannot disagree with
# the record it displays. The self-tests assert MF_TAGS_USED equals
# this page's own MF_TAGS — capture-vs-page drift fails loudly — and
# recompute both spot fragments with the identical functions.
...
# ── MF_REFERENCE — pinned data for pages/11_Magic_Formula.py ─────────
# Captured 2026-09-18 by the Baselines app's MF capture. Yield uses
# this date's prices; ROC moves only with new 10-Ks.
MF_REFERENCE_AS_OF = "2026-09-18"
MF_TAGS_USED = {'OI': (['OperatingIncomeLoss'], ['ProfitLossFromOperatingActivities']), 'CAE': (['CostsAndExpenses'], []), 'INTEXP': (['InterestExpense', 'InterestExpenseNonoperating', 'InterestIncomeExpenseNet'], []), 'CA': ['AssetsCurrent'], 'CL': ['LiabilitiesCurrent'], 'PPE': ['PropertyPlantAndEquipmentNet', 'PropertyPlantAndEquipmentAndFinanceLeaseRightOfUseAssetAfterAccumulatedDepreciationAndAmortization']}
MF_LAG_CEILING_USED = 3
MF_OP_CASH_PCT_USED = 0.02
MF_REFERENCE_ROWS = [
    MFRefRow(ticker='AAPL', fy=2025, rung='D1', lag=0,
             ebit=133050.0, gmw=-5603.248548551113, price=336.14, shares=14773.26,
             mktcap=4965883.6164, debt=90678.0, cash_total=132420.0,
             floor=8323.22, excess=124096.78, ca=147957.0, cl=165631.0, std=12350.0,
             cash_current=54697.0, kept=8323.22, nwc=-51697.78, ppe=49834.0,
             ppe_tag='PropertyPlantAndEquipmentNet', stale=()),
    MFRefRow(ticker='ADBE', fy=2025, rung='D1', lag=0,
             ebit=8706.0, gmw=1571.7723576472363, price=250.28, shares=413.0,
             mktcap=103365.64, debt=6210.0, cash_total=6595.0,
             floor=475.38, excess=6119.62, ca=10163.0, cl=10200.0, std=0.0,
             cash_current=6595.0, kept=475.38, nwc=-6156.62, ppe=1873.0,
             ppe_tag='PropertyPlantAndEquipmentNet', stale=()),
    MFRefRow(ticker='ADI', fy=2025, rung='D1', lag=0,
             ebit=2932.496, gmw=-230.7762338887307, price=370.035, shares=489.654097,
             mktcap=181189.153783395, debt=8145.066, cash_total=3774.6059999999998,
             floor=220.39414000000002, excess=3554.21186, ca=7108.061, cl=3245.801, std=0.0,
             cash_current=3652.321, kept=220.39414000000002, nwc=430.33313999999973, ppe=3315.696,
             ppe_tag='PropertyPlantAndEquipmentNet', stale=()),
    MFRefRow(ticker='AMGN', fy=2025, rung='D1', lag=0,
             ebit=9080.0, gmw=-75.92874806722, price=385.675, shares=538.8,
             mktcap=207801.69, debt=54604.0, cash_total=9129.0,
             floor=735.02, excess=8393.98, ca=29057.0, cl=25489.0, std=4599.0,
             cash_current=9129.0, kept=735.02, nwc=-226.97999999999956, ppe=7913.0,
             ppe_tag='PropertyPlantAndEquipmentNet', stale=('STI at FY2023, 2 behind',)),
    MFRefRow(ticker='AMZN', fy=2025, rung='D1', lag=0,
             ebit=79975.0, gmw=-10892.7699508667, price=254.16, shares=10731.0,
             mktcap=2727390.96, debt=68396.0, cash_total=123029.0,
             floor=14338.48, excess=108690.52, ca=229083.0, cl=218005.0, std=2748.0,
             cash_current=123029.0, kept=14338.48, nwc=-94864.52, ppe=357025.0,
             ppe_tag='PropertyPlantAndEquipmentNet', stale=()),
    MFRefRow(ticker='COST', fy=2025, rung='D1', lag=0,
             ebit=10383.0, gmw=-542.6589762420654, price=896.9, shares=443.237,
             mktcap=397539.2653, debt=5788.0, cash_total=15284.0,
             floor=5504.7, excess=9779.3, ca=38380.0, cl=37108.0, std=75.0,
             cash_current=15284.0, kept=5504.7, nwc=-8432.3, ppe=31909.0,
             ppe_tag='PropertyPlantAndEquipmentNet', stale=()),
    MFRefRow(ticker='CRM', fy=2026, rung='D1', lag=0,
             ebit=8331.0, gmw=105.952392578125, price=239.52, shares=929.0,
             mktcap=222514.08000000002, debt=14439.0, cash_total=10948.996,
             floor=830.5, excess=10118.496, ca=28222.0, cl=37118.0, std=4000.0,
             cash_current=9565.0, kept=830.5, nwc=-13630.5, ppe=3120.0,
             ppe_tag='PropertyPlantAndEquipmentAndFinanceLeaseRightOfUseAssetAfterAccumulatedDepreciationAndAmortization', stale=()),
    MFRefRow(ticker='CSCO', fy=2026, rung='D1', lag=0,
             ebit=15368.0, gmw=-2144.9430894118086, price=108.92, shares=3946.0,
             mktcap=429798.32, debt=22872.0, cash_total=15918.0,
             floor=1266.5, excess=14651.5, ca=38665.0, cl=41525.0, std=3500.0,
             cash_current=15918.0, kept=1266.5, nwc=-14011.5, ppe=2760.0,
             ppe_tag='PropertyPlantAndEquipmentNet', stale=()),
    MFRefRow(ticker='GOOGL', fy=2025, rung='D1', lag=0,
             ebit=129039.0, gmw=-8293.602443695068, price=351.8, shares=12088.0,
             mktcap=4252558.4, debt=48543.0, cash_total=127109.0,
             floor=8056.72, excess=119052.28, ca=206038.0, cl=102745.0, std=1996.0,
             cash_current=126843.0, kept=8056.72, nwc=-13497.279999999999, ppe=246597.0,
             ppe_tag='PropertyPlantAndEquipmentAndFinanceLeaseRightOfUseAssetAfterAccumulatedDepreciationAndAmortization', stale=()),
    MFRefRow(ticker='INTU', fy=2026, rung='D1', lag=0,
             ebit=5884.0, gmw=1443.9737816238403, price=302.98, shares=268.384,
             mktcap=81314.98432, debt=7669.0, cash_total=5438.0,
             floor=428.96000000000004, excess=5009.04, ca=15311.0, cl=10163.0, std=1249.0,
             cash_current=4705.0, kept=428.96000000000004, nwc=2120.959999999999, ppe=1023.0,
             ppe_tag='PropertyPlantAndEquipmentNet', stale=('STI last filed FY2022, 4 behind — beyond the ceiling, treated as no longer filed, excluded at zero',)),
    MFRefRow(ticker='META', fy=2025, rung='D1', lag=0,
             ebit=83276.0, gmw=-21530.58998616536, price=673.43, shares=2530.0,
             mktcap=1703777.9, debt=58744.0, cash_total=81592.0,
             floor=4019.32, excess=77572.68, ca=108722.0, cl=41836.0, std=0.0,
             cash_current=81592.0, kept=4019.32, nwc=-10686.68, ppe=176400.0,
             ppe_tag='PropertyPlantAndEquipmentAndFinanceLeaseRightOfUseAssetAfterAccumulatedDepreciationAndAmortization', stale=('STD: no year at or before FY2025',)),
    MFRefRow(ticker='MSFT', fy=2026, rung='D1', lag=0,
             ebit=155237.0, gmw=-10262.771662394207, price=496.04, shares=7427.0,
             mktcap=3684089.08, debt=40294.0, cash_total=113191.0,
             floor=6636.78, excess=106554.22, ca=207710.0, cl=168825.0, std=9227.0,
             cash_current=76843.0, kept=6636.78, nwc=-22094.22, ppe=313076.0,
             ppe_tag='PropertyPlantAndEquipmentNet', stale=()),
    MFRefRow(ticker='NFLX', fy=2025, rung='D1', lag=0,
             ebit=13326.603, gmw=-2012.118733456547, price=71.94, shares=4222.16215,
             mktcap=303742.345071, debt=14462.836, cash_total=9062.359,
             floor=903.66072, excess=8158.6982800000005, ca=13020.191, cl=10980.93, std=998.865,
             cash_current=9062.359, kept=903.66072, nwc=-5120.57228, ppe=2004.35,
             ppe_tag='PropertyPlantAndEquipmentNet', stale=()),
    MFRefRow(ticker='NVDA', fy=2026, rung='D1', lag=0,
             ebit=130387.0, gmw=-13707.793797419617, price=219.53, shares=24304.0,
             mktcap=5335457.12, debt=8468.0, cash_total=45226.0,
             floor=4318.76, excess=40907.24, ca=125605.0, cl=32163.0, std=999.0,
             cash_current=45226.0, kept=4318.76, nwc=53533.759999999995, ppe=10383.0,
             ppe_tag='PropertyPlantAndEquipmentNet', stale=('STI at FY2025, 1 behind',)),
    MFRefRow(ticker='PAYX', fy=2026, rung='D1', lag=0,
             ebit=2510.5, gmw=46.72984193166096, price=116.12, shares=355.6,
             mktcap=41292.272000000004, debt=4556.1, cash_total=1124.5,
             floor=126.022, excess=998.478, ca=8613.3, cl=6840.6, std=0.0,
             cash_current=1124.5, kept=126.022, nwc=774.2219999999988, ppe=588.9,
             ppe_tag='PropertyPlantAndEquipmentNet', stale=()),
    MFRefRow(ticker='QCOM', fy=2025, rung='D1', lag=0,
             ebit=12355.0, gmw=-552.6500549316406, price=178.455, shares=1074.0,
             mktcap=191660.67, debt=14811.0, cash_total=10155.0,
             floor=885.6800000000001, excess=9269.32, ca=25754.0, cl=9144.0, std=0.0,
             cash_current=10155.0, kept=885.6800000000001, nwc=7340.68, ppe=4690.0,
             ppe_tag='PropertyPlantAndEquipmentNet', stale=()),
    MFRefRow(ticker='TTWO', fy=2026, rung='D1', lag=0,
             ebit=-104.2, gmw=-1637.4206649780276, price=207.63, shares=185.4,
             mktcap=38494.602, debt=2518.0, cash_total=1989.3,
             floor=133.128, excess=1856.172, ca=3199.5, cl=2588.8, std=30.0,
             cash_current=1989.3, kept=133.128, nwc=-1215.4720000000002, ppe=445.4,
             ppe_tag='PropertyPlantAndEquipmentNet', stale=()),
    MFRefRow(ticker='TXN', fy=2025, rung='D1', lag=0,
             ebit=6023.0, gmw=-186.26381885579303, price=263.76, shares=907.550774,
             mktcap=239375.59215024, debt=14048.0, cash_total=4897.0,
             floor=353.64, excess=4543.36, ca=13750.0, cl=3159.0, std=500.0,
             cash_current=4881.0, kept=353.64, nwc=6563.639999999999, ppe=12320.0,
             ppe_tag='PropertyPlantAndEquipmentNet', stale=()),
    MFRefRow(ticker='CLMB', fy=2025, rung='D1', lag=0,
             ebit=29.185, gmw=1.6583956321487432, price=27.87, shares=18.442472,
             mktcap=513.99169464, debt=0.191, cash_total=36.563,
             floor=13.050340000000002, excess=23.51266, ca=374.235, cl=337.487, std=0.191,
             cash_current=36.563, kept=13.050340000000002, nwc=13.426339999999982, ppe=13.339,
             ppe_tag='PropertyPlantAndEquipmentNet', stale=('STI last filed FY2013, 12 behind — beyond the ceiling, treated as no longer filed, excluded at zero',)),
    MFRefRow(ticker='CROX', fy=2025, rung='D1', lag=0,
             ebit=149.515, gmw=36.701, price=120.175, shares=50.2,
             mktcap=6032.785, debt=1230.885, cash_total=130.354,
             floor=80.81294, excess=49.541060000000016, ca=885.883, cl=700.129, std=0.0,
             cash_current=130.354, kept=80.81294, nwc=136.21294, ppe=238.191,
             ppe_tag='PropertyPlantAndEquipmentNet', stale=('STD at FY2024, 1 behind', 'STI: no year at or before FY2025')),
    MFRefRow(ticker='PDEX', fy=2026, rung='D1', lag=0,
             ebit=12.991, gmw=0.7729999999999999, price=71.5, shares=3.186135,
             mktcap=227.80865250000002, debt=17.513, cash_total=9.496,
             floor=1.5509600000000001, excess=7.9450400000000005, ca=53.248, cl=12.983, std=0.0,
             cash_current=9.342, kept=1.5509600000000001, nwc=32.473960000000005, ppe=5.373,
             ppe_tag='PropertyPlantAndEquipmentNet', stale=('STD: no year at or before FY2026',)),
    MFRefRow(ticker='PLTR', fy=2025, rung='D1', lag=0,
             ebit=1414.015, gmw=-6689.458108292896, price=177.197, shares=2391.192,
             mktcap=423712.048824, debt=0.0, cash_total=7177.043000000001,
             floor=89.50892, excess=7087.53408, ca=8358.174, cl=1175.581, std=0.0,
             cash_current=7177.043000000001, kept=89.50892, nwc=95.0589200000004, ppe=51.96,
             ppe_tag='PropertyPlantAndEquipmentNet', stale=('STD: no year at or before FY2025',)),
    MFRefRow(ticker='RDDT', fy=2025, rung='D1', lag=0,
             ebit=441.984, gmw=-1601.2875859794992, price=151.75, shares=190.892108,
             mktcap=28967.877389, debt=0.0, cash_total=2476.8109999999997,
             floor=44.05012, excess=2432.76088, ca=3135.985, cl=271.283, std=0.0,
             cash_current=2476.8109999999997, kept=44.05012, nwc=431.9411200000004, ppe=12.71,
             ppe_tag='PropertyPlantAndEquipmentNet', stale=('STD: no year at or before FY2025',)),
    MFRefRow(ticker='TGTX', fy=2025, rung='D1', lag=0,
             ebit=123.324, gmw=-6.647100873757992, price=57.575, shares=155.305953,
             mktcap=8941.740243975, debt=245.645, cash_total=201.106,
             floor=12.325740000000001, excess=188.78026, ca=630.764, cl=153.76, std=0.0,
             cash_current=141.97, kept=12.325740000000001, nwc=347.35974, ppe=0.095,
             ppe_tag='PropertyPlantAndEquipmentNet', stale=('STD at FY2022, 3 behind', 'PPE at FY2023, 2 behind')),
    MFRefRow(ticker='XPEL', fy=2025, rung='D1', lag=0,
             ebit=62.649, gmw=1.2396524418306352, price=45.21, shares=27.604183,
             mktcap=1247.98511343, debt=0.458188, cash_total=50.864,
             floor=9.524, excess=41.339999999999996, ca=230.697, cl=71.0, std=0.0,
             cash_current=50.864, kept=9.524, nwc=118.357, ppe=15.797,
             ppe_tag='PropertyPlantAndEquipmentNet', stale=('STD: no year at or before FY2025', 'STI: no year at or before FY2025')),
]
MF_REFERENCE_DROPPED = [
    ('BBW',
     'EBIT refused: a filed subtotal exists but stopped more than 3 years behind the filings, with no all-in expense total to derive from'),
]
MF_SPOT_YIELD = ('AAPL', 133050.0, 4932464.836399999, 0.026974343338067797)
MF_SPOT_ROC = ('AAPL', 133050.0, 49834.0, 2.6698639483083837)
...
# ══════════════════════════════════════════════════════════════════════
#  PAGE SENTENCES AND GATES — every stop routed by menu name
# ══════════════════════════════════════════════════════════════════════

def ranking_refusal() -> str:
    """The page's founding refusal, printed on every load."""
    return ("The Magic Formula is a market-wide RANKING: Greenblatt's screen orders "
            "thousands of companies by the two legs combined, and a ranking needs a "
            "universe this kit does not fetch. This page computes the two legs for one "
            "ticker, with his definitions, and shows where the figures sit against a "
            "small dated table of the names this kit reads — context, never a rank. "
            "It never combines the two positions into one score, because that combined "
            "score IS the formula's ranking operation, and running it on two dozen "
            "names would be a fake universe wearing his method's name.")


def mf_financial_stop(tk: str, fin_class: str, fin_reason: str) -> str:
    """Greenblatt's own exclusion, stated as HIS reason — the rare case
    where the author's rule and this kit's gate agree exactly."""
    desc = {"bank": "a bank", "insurer": "an insurer", "reit": "a REIT",
            "broker": "a client-asset broker"}.get(fin_class)
    if desc:
        return (f"**{tk} is {desc}.** {fin_reason} Greenblatt excludes financials from "
                "the formula himself: EBIT and a working-capital-plus-fixed-assets base "
                "mean nothing for a balance-sheet business — the rare case where the "
                "author's own rule and this kit's gate agree exactly. The Financials "
                "Checker prices banks, insurers, REITs and client-asset brokers on a "
                "filed base per share — use that page.")
    return (f"**{tk} is in a financial class this kit refuses.** {fin_reason} "
            "Greenblatt excludes financials from the formula, and no page here prices "
            "this class honestly either.")


def mf_carveout_banner(tk: str, fin_reason: str) -> str:
    """The managed-care decision of record (18 Sep 2026), on the page in
    full: the exclusion is the METHOD'S boundary, and Greenblatt's screen
    classifies managed care as health care — CI is on his own list."""
    return (f"**Managed care (SIC 6324) — the method's boundary, not the kit's.** "
            f"The kit's other pages classify {tk} as an insurer ({fin_reason.rstrip('.')}) "
            "and route it to the Financials Checker. Greenblatt's screen classifies "
            "managed care as health care and includes it, so this page follows the "
            "method's boundary and computes the legs. The excess-cash default for this "
            "class is ZERO — the cash pool likely backs policy liabilities — and the "
            "box below carries your judgement of the true excess.")


def mf_utility_stop(tk: str, sic: str, sic_desc: str) -> str:
    return (f"**{tk} is a utility (SIC {sic}, {sic_desc}).** Greenblatt excludes "
            "utilities from the formula: regulated returns on a rate base are set by "
            "commissions, not by the capital efficiency the two legs measure. The "
            "exclusion is the method's, not this kit's — detection here is SIC-range "
            f"honest only ({MF_UTILITY_SIC[0]}–{MF_UTILITY_SIC[1]}) — and the Tragic "
            "Algebra Analyzer will price it; use that page.")


def mf_foreign_route_suffix(msg: str) -> str:
...
def mf_lag_sentence(lag: int, fy_star: int, latest_filed: int) -> str:
    if lag == 0:
        return ""
    return (f"**The legs are {lag} year{'s' if lag != 1 else ''} old, and this page "
            f"says so.** EBIT last reads FY{fy_star}; the filings run to "
            f"FY{latest_filed} — the filer stopped presenting the lines this page "
            "reads. Return on capital is computed at that year's own balance sheet; "
            "the yield is that year's EBIT against today's enterprise value.")


def mf_artifact_caption(capital: float, ev: float) -> str:
    return (f"**Near-zero capital base.** Capital is {money(capital)}M against an "
            f"enterprise value of {money(ev)}M — under {MF_MIN_CAPITAL_EV:.0%} of it. "
            "The return-on-capital figure above is the artifact of a nearly empty "
            "denominator, not a measurement of anything scarce; Greenblatt's leg "
            "rewards needing little capital, and at this extreme the ratio stops "
            "carrying information.")


def mf_no_normalization_sentence() -> str:
    return ("The year table shows each year as filed — no normalization, no "
            "averaging; that is EPV's job, and its page capitalizes the cycle mean. "
            "Here cyclicality is shown, not smoothed.")


def mf_history_note() -> str:
    return ("The history above is the filed series for each line, latest ten years "
            "with a figure. \"Yield on today's EV\" is each year's EBIT against "
            "TODAY'S enterprise value — an honest cyclicality view that fabricates "
            "no historical market caps; the header row is the only market-priced "
            "column.")


def mf_reference_intro() -> str:
    return (f"**The reference table — the names this kit reads, as of "
            f"{MF_REFERENCE_AS_OF}.** Context, never a rank: these are the kit's own "
            f"{len(MF_REFERENCE_ROWS)} readable regression names, not a market. The "
            "yield column carries the capture date's prices and ages daily; return on "
            "capital moves only with new 10-Ks. Every cell recomputes from the pinned "
            "raw components — the table cannot disagree with the record it displays.")


def mf_position_sentence(tk: str, yld, roc, ref_legs: list) -> str:
    """Per-leg position counts against the pinned set — counts, never a
    combined ordinal (the ranking refusal owns why)."""
    parts = []
    if yld is not None:
        n = sum(1 for _, y, _ in ref_legs if y is not None and yld > y)
        parts.append(f"a higher earnings yield than {n} of the {len(ref_legs)} names")
    if roc is not None:
        n = sum(1 for _, _, r in ref_legs if r is not None and roc > r)
        parts.append(f"a higher return on capital than {n} of the {len(ref_legs)}")
    if not parts:
        return ""
    return (f"Against the reference set, {tk} shows " + " and ".join(parts)
            + ". Two positions, never one score.")


def mf_floor_sentence(sic: str, pct: float, floor: float, revenue: float,
                      cash_total: float) -> str:
    if sic in MF_MANAGED_CARE_SIC:
        return (f"Working-cash floor: the WHOLE cash pool ({money(cash_total)}M) — the "
                "managed-care class default, excess cash zero; the box above carries "
                "your judgement.")
    return (f"Working-cash floor: {pct:.1%} of revenue = {money(floor)}M — a stated "
            "convention, shared with the 100-Bagger Checker's ROIC waterfall; no "
            "published figure exists for the split. Cash up to the floor stays in "
            "working capital; everything above it is excess and is netted out of "
            "enterprise value.")


def mf_capital_contrast_sentence() -> str:
    return ("Greenblatt's capital base deliberately excludes goodwill, intangibles "
            "and excess cash — what the business NEEDS, not what was paid. The "
            "100-Bagger Checker's ROIC keeps goodwill and acquisition debt in, "
            "because Burry's question is what the whole dollar invested returned. "
            "Two different questions; the two figures are not disagreeing "
...
MF_BANNED = ("ranked", "score:", " top ", "buy ", " cheap",
             "expensive", "verdict")


def mf_disowned(s: str) -> bool:
    """"rank" and "score" may appear in an output sentence ONLY inside a
    disowning phrase — "never a rank", "never one score" — because the
    refusal must be allowed to name the thing it refuses. Anywhere else
    they are verdict words and the scan fails."""
    t = s.lower()
    for w in ("rank", "score"):
        i = t.find(w)
        while i != -1:
...
def mf_count_label(rung: str, period: str) -> str:
    if rung == "quarterly":
        return (f"latest filed quarterly diluted, {period} — no full-year "
                "count filed")
...
def mf_page_record(t: str) -> dict:
    rec = mf_record(t)
    _mf_attach_series(rec, t)
    return rec


def mf_page_record_fallback(t: str, why: str) -> dict:
    rec = mf_record_fallback(t, why)
    _mf_attach_series(rec, t)
    return rec


def _mf_attach_series(rec: dict, t: str) -> None:
    cmap = _ticker_map()
    facts = _facts(cmap[resolve_ticker(t, cmap)])
    rec["rev_s"] = _mf_rev(facts)
    rec["oi_s"] = _mf_flow(facts, MF_TAGS["OI"])
    rec["cae_s"] = _mf_flow(facts, MF_TAGS["CAE"])
    sk: list = []
    rec["ca_s"] = _mf_inst(facts, MF_TAGS["CA"], sk)
    rec["cl_s"] = _mf_inst(facts, MF_TAGS["CL"], sk)
    rec["std_s"] = _mf_inst(facts, BALANCE["std"], sk)
    rec["ppe_s"] = _mf_inst(facts, MF_TAGS["PPE"], sk)
    rec["cash_s"] = _mf_inst(facts, BALANCE["cash"], sk)
    rec["sti_s"] = _mf_inst(facts, BALANCE["sti"], sk)
    rec["pretax_s"] = {fy: v[2] / 1e6 for fy, v in
                       _annual(facts, *MF_PRETAX_TAGS, None, True, False, {}).items()}
    rec.setdefault("sic_desc", _sic(cmap[resolve_ticker(t, cmap)])[1])
    rec.setdefault("fin_reason",
                   financial_class(rec.get("sic", ""), facts)[1])


def mf_year_rows(rec: dict, gw: dict, today_ev, sic: str) -> list[dict]:
    """The year table, cells from the filed series. Adjusted columns only
    where the reader's window carries G and Omega for the year; capital
    only where CA, CL and PPE all read AT that year — a refused cell is a
    dash, never a zero wearing a number's clothes."""
    _, _, cells = mf_ebit_ladder(rec["oi_s"], rec["rev_s"], rec["cae_s"])
    fys = sorted(cells)[-10:]
    rows = []
    for fy in fys:
        ebit = cells[fy][0]
        rev = rec["rev_s"].get(fy)
        r = {"FY": fy, "Revenue": rev, "EBIT": ebit,
             "Margin": (ebit / rev) if rev else None,
             "Adj EBIT": None, "Capital": None, "ROC": None, "Adj ROC": None,
             "Yield on today's EV": (ebit / today_ev)
             if today_ev and today_ev > 0 else None}
        if fy in gw:
            r["Adj EBIT"] = ebit + gw[fy][0] - gw[fy][1]
        if (fy in rec["ca_s"] and fy in rec["cl_s"] and fy in rec["ppe_s"]):
            floor = mf_class_floor(sic, rev or 0.0,
                                   rec["cash_s"].get(fy, 0.0)
                                   + rec["sti_s"].get(fy, 0.0),
                                   MF_OP_CASH_PCT_DEFAULT)
            cc = rec["cash_s"].get(fy, 0.0) + rec["sti_s"].get(fy, 0.0)
            kept, _ = mf_cash_split(cc, cc, floor)
            nwc = mf_nwc(rec["ca_s"][fy], rec["cl_s"][fy],
                         rec["std_s"].get(fy, 0.0), cc, kept)
            cap = mf_capital(nwc, rec["ppe_s"][fy])
            r["Capital"] = cap
            if cap > 0:
                r["ROC"] = ebit / cap
                if r["Adj EBIT"] is not None:
                    r["Adj ROC"] = r["Adj EBIT"] / cap
        rows.append(r)
    return rows


def mf_ref_legs() -> list[tuple[str, float | None, float | None]]:
    """(ticker, yield, roc) recomputed from every pinned row — cells from
    records, the only path any reference figure takes to the screen."""
    out = []
    for r in MF_REFERENCE_ROWS:
        ev = mf_ev(r.mktcap, r.debt, r.excess)
        cap = mf_capital(r.nwc, r.ppe)
        y, roc, _, _ = mf_legs(r.ebit, ev, cap)
        out.append((r.ticker, y, roc))
    return out


def mf_ref_adj_legs(r: MFRefRow) -> tuple[float | None, float | None]:
    if r.gmw is None:
        return None, None
    ev = mf_ev(r.mktcap, r.debt, r.excess)
    cap = mf_capital(r.nwc, r.ppe)
    y, roc, _, _ = mf_legs(r.ebit + r.gmw, ev, cap)
    return y, roc


# ══════════════════════════════════════════════════════════════════════
#  SELF-TESTS — the Magic Formula suite
# ══════════════════════════════════════════════════════════════════════

def mf_self_test() -> list[tuple[str, bool, str]]:
    out = []

    # ── the shared block's arithmetic, re-proved on this carrier ─────
    _rev = {2023: 3500.0, 2024: 3600.0, 2025: 3610.0}
    out.append(("MF: ladder — D1 fresh, D1 lagged, D4 with printed arithmetic, "
                "stale, none",
    ...
    out.append(("MF: arithmetic formatter — integers plain, the HRB pair escalates "
                ...
                _a2))
    out.append(("MF: balance ceiling — fresh silent, carried named, beyond the "
    ...
    out.append(("MF: lag rule — ok at 0, note to the ceiling, refuse past it",
                mf_lag(2025, 2025) == (0, "ok") and mf_lag(2025, 2022) == (3, "note")
                and mf_lag(2025, 2021) == (4, "refuse"),
                "ceiling 3"))
    out.append(("MF: waterfall and class floors — kept capped, excess above the "
    ...
    out.append(("MF: Greenblatt capital hand-check and the AZO floor — negative "
    ...
    out.append(("MF: legs exact, each denominator guard refuses alone, negative "
                "EBIT prints",
    ...
    out.append(("MF: the signature pins — yield gap exactly (G−Ω)/EV, capital "
                "identity exactly (G−Ω)/capital",
                abs((_ya - _y) - (_g - _w) / 5900.0) < 1e-12
                and abs((_ra - _r) - (_g - _w) / 690.0) < 1e-12,
                "both gaps"))
    out.append(("MF: disagreement guard dormant below two derived candidates, "
                "names both beyond tolerance",
                mf_rung_disagreement({"D4": 413.0}) == ""
                and mf_rung_disagreement({"D3": 500.0, "D4": 413.0})
                == "D3 500 vs D4 413",
                "guard"))
    out.append(("MF: interest note — tag named as the filing signs it, the net "
    ...
    out.append(("MF: fallback count ladder — quarterly rung picks the latest "
                "quarter, half-year never counts, labels verbatim",
                _mf_fallback_diluted(_fq)
                == (141.3, "quarter ended 2026-06-30", "quarterly")
                and mf_count_label("quarterly", "quarter ended 2026-06-30")
                == ("latest filed quarterly diluted, quarter ended 2026-06-30 "
                    "— no full-year count filed")
                and mf_count_label("annual", "FY2025")
                == "latest filed diluted, FY2025",
                "rung 2"))
    out.append(("MF: artifact caption fires under 1% of EV and only there, and "
                "the sentence owns the artifact",
                mf_capital_artifact(50.0, 10000.0)
                and not mf_capital_artifact(200.0, 10000.0)
                and "nearly empty denominator" in mf_artifact_caption(50.0, 10000.0),
                "1% trigger"))

    # ── the pinned reference block: drift, spots, cells from records ─
    out.append(("MF: capture-vs-page drift check — the pinned tag lists and "
                "settings equal this page's own",
                MF_TAGS_USED == MF_TAGS
                and MF_LAG_CEILING_USED == MF_LAG_CEILING
                and MF_OP_CASH_PCT_USED == MF_OP_CASH_PCT_DEFAULT,
                "no drift"))
    _sy = MF_SPOT_YIELD
    _sr = MF_SPOT_ROC
    _row0 = next(r for r in MF_REFERENCE_ROWS if r.ticker == _sy[0])
    out.append(("MF: both spot fragments recompute with the page's own functions "
                "to the last digit",
                mf_legs(_sy[1], _sy[2], 1.0)[0] == _sy[3]
                and mf_legs(_sr[1], 1.0, _sr[2])[1] == _sr[3]
                and mf_ev(_row0.mktcap, _row0.debt, _row0.excess) == _sy[2]
                and mf_capital(_row0.nwc, _row0.ppe) == _sr[2],
                f"{_sy[0]} spots"))
    _pd = next(r for r in MF_REFERENCE_ROWS if r.ticker == "PDEX")
    _pev = mf_ev(_pd.mktcap, _pd.debt, _pd.excess)
    _pcap = mf_capital(_pd.nwc, _pd.ppe)
    _py, _pr, _, _ = mf_legs(_pd.ebit, _pev, _pcap)
    out.append(("MF: a pinned row's cells recompute — PDEX components through the "
                "waterfall land on the legs to 1e-12",
                abs(_pev - (_pd.mktcap + _pd.debt - _pd.excess)) < 1e-9
                and abs(_pcap - (max(_pd.nwc, 0.0) + _pd.ppe)) < 1e-12
                and abs(_py - _pd.ebit / _pev) < 1e-15
                and abs(_pr - _pd.ebit / _pcap) < 1e-15,
                f"PDEX {_py:.4%} / {_pr:.4%}"))
    _legs = mf_ref_legs()
    _pos = mf_position_sentence("SYN", 0.06, None, _legs)
    _hand = sum(1 for _, y, _ in _legs if y is not None and 0.06 > y)
    out.append(("MF: position counts recompute from the pinned rows and the "
                "sentence never combines them",
                len(_legs) == len(MF_REFERENCE_ROWS)
                and f"higher earnings yield than {_hand} of the {len(_legs)}" in _pos
                and "never one score" in _pos
                and "return on capital" not in _pos,
                _pos))
    out.append(("MF: the reference intro carries the as-of date and the aging "
                "honesty, and the dropped list carries BBW's reason",
                MF_REFERENCE_AS_OF in mf_reference_intro()
                and "ages daily" in mf_reference_intro()
                and any(t == "BBW" and "stopped more than 3 years" in w
                        for t, w in MF_REFERENCE_DROPPED),
                "as-of + dropped"))

    # ── stops, routes, banners ───────────────────────────────────────
    out.append(("MF: the financial stop states Greenblatt's own exclusion, routes "
                "the four priced classes, and the refused classes get the no-page "
                "sentence",
                all(_route_ok(mf_financial_stop("T", c, "r."))
                    and "Greenblatt excludes financials" in mf_financial_stop("T", c, "r.")
                    for c in ("bank", "insurer", "reit", "broker"))
                and "no page here prices" in mf_financial_stop("T", "refused",
                                                               "r.").lower(),
                ""))
    out.append(("MF: the 6324 carve-out banner states the boundary decision in "
                "full and names the zero-excess default",
                "the method's boundary" in mf_carveout_banner("CI", "r.")
                and "ZERO" in mf_carveout_banner("CI", "r.")
                and _route_ok(mf_carveout_banner("CI", "r.")),
                ""))
    out.append(("MF: the utility stop names the SIC range as the whole detection "
                "and routes to the Tragic Algebra Analyzer",
                "4900–4999" in mf_utility_stop("T", "4911", "Electric Services")
                and _route_ok(mf_utility_stop("T", "4911", "Electric Services")),
                ""))
    out.append(("MF: the foreign suffix routes to the Non-US Checker, keys on the "
                "reader's own sentence, and that sentence still exists in the span",
                _route_ok(mf_foreign_route_suffix("Foreign private issuers are "
                                                  "not supported"))
                and mf_foreign_route_suffix("something else") == ""
                and ("Foreign private issuers are " + "not supported")
                in Path(__file__).read_text(encoding="utf-8"),
                ""))
    out.append(("MF: the lag sentence names both years and the split computation; "
                "silent at zero",
                "2 years old" in mf_lag_sentence(2, 2023, 2025)
                and "FY2023" in mf_lag_sentence(2, 2023, 2025)
                and "today's enterprise value" in mf_lag_sentence(2, 2023, 2025)
                and mf_lag_sentence(0, 2025, 2025) == "",
                ""))
    out.append(("MF: the no-normalization sentence names EPV as the page whose "
                "job that is, and routes",
                "EPV" in mf_no_normalization_sentence()
                and _route_ok(mf_no_normalization_sentence()),
                ""))
    out.append(("MF: the reconciliation line brackets derived EBIT against filed "
                "pretax and states the residual; the no-pretax variant states the "
                "limit",
                "net to \\$-495M" in mf_recon_sentence(413.0, 908.0, 2026)
                and "cannot be bracketed" in mf_recon_sentence(413.0, None, 2026),
                ""))
    out.append(("MF: the floor sentence — ordinary names the shared convention, "
                "the carve-out class names the whole pool",
                "shared with the 100-Bagger Checker" in
                mf_floor_sentence("5651", 0.02, 100.0, 5000.0, 300.0)
                and "WHOLE cash pool" in
                mf_floor_sentence("6324", 0.02, 300.0, 5000.0, 300.0),
                ""))

    # ── the founding refusal and the banned-words discipline ─────────
    _fixture_texts = [ranking_refusal(), mf_reference_intro(),
                      mf_position_sentence("T", 0.06, 0.5, _legs),
                      mf_lag_sentence(2, 2023, 2025),
                      mf_artifact_caption(50.0, 10000.0),
                      mf_no_normalization_sentence(), mf_history_note(),
                      mf_capital_contrast_sentence(),
                      mf_floor_sentence("5651", 0.02, 100.0, 5000.0, 300.0)]
    out.append(("MF: the ranking refusal says why the combined score never "
                "prints, and no output sentence uses a rank or verdict word",
                "never a rank" in ranking_refusal()
                and "fake universe" in ranking_refusal()
                and all(not any(b in s.lower() for b in MF_BANNED)
                        for s in _fixture_texts[1:])
                and all(mf_disowned(s) for s in _fixture_texts[1:])
                and not mf_disowned("its combined score is 4"),
                "banned scan"))

    # ── seam and structure ───────────────────────────────────────────
    out.append(("MF: FROZEN_MENU carries EPV for the no-normalization route; the "
                "span gained nothing else",
                "EPV" in FROZEN_MENU and "Expectations" not in FROZEN_MENU
                and "OI" not in CONCEPTS,
                "additive only"))
...
# ══════════════════════════════════════════════════════════════════════
#
# NOTE ON DOLLAR SIGNS: Streamlit markdown parses $...$ as LaTeX; literal
# dollar amounts in markdown surfaces are escaped via money()/d below.
...
def _page_footer() -> None:
    ...
    with _r1:
        with st.expander("What the numbers mean", expanded=False):
            st.markdown(
                "**Earnings yield** — EBIT over enterprise value (market cap + total "
                "debt − excess cash): what the operating business earns, pre-tax, on "
                "the price of the whole enterprise.\\n\\n"
                "**Return on capital** — the same EBIT over Greenblatt's base: working "
                "capital (floored at zero) plus net fixed assets. "
                + mf_capital_contrast_sentence() + "\\n\\n"
                "**The adjusted pair** — each leg re-read with the GAAP stock-comp "
                "charge added back and the true cost Ω taken out (EBIT + G − Ω). "
                "Capital and EV are identical between legs, so the yield gap is "
                "exactly (G − Ω)/EV and the capital gap exactly (G − Ω)/capital — "
                "a formula screened on pre-SBC earnings is exactly where the hidden "
                "cost hides.\\n\\n" + mf_no_normalization_sentence()
            )
    with _r2:
        with st.expander("Verify the engine"):
            st.caption(
                "Two suites. The engine checks are the Tragic Algebra Analyzer's own "
                "— this page carries its reader and must prove it unchanged. The "
                "Magic Formula checks pin this page's arithmetic: the ladder, the "
                "waterfall, both linearity gaps, every refusal and route, and the "
                "pinned reference block's integrity against drift."
            )
            if st.button("Run checks"):
                for _label, _suite in (("Engine", self_test()),
                                       ("Magic Formula", mf_self_test())):
                    _sev, _line = test_summary(_suite)
                    getattr(st, _sev)(f"{_label}: " + _line.strip("*"))
                    for name, ok, got in _suite:
                        if not ok:
                            st.write("❌ " + f"{name} — {got}")
                st.caption("Only failures list line by line; the counts above are "
                           "the record.")
    st.caption(
        "Research aid, not financial advice. The method follows Joel Greenblatt's "
        "published writing (\"The Little Book That Beats the Market\"); the "
        "refusals, the SBC-corrected legs, the derived-EBIT arithmetic and the "
        "reference-table honesty are this project's own. Independent; not "
        "affiliated with or endorsed by Joel Greenblatt."
    )


st.set_page_config(
    page_title="Magic Formula — Investor Toolkit",
    page_icon="🪄",
    layout="centered",
...
if _tk:
    try:
        ...
        if any(_tr in str(_e) for _tr in MF_FALLBACK_TRIGGERS):
            try:
                _rec = mf_page_record_fallback(_tk, str(_e).split(". ")[0])
                _load_err = ""
            except ValueError as _e2:
                _rec, _load_err = None, str(_e2)
        else:
            _rec, _load_err = None, str(_e)

    if _rec is None:
        st.error(_load_err + mf_foreign_route_suffix(_load_err))
        _page_footer()
        st.stop()

    _msic = _rec.get("sic", "")
    if _rec.get("fin_class") in ("bank", "insurer", "reit", "broker", "refused") \
            and _msic not in MF_MANAGED_CARE_SIC:
        st.error(mf_financial_stop(_tk, _rec["fin_class"],
                                   _rec.get("fin_reason")
                                   or "The kit's gate classifies it so."))
        _page_footer()
        st.stop()
    if _msic.isdigit() and MF_UTILITY_SIC[0] <= int(_msic) <= MF_UTILITY_SIC[1]:
        st.error(mf_utility_stop(_tk, _msic, _rec.get("sic_desc", "")))
        _page_footer()
        st.stop()
    if _rec["rung"] in ("stale", "none", "refused"):
        st.error(f"**No EBIT this page can stand behind for {_tk}.** "
                 + _rec["why"] + " The tag panel below names every element "
                 "looked for.")
        _page_footer()
        st.stop()
    if _rec.get("lag_status") == "refuse":
        st.error(f"**The lines this page reads stopped {_rec['lag']} years behind "
                 f"the filings** — the last readable year is FY{_rec['fy']}, and "
                 f"beyond {MF_LAG_CEILING} years a note is not licensed to carry "
                 "the staleness. The Tragic Algebra Analyzer prices on net income "
                 "and does not need these lines; use that page.")
        _page_footer()
        st.stop()

    if _msic in MF_MANAGED_CARE_SIC:
        st.warning(mf_carveout_banner(_tk, _rec.get("fin_reason", "SIC 6324")))
        _exc_box = st.number_input(
            "Excess cash you judge deployable ($M) — class default 0",
            value=0.0, step=100.0, min_value=0.0)
        _rec["floor"] = max(0.0, _rec["cash_total"] - _exc_box)
        _rec["kept"], _rec["excess"] = mf_cash_split(
            _rec["cash_current"], _rec["cash_total"], _rec["floor"])
        _pct_used = None
    else:
        _pct_used = st.number_input(
            "Operating cash (% of revenue) — the working-cash floor",
            value=MF_OP_CASH_PCT_DEFAULT * 100, step=0.5, min_value=0.0) / 100.0
        _rec["floor"] = mf_class_floor(_msic, _rec["revenue"],
                                       _rec["cash_total"], _pct_used)
        _rec["kept"], _rec["excess"] = mf_cash_split(
            _rec["cash_current"], _rec["cash_total"], _rec["floor"])

    _rec["nwc"] = mf_nwc(_rec["ca"], _rec["cl"], _rec["std"],
                         _rec["cash_current"], _rec["kept"])
    _rec["capital"] = mf_capital(_rec["nwc"], _rec["ppe"])
    if _rec.get("price"):
        _rec["ev"] = mf_ev(_rec["mktcap"], _rec["debt"], _rec["excess"])
        _rec["yld"], _rec["roc"], _rec["y_why"], _rec["r_why"] = \
            mf_legs(_rec["ebit"], _rec["ev"], _rec["capital"])
        if _rec.get("gmw") is not None:
            _rec["yld_adj"], _rec["roc_adj"], _, _ = \
                mf_legs(_rec["ebit"] + _rec["gmw"], _rec["ev"], _rec["capital"])
    else:
        st.error("No current price could be fetched, so market cap and the yield "
                 "leg cannot be computed — try again in a minute.")
        _page_footer()
        st.stop()

    if _rec.get("lag"):
        st.warning(mf_lag_sentence(_rec["lag"], _rec["fy"], _rec["latest_filed"]))
    if _rec["rung"] == "D4":
        st.info(f"**EBIT (derived)** — no operating-income subtotal is filed; the "
                f"figure is two filed lines with the arithmetic shown: "
                f"revenue − total costs and expenses = [{_rec['arith']}] at "
                f"FY{_rec['fy']}. "
                + mf_interest_note(_rec["fy"], _rec.get("intexp_at"),
                                   _rec.get("intexp_tag", ""),
                                   _rec.get("intexp_latest", 0)))
        st.caption(mf_recon_sentence(_rec["ebit"],
    ...
    with _c1:
        st.metric(f"Earnings yield — {_lab} FY{_rec['fy']} / EV today",
                  _mf_pctcell(_rec.get("yld")))
        if _rec.get("yld") is None:
            st.error("Yield refused: " + _rec["y_why"] + ".")
        elif _rec.get("yld_adj") is not None:
            st.caption(f"SBC-corrected: {_rec['yld_adj']:.2%} — the gap "
                       f"{_rec['yld_adj'] - _rec['yld']:+.2%} is exactly "
                       f"(G − Ω)/EV, nothing else.")
    with _c2:
        st.metric(f"Return on capital — {_lab} / capital at FY{_rec['fy']}",
                  _mf_pctcell(_rec.get("roc")))
        if _rec.get("roc") is None:
            st.error("Return on capital refused: " + _rec["r_why"] + ".")
        elif _rec.get("roc_adj") is not None:
            st.caption(f"SBC-corrected: {_rec['roc_adj']:.2%} — the gap "
                       f"{_rec['roc_adj'] - _rec['roc']:+.2%} is exactly "
                       f"(G − Ω)/capital.")
    if _rec.get("roc") is not None and mf_capital_artifact(_rec["capital"],
                                                           _rec.get("ev")):
        st.warning(mf_artifact_caption(_rec["capital"], _rec["ev"]))
    st.caption(mf_floor_sentence(_msic, _pct_used or 0.0, _rec["floor"],
                                 _rec["revenue"], _rec["cash_total"]))
    ...
    if not _rec.get("fallback"):
        try:
            _yrs, _notes2, _meta2 = load(_tk)
            _gw = {y.fy: (y.G, y.omega) for y in _yrs}
            _fnote = next((n for n in _notes2
                           if ("Foreign private issuers are "
                               + "not supported") in str(n)), "")
            if _fnote:
                st.error(str(_fnote) + mf_foreign_route_suffix(str(_fnote)))
                _page_footer()
                st.stop()
        except Exception:
            _gw = {}
    _yr_rows = mf_year_rows(_rec, _gw, _rec.get("ev"), _msic)
    if _yr_rows:
        st.dataframe(pd.DataFrame([{
            "FY": r["FY"],
            "Revenue $M": None if r["Revenue"] is None else round(r["Revenue"], 1),
            "EBIT $M": round(r["EBIT"], 1),
            "Margin": _mf_pctcell(r["Margin"]),
            "Adj EBIT $M": None if r["Adj EBIT"] is None else round(r["Adj EBIT"], 1),
            "Capital $M": None if r["Capital"] is None else round(r["Capital"], 1),
            "ROC": _mf_pctcell(r["ROC"]),
            "Adj ROC": _mf_pctcell(r["Adj ROC"]),
            "Yield on today's EV": _mf_pctcell(r["Yield on today's EV"]),
        } for r in _yr_rows]), width='stretch', hide_index=True)
        st.caption(mf_history_note() + " " + mf_no_normalization_sentence())

    st.markdown(mf_reference_intro())
    _ref = mf_ref_legs()
    st.dataframe(pd.DataFrame([{
        "Ticker": r.ticker, "FY": r.fy,
        "Yield": _mf_pctcell(y), "Adj yield": _mf_pctcell(mf_ref_adj_legs(r)[0]),
        "ROC": _mf_pctcell(roc), "Adj ROC": _mf_pctcell(mf_ref_adj_legs(r)[1]),
    } for r, (_, y, roc) in zip(MF_REFERENCE_ROWS, _ref)],),
        width='stretch', hide_index=True,
        height=min(38 * len(_ref) + 40, 600))
    for _t3, _w3 in MF_REFERENCE_DROPPED:
        st.caption(f"· {_t3} — {_w3}")
    _pos = mf_position_sentence(_tk, _rec.get("yld"), _rec.get("roc"), _ref)
    if _pos:
        st.markdown(_pos)

    with st.expander("Assumptions used — paste this if a figure looks wrong"):
        st.code("\n".join([
            f"ticker {_rec['resolved']}   fy FY{_rec['fy']}   rung {_rec['rung']}"
            + (f"   [{_rec['arith']}]" if _rec["rung"] == "D4" else "")
            + (f"   lag {_rec.get('lag', 0)}y" if _rec.get("lag") else ""),
            f"price {_rec['price']}   shares {_rec['shares']:,.3f}M"
            + ("   (" + mf_count_label(_rec.get("count_rung", ""),
                                       _rec.get("count_period", "")) + ")"
               if _rec.get("fallback") else "   (reader count route)"),
            f"mktcap {_rec['mktcap']:,.1f}   debt {_rec['debt']:,.1f}   "
            f"cash pool {_rec['cash_total']:,.1f}",
            f"floor {_rec['floor']:,.2f}   kept {_rec['kept']:,.2f}   "
            f"excess {_rec['excess']:,.2f}   EV {_rec['ev']:,.1f}",
            f"CA {_rec['ca']:,.1f}   CL {_rec['cl']:,.1f}   STD {_rec['std']:,.1f}"
            f"   cash+STI {_rec['cash_current']:,.1f}",
            f"NWC {_rec['nwc']:,.2f}   PPE {_rec['ppe']:,.1f} [{_rec['ppe_tag']}]"
            f"   capital {_rec['capital']:,.2f}",
            f"EBIT {_rec['ebit']:,.2f}   G−Ω {_rec['gmw']}",
            f"yield {_mf_pctcell(_rec.get('yld'))} / adj "
            f"{_mf_pctcell(_rec.get('yld_adj'))}   ROC "
            f"{_mf_pctcell(_rec.get('roc'))} / adj "
            f"{_mf_pctcell(_rec.get('roc_adj'))}",
            f"reference as-of {MF_REFERENCE_AS_OF}",
        ]), language="text")

    with st.expander("Tag panel — every element this page read or looked for"):
        def _spanrow(k, ser):
            return {"Line": k, "Years": len(ser),
                    "Latest": max(ser) if ser else "—"}
        st.dataframe(pd.DataFrame([
            _spanrow("Revenue (reader family)", _rec["rev_s"]),
            _spanrow("Operating income [" + "+".join(MF_TAGS["OI"][0]) + "]",
                     _rec["oi_s"]),
            _spanrow("Costs and expenses [CostsAndExpenses]", _rec["cae_s"]),
            _spanrow("Interest ["
                     + (_rec.get("intexp_tag") or "none read at the headline year")
                     + "]",
                     {_rec["fy"]: 1} if _rec.get("intexp_at") is not None else {}),
            _spanrow("Current assets [AssetsCurrent]", _rec["ca_s"]),
            _spanrow("Current liabilities [LiabilitiesCurrent]", _rec["cl_s"]),
            _spanrow("Short-term debt (reader group)", _rec["std_s"]),
            _spanrow("Net PP&E [" + _rec["ppe_tag"] + "]", _rec["ppe_s"]),
            _spanrow("Cash (reader group)", _rec["cash_s"]),
            _spanrow("Short-term investments (reader group)", _rec["sti_s"]),
            _spanrow("Pretax income (reconciliation only)", _rec["pretax_s"]),
        ]), width='stretch', hide_index=True)
        if _rec.get("skips"):
            st.caption("Tag swaps: "
                       + "; ".join(f"{w} for {l}"
                                   for l, _ly, w, _wy in _rec["skips"]) + ".")
