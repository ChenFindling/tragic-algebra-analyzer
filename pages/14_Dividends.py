"""
Dividends — the filed record and its cost
=========================================
The filed dividend record for ONE ticker at a time from audited SEC
annual filings: per-share history from ONE serving element, in-window
streaks on verified pairs, endpoint growth, and the coverage pair in
dollars — cash paid against reported net income, and the same cash
against owners' earnings through the kit's measured ΔE. Nobody's
formula: the record itself, and what it costs.

THE HONESTY THE PAGE IS BUILT ON
--------------------------------
ONE per-share element serves the whole history — never stitched across
declared and paid, because a seam mid-series fabricates a raise out of
pure timing. Raises count only between fiscal years whose end dates
sit 330-400 days apart (the same window the reader demands of annual
flows) and whose split basis is verified against the headline year
from the reader's own count evidence; a year that cannot prove its
basis REFUSES from streak and growth by name, because a split halves
a filed per-share figure exactly the way a cut does. Streak labels
say "the last N readable years", the window this page reads, never
the company's full history. Coverage is asked twice in dollars for
one year — leg 2 = leg 1 / ΔE, an identity printed as arithmetic,
measured and uncapped — and refuses out loud on a loss, an unmeasured
ΔE, or a stale dollar line. A filer that never tagged a dividend gets
a clean answer, not a failure; a stale line names its last filed year
and claims neither cause. No dividend discount model, by decision:
the Expectations page answers the price-implied question. Financials
route to the Financials Checker; foreign filers to the Non-US Checker.

The reader below is the Tragic Algebra Analyzer's, per the kit's
per-page copy pattern, byte-verbatim from the post-Deploy-0 span of
25 Sep 2026 with ZERO divergence: it already carries load()'s
shares_by_fy AND shares_asfiled_by_fy meta keys, the split-basis
evidence this page's refusal rule consumes. (The span's own menu-map
comment stays at its 12-Sep vintage by decision of record; THIS
docstring is the file's map of record.)

THE MENU MAP IS FROZEN (toolkit pass, 12 Sep 2026; 9 added 13 Sep
2026; 10 added 17 Sep 2026; 11 added 18 Sep 2026; 12 added 20 Sep
2026; 13 added 21 Sep 2026; 14 added 25 Sep 2026):
...
3 is retired and is never reused; 8 is reserved. Never reuse a number.
"""
...
# ══════════════════════════════════════════════════════════════════════
#  DIVIDENDS — the filed record and its cost (this page's own code; the
#  reader above is the Tragic Algebra Analyzer's)
# ══════════════════════════════════════════════════════════════════════
#
# Everything from here down is this page's own. The span above is the
# reader of record byte-verbatim, copied from tool 1 AFTER this page's
# Deploy 0 (25 Sep 2026), so it carries load()'s shares_asfiled_by_fy
# meta key beside shares_by_fy with ZERO divergence — the second page in
# a row whose span needed no licensed exception, because the shared need
# (the split-basis evidence) went into the shared code first.
#
# Whose method: nobody's formula. This page prints the filed dividend
# record — per-share history from ONE serving element, in-window streaks
# on verified pairs, endpoint growth — and then asks the payout question
# twice in dollars for the headline year: cash paid against reported net
# income, and the same cash against owners' earnings through the kit's
# measured ΔE. Leg 2 = leg 1 / ΔE is an identity, printed as arithmetic;
# the gap between the legs is the cost stock compensation hides. There
# is no dividend discount model here by decision (the same reasoning
# that declined Lynch/PEG): a Gordon model needs an assumed growth rate,
# and the Expectations page answers the price-implied question.
#
# Registration of record: 25 Sep 2026 census — ADP, MSFT, QCOM element
# pastes (thirteen in all) and PDEX's four-way NoSuchKey zero-state.
# The serving rule, the paid ladder, the 330-400 day pair window, the
# cross-check tolerances and the split-basis refusal were all fixed on
# that evidence before this file existed.

DIV_REGISTERED_AS_OF = "2026-09-25"

# Per-share candidates: ONE tag serves the whole series, never filled
# across (the named divergence from page 5's _per_share, which fills —
# there the pair feeds one latest-year sentence, where fill is harmless;
# here a Declared/CashPaid seam mid-series would fabricate a raise or a
# cut out of pure timing). Reconcile, not fork: same elements, different
# rule, reason stated.
DIV_DPS_TAGS = ["CommonStockDividendsPerShareDeclared",
                "CommonStockDividendsPerShareCashPaid"]

# Dollar ladder: page 5's DIV list verbatim, filled across with per-year
# origin (the CFO-note pattern). QCOM is the live multi-rung filer:
# PaymentsOfDividends went stale at FY2013 when the filer switched
# elements; PaymentsOfOrdinaryDividends serves from there. A stale rung
# fills the past and is never carried forward.
DIV_PAID_TAGS = ["PaymentsOfDividendsCommonStock", "PaymentsOfDividends",
                 "PaymentsOfOrdinaryDividends"]

DIV_PAIR_MIN_DAYS, DIV_PAIR_MAX_DAYS = 330, 400   # _annual's own window
DIV_XCHECK_NOTE = 0.05    # cross-check silent below 5%
DIV_XCHECK_SPLIT = 0.40   # split-suspect territory at 40%+
DIV_BASIS_TOL = 0.01      # split-basis scale must match headline within 1%

_DIV_DISCOVERY = ["Assets", "AssetsCurrent", "LiabilitiesCurrent"]


def div_pct(x: float) -> str:
    return f"{x * 100:.1f}%"


def div_fy_span(fys) -> str:
    """FY2012-FY2013, FY2015 — compact spans for the fill note."""
    fys = sorted(fys)
    runs, s, p = [], fys[0], fys[0]
    for y in fys[1:]:
        if y == p + 1:
            p = y
        else:
            ...
            s = p = y
    runs.append((s, p))
    return ", ".join(f"FY{a}" if a == b else f"FY{a}-FY{b}"
                     for a, b in runs)


def div_gap_days(d_a: str, d_b: str) -> int:
    return abs((dt.date.fromisoformat(d_b)
                - dt.date.fromisoformat(d_a)).days)


# ── the per-share reader ─────────────────────────────────────────────


def div_per_share(facts: dict, concept: str
                  ) -> tuple[dict[int, tuple[str, str, float]],
                             dict[int, list[str]]]:
    """One per-share concept: USD/shares unit, annual forms only,
    full-year durations only (330-400 days), latest-filed-wins per
    fiscal year — page 5's _per_share discipline per single concept,
    WITHOUT fill, keeping dates for the pair filter.

    The duration filter earns its keep on day one: ADP's FY2016 10-K
    carries a 273-day DPS slide stamped 2014-07-01 to 2015-03-31 at
    1.95 (accn 0000008670-16-000053) beside the genuine FY2015 full
    year at the same value — the mis-dated fact is excluded
    structurally, before any dedup can be confused by it.

    Also returned: every distinct accepted period end per end-year (the
    straddle evidence — two genuine adjacent 52/53-week years can share
    one calendar end-year, the SFM shape page 13 met live)."""
    tax = facts.get("facts", {}).get("us-gaap", {})
    got: dict[int, tuple[str, str, str, float]] = {}
    ends: dict[int, set] = {}
    for row in (tax.get(concept, {}).get("units", {})
                .get("USD/shares", [])):
        if row.get("form") not in ANNUAL_FORMS:
            continue
        start, end = row.get("start"), row.get("end")
        if not (start and end):
            continue
        if not DIV_PAIR_MIN_DAYS <= (dt.date.fromisoformat(end)
                                     - dt.date.fromisoformat(start)).days \
                <= DIV_PAIR_MAX_DAYS:
            continue
        fy, filed = int(end[:4]), row.get("filed", "")
        ends.setdefault(fy, set()).add(end)
        if fy not in got or filed > got[fy][0]:
            got[fy] = (filed, start, end, float(row.get("val", 0.0)))
    ser = {fy: (v[1], v[2], v[3]) for fy, v in got.items()}
    dups = {fy: sorted(es) for fy, es in ends.items() if len(es) > 1}
    return ser, dups


def div_serve(facts: dict) -> tuple[dict, str, dict, str]:
    """ONE element serves the whole per-share series. Pick: the tag
    whose series reaches the latest fiscal year; tie goes to Declared
    (the rate-setting fact — "raised its dividend" is a claim about
    declaration). A CashPaid-served series carries the timing caveat
    out loud. Never filled across — see DIV_DPS_TAGS."""
    cands = []
    for i, tag in enumerate(DIV_DPS_TAGS):
        ser, dups = div_per_share(facts, tag)
        if ser:
            cands.append((max(ser), -i, tag, ser, dups))
    if not cands:
        return {}, "", {}, ""
    _, _, tag, ser, dups = max(cands, key=lambda c: (c[0], c[1]))
    note = ""
    if tag == DIV_DPS_TAGS[1]:
        note = ("Per-share amounts on this record are dividends PAID in "
                "each fiscal year, not declared — the serving element is "
                "CommonStockDividendsPerShareCashPaid, so a December "
                "declaration paid in January lands in the later year and "
                "a raise can show one year late.")
    return ser, tag, dups, note


def div_paid(facts: dict) -> tuple[dict, list, dict]:
    """The dollar line through the span's own _annual: page 5's ladder,
    fill across rungs, per-year origin. Values are made positive at the
    reader (the census pastes arrive positive; abs() guards a filer
    tagging the outflow negative, page 5's stated convention)."""
    src: list[str] = []
    origin: dict[int, str] = {}
    ser = _annual(facts, DIV_PAID_TAGS, [], src, True, False, origin)
    return ({fy: (v[0], v[1], abs(float(v[2]))) for fy, v in ser.items()},
            src, origin)


def div_paid_note(origin: dict) -> str:
    """Fires when more than one rung answered (names each element and
    its years — the CFO-note pattern) and whenever the serving element
    is a broad-scope one."""
    if not origin:
        return ""
    parts = []
    if len(set(origin.values())) > 1:
        by: dict[str, list] = {}
        for fy, tg in origin.items():
            by.setdefault(tg, []).append(fy)
        seg = "; ".join(
            f"{tg} answered {div_fy_span(f)}"
            for tg, f in sorted(by.items(), key=lambda kv: min(kv[1])))
        parts.append("The dollar line was filled across elements: " + seg
                     + " — which element answered which year, named "
                     "rather than hidden; a stale element fills the past "
                     "and is never carried forward.")
    serving = origin[max(origin)]
    if serving in DIV_PAID_TAGS[1:]:
        parts.append(f"The serving element {serving} spans common, "
                     "preferred and any noncontrolling distributions by "
                     "definition; for a single-class filer with none of "
                     "those, the caveat is vacuous but stated.")
    return " ".join(parts)


# ── pairs, basis, streaks, growth ────────────────────────────────────


def div_pair_why(fy_new: int, fy_old: int, ser: dict, dups: dict) -> str:
    """Empty string means the pair tiles. Otherwise the one refusal
    sentence: a duplicate end-year decided by its gap (straddle vs
    year-end change — page 13's Deploy-2 rule, inherited on day one),
    or an out-of-window gap between the pair's own end dates."""
    for y in (fy_new, fy_old):
        if y in dups:
            ds = sorted(dups[y])
            g = div_gap_days(ds[0], ds[-1])
            if (len(ds) == 2
                    and DIV_PAIR_MIN_DAYS <= g <= DIV_PAIR_MAX_DAYS):
                return (f"FY{y} has two annual period ends ({ds[0]} and "
                        f"{ds[1]}, {g} days apart) — adjacent fiscal "
                        "years of a 52/53-week filer straddling New "
                        "Year, which this page's calendar-year labeling "
                        "cannot tile; the pair refuses as this page's "
                        "own stated limitation, not a year-end change")
            return (f"FY{y} has {len(ds)} annual period ends ({ds[0]} to "
                    f"{ds[-1]}, {g} days apart) — a fiscal year-end "
                    "change; the years do not tile and the pair refuses "
                    "rather than bridge them")
    if fy_new not in ser or fy_old not in ser:
        return ""
    d_new, d_old = ser[fy_new][1], ser[fy_old][1]
    g = div_gap_days(d_old, d_new)
    if not DIV_PAIR_MIN_DAYS <= g <= DIV_PAIR_MAX_DAYS:
        return (f"the year-end dates {d_old} and {d_new} sit {g} days "
                f"apart, outside the {DIV_PAIR_MIN_DAYS}-"
                f"{DIV_PAIR_MAX_DAYS} day window one fiscal year can "
                "span — the years do not tile, and the pair refuses "
                "rather than bridge a fiscal year-end change")
    return ""


def div_basis(fys, meta: dict, fy_head: int) -> tuple[set, dict]:
    """v1's split answer: REFUSE, never restate. Each year must prove
    it sits on the headline year's basis: scale(fy) = shares_by_fy /
    shares_asfiled_by_fy, two engine-served dicts (Deploy 0's meta
    keys), and a year whose cumulative adjustment differs from the
    headline's by more than DIV_BASIS_TOL, or that is absent from
    either dict, refuses from streak and growth by name. Restating DPS
    is deliberately v2-on-evidence: post-split filings already restate
    the comparative years they carry, so a blind divide double-adjusts
    exactly the years nearest the split."""
    by = meta.get("shares_by_fy") or {}
    asf = meta.get("shares_asfiled_by_fy") or {}

    def _scale(fy):
        a, b = by.get(fy), asf.get(fy)
    ...
    for fy in fys:
        s = _scale(fy)
        if s0 is None or s is None:
            why[fy] = (f"FY{fy}'s filed-basis share count is not on the "
                       "reader's record, so the year's split basis "
                       f"cannot be verified against FY{fy_head}'s — the "
                       "year refuses from streak and growth rather than "
                       "guess")
        elif abs(s / s0 - 1.0) > DIV_BASIS_TOL:
            why[fy] = (f"FY{fy} sits on a different split basis than "
                       f"FY{fy_head} (cumulative adjustment {s:.2f} vs "
                       f"{s0:.2f}) — a split halves a filed per-share "
                       "figure exactly the way a cut does, so v1 refuses "
                       "the year from streak and growth rather than "
                       "restate it")
        else:
            ok.add(fy)
    return ok, why
...
def div_streaks(ser: dict, dups: dict, basis_ok: set,
                basis_why: dict) -> dict:
    """Walk backward from the latest readable year. Paid streak:
    consecutive basis-verified years with a positive amount and tiling
    dates. Raise streak: strictly higher than the prior year; a held
    year ENDS the raise streak and CONTINUES the paid streak — MSFT
    held 0.52 across FY2009-FY2010, the census's real fixture. Any
    break (hole, refused pair, refused basis) stops both walks and is
    named."""
    out = {"paid": 0, "raised": 0, "first_fy": None,
           "raise_stop": None, "notes": []}
    if not ser:
        return out
    latest, mn = max(ser), min(ser)
    if latest not in basis_ok:
        out["notes"].append(basis_why.get(
            latest, f"FY{latest} could not be basis-verified"))
        return out
    if ser[latest][2] <= 0:
        out["notes"].append(f"FY{latest} filed a zero per-share amount, "
                            "so no streak is claimed at the latest year")
        return out
    fy, raising = latest, True
    out["paid"], out["first_fy"] = 1, latest
    while True:
        prev = fy - 1
        if prev not in ser:
            if prev >= mn:
                out["notes"].append(
                    f"FY{prev} has no readable full-year per-share fact, "
                    "so the streak stops there rather than bridge the "
                    "hole")
            break
        if prev not in basis_ok:
            out["notes"].append(basis_why[prev])
            break
        why = div_pair_why(fy, prev, ser, dups)
        if why:
        ...
        out["first_fy"] = prev
        if raising:
            diff = round(ser[fy][2] - ser[prev][2], 9)
            if diff > 0:
                out["raised"] += 1
            else:
                raising = False
                out["raise_stop"] = ("held" if diff == 0 else "cut", fy)
        fy = prev
    return out


def div_cagr(v0: float, fy0: int, v1: float, fy1: int):
    """Endpoints only, k = elapsed fiscal years, both endpoints
    positive and on the verified walk. The sentence prints the
    arithmetic so the displayed endpoints reproduce the displayed
    rate."""
    k = fy1 - fy0
    if k <= 0 or v0 <= 0 or v1 <= 0:
        return None
    rate = (v1 / v0) ** (1.0 / k) - 1.0
    return {"rate": rate, "k": k, "v0": v0, "v1": v1, "fy0": fy0,
            "fy1": fy1,
            "sentence": (f"${v0:,.2f} (FY{fy0}) to ${v1:,.2f} (FY{fy1}) "
                         f"over {k} years is {rate * 100:.1f}% a year — "
                         "endpoints only, no smoothing, on the walk of "
                         "consecutive verified years.")}


# ── coverage — the page's reason to exist ────────────────────────────


def div_coverage(n_dollars, paid_head, recent, pooled, no_counts,
                 win) -> dict:
    """The DOLLAR frame, one year, one N, one ΔE. Leg 1 = cash
    dividends paid / reported net income for the headline year. Leg 2 =
    leg 1 / ΔE, an identity printed as arithmetic, with ΔE MEASURED
    over the recent pool, uncapped — coverage is a measurement, not a
    projection, so the seed cap does not apply; above 125% the
    measurement itself is unusable and leg 2 refuses (the reader's own
    ceiling). A pool above 100% prints leg 2 BELOW leg 1: the dividend
    takes fewer cents of each owners' dollar than of each reported one
    — MSFT is the census exhibit, with the cause named by the engine's
    own buyback test."""
    out = {"leg1": None, "leg2": None, "identity": "", "lines": [],
           "refusals": []}
    if paid_head is None:
        out["refusals"].append(
            "Coverage refuses: the cash-dividends dollar line does not "
            "read the headline year, so there is no numerator to set "
            "against it.")
        return out
    if n_dollars is None or n_dollars <= 0:
        out["refusals"].append(
            "Coverage refuses: reported net income for the headline year "
            "is not positive, and a payout ratio against a loss or a "
            "zero is noise, not a percentage.")
        return out
    leg1 = paid_head / n_dollars
    out["leg1"] = leg1
    out["lines"].append(f"Leg 1: dividends paid / reported net income = "
                        f"{div_pct(leg1)} — one year, one filed "
                        "numerator, one filed denominator.")
    if no_counts:
        out["refusals"].append(
            "Leg 2 refuses: the reader found no share counts for this "
            "filer, so ΔE is unmeasured and dividing by it would be a "
            "guess.")
        return out
    if recent is None:
        out["refusals"].append(
            "Leg 2 refuses: the reader's years cannot be pooled for ΔE "
            "(nothing to pool after exclusions), so owners'-earnings "
            "coverage is unmeasured.")
        return out
    if not recent.dE_defined:
        out["refusals"].append(
            "Leg 2 refuses: pooled net income is not positive over the "
            "recent window, and ΔE with a non-positive denominator "
            "flips sign — the Rivian lesson; the ratio would flatter a "
            "loss-maker, so it is not used.")
        return out
    if not 0 < recent.dE <= 1.25:
        out["refusals"].append(
            f"Leg 2 refuses: the recent pool measures ΔE at "
            f"{div_pct(recent.dE)}, outside the engine's usable range "
            "(0, 125%] — a measurement that extreme is noise, not a "
            "divisor.")
        return out
    leg2 = leg1 / recent.dE
    out["leg2"] = leg2
    ...
    if recent.dE > 1.0:
        if buybacks_shrank_count(win):
            cause = ("filed buybacks shrank the count at real cost over "
                     "the window, so owners' earnings ran above net "
                     "income")
        else:
            cause = ("the GAAP stock-comp charge ran past the measured "
                     "cost over the window, with no buyback dollars "
                     "accounting for the excess")
        out["lines"].append(
            f"ΔE measured above 100% — {cause} — so the dividend takes "
            "a smaller share of each owners' dollar than of each "
            "reported one, and leg 2 prints below leg 1. Measured, not "
            "capped: coverage is a measurement, and the seed cap "
            "belongs to projections.")
    if pooled is not None and pooled.years != recent.years:
        out["lines"].append(
            f"Full-window pool for context: ΔE {div_pct(pooled.dE)} "
            f"over {pooled.years} years."
            if pooled.dE_defined else
            "Full-window pool for context: ΔE undefined (pooled net "
            "income not positive).")
    return out


# ── the cross-check — a note, never a gate ───────────────────────────


def div_xcheck(ser_w: dict, paid: dict, by_fy: dict
               ) -> tuple[list, set]:
    """Where both lines and the reader's count read for a year: implied
    count = paid / DPS against the reader's year-end count. Three
    census witnesses fixed the thresholds pulling three ways: ADP
    prints a few percent ABOVE (year-end vs payment-weighted count
    under a steady buyback), MSFT a few percent BELOW (declared vs paid
    timing on a raiser), and QCOM's FY2019 buyback shock approaches 5%
    ...
    for fy in sorted(set(ser_w) & set(paid) & set(by_fy)):
        ...
        if dps <= 0 or not real:
            continue
        implied = paid[fy][2] / dps
        r = implied / real - 1.0
        if abs(r) < DIV_XCHECK_NOTE:
            continue
        if abs(r) >= DIV_XCHECK_SPLIT:
            ...
            if 2 <= k <= 20 and abs(ratio - k) <= 0.06 * k:
                ...
                notes.append(
                    ...
                    f"shares against {real / 1e6:,.0f}M on the reader's "
                    f"record — a near {k}:1 gap, the shape of a "
                    "split-basis mismatch between the two lines; the "
                    "year refuses from streak and growth, and the note "
                    "stays a note everywhere else.")
                continue
            notes.append(
                f"FY{fy}: paid / DPS implies {implied / 1e6:,.0f}M "
                f"shares against {real / 1e6:,.0f}M on the reader's "
                f"record ({r:+.1%}) — a large gap with no split shape; "
                "noted with its size, never resolved silently and never "
                "a gate.")
            continue
        notes.append(
            f"FY{fy}: paid / DPS implies {implied / 1e6:,.0f}M shares "
            f"against {real / 1e6:,.0f}M on the reader's record "
            f"({r:+.1%}) — year-end vs payment-weighted counts and "
            "declared-vs-paid timing move this a few percent; noted, "
            "never a gate.")
    return notes, suspects


# ── zero-state, stale, and the split between them ────────────────────


def div_family(facts: dict) -> dict:
    """Latest annual-form end-year per dividend element, or None where
    the element has no readable annual fact at all — the never-tagged
    vs stale evidence, all five elements this page reads."""
    tax = facts.get("facts", {}).get("us-gaap", {})
    last: dict[str, int | None] = {}
    for tag in DIV_DPS_TAGS + DIV_PAID_TAGS:
        best = None
        for rows in tax.get(tag, {}).get("units", {}).values():
            for row in rows:
                if row.get("form") in ANNUAL_FORMS and row.get("end"):
                    fy = int(row["end"][:4])
                    best = fy if best is None else max(best, fy)
        last[tag] = best
    return last


def div_zero_sentence(tk: str) -> str:
    return (f"**{tk} has no dividend record in its filings.** None of "
            "the five dividend elements this page reads — per-share "
            "declared, per-share paid, or dollars paid under any of the "
            "three payment elements — carries a single full-year "
            "annual-form fact for this filer. That is a clean answer, "
            "not a failure: the filings show no dividend history to "
            "report.")
...
def div_stale_sentence(tk: str, dps_last, paid_last,
    ...
    if dps_last is None and paid_last is None:
        return ""
    if (dps_last or 0) >= fy_head and (paid_last or 0) >= fy_head:
        return ""
    if (dps_last is not None and dps_last < fy_head
        ...
        return (f"**The per-share line is stale; the dollar line is "
                ...
                f"reads FY{fy_head} — the filer still pays; it stopped "
                "tagging the per-share element. The record table ends "
                "where per-share data ends, and coverage runs on the "
                "dollar line, which does not need it.")
    if (paid_last is not None and paid_last < fy_head
            and (dps_last or 0) >= fy_head):
        return (f"**The dollar line is stale; the per-share line is "
                f"not.** {tk}'s per-share record reads FY{fy_head}, but "
                "the cash-dividends dollar line last filed "
                f"FY{paid_last} — coverage refuses on the missing "
                "headline dollars, while the per-share record stands.")
    last = max(x for x in (dps_last, paid_last) if x is not None)
    return (f"**{tk}'s dividend record is stale.** The last filed "
            f"full-year dividend fact is FY{last}, behind the headline "
            f"year FY{fy_head}. A suspended dividend and a stale tag "
            "are different facts, and the filings this page reads "
            "cannot tell them apart from absence alone — so this page "
            "reports the staleness and claims neither.")


# ── stops and routes (page 13's shapes, this page's frame) ───────────


def div_financial_stop(tk: str, fin_class: str, fin_reason: str) -> str:
    desc = {"bank": "a bank", "insurer": "an insurer", "reit": "a REIT",
            "broker": "a client-asset broker"}.get(fin_class)
    if desc:
        return (f"**{tk} is {desc}.** {fin_reason} Coverage here "
                "divides by the kit's owners'-earnings machinery, which "
                "refuses financial balance sheets — and the Financials "
                "Checker already carries payout on a filed base per "
                "share for its classes, so this page neither duplicates "
                "nor contradicts it. Use that page.")
    return (f"**{tk} is in a financial class this kit refuses.** "
            f"{fin_reason} The owners'-earnings divisor is the wrong "
            "frame here, and no page in this kit prices this class "
            "honestly either.")


def div_foreign_stop(tk: str, ccy: str, n_f: int, n_usd: int) -> str:
    return (f"**{tk} tags its statements in {ccy}, not US dollars** — "
            f"{n_f} annual facts in {ccy} against {n_usd} in USD, and "
            "the few USD facts a foreign issuer tags are usually "
            "convenience translations for a year or two. The Non-US "
            "Checker prices 20-F and 40-F filers in the filing "
            "currency, with a paste-your-own-figures mode for companies "
            "EDGAR has never heard of — use that page.")


def div_foreign_route_suffix(msg: str) -> str:
    """Keyed on the reader's exact phrase; the self-test asserts the
    phrase still exists in the span above."""
    if ("Foreign private issuers are " + "not supported") not in msg:
        return ""
    return (" The Non-US Checker prices 20-F and 40-F filers in the "
            "filing currency, with a paste-your-own-figures mode for "
            "companies EDGAR has never heard of — use that page.")


def div_method_text() -> str:
    return ("The filed dividend record and what it costs. One per-share "
            "element serves the whole history (never stitched across "
            "declared and paid), raises are measured only between "
            "fiscal years whose dates tile and whose split basis is "
            "verified, and the payout question is asked twice in "
            "dollars for the headline year: cash paid against reported "
            "net income, then the same cash against owners' earnings "
            "through the kit's measured ΔE — the second is the first "
            "divided by ΔE, printed as arithmetic. It never grades a "
            "payout \"safe\", never \"sustainable\", never counts "
            "\"aristocrat\" years — those are forecasts and marketing "
            "labels; this page prints the filed record and its cost.")


# ── sentence discipline ──────────────────────────────────────────────

DIV_BANNED = (" buy ", " sell ", " safe", "sustainab", "aristocrat",
              "undervalued", "overvalued", "bargain", " cheap")


def _div_banned_hit(s: str) -> bool:
    """A banned stem is legal only inside a disowning never-phrase
    (page 13's rule: 'never a buy signal' is legal, a bare 'buy' is
    not)."""
    t = " " + s.lower() + " "
    for b in DIV_BANNED:
        i = t.find(b)
        while i != -1:
            if "never" not in t[max(0, i - 25):i]:
                return True
            i = t.find(b, i + 1)
    return False
...
def div_record(t: str) -> dict:
    """One name's full dividend record. Firing order is page 12/13's:
    financial gate → foreign pre-check → the reader (load raises its
    own sentences, inherited verbatim) → page-local reads → surfaces."""
    cmap = _ticker_map()
    resolved = resolve_ticker(t, cmap)
    if resolved is None:
        load(t)  # raises the reader's own unknown-ticker sentence
    cik = cmap[resolved]
    facts = _facts(cik)
    sic, sic_desc = _sic(cik)
    fin_class, fin_reason = financial_class(sic, facts)
    rec = {"ticker": t, "resolved": resolved, "sic": sic,
           "fin_class": fin_class, "fin_reason": fin_reason, "stop": ""}
    if fin_class != "ordinary":
        rec["stop"] = "financial"
        return rec
    _ccy = currency_facts(facts, _DIV_DISCOVERY)
    _f = {u: n for u, n in _ccy.items() if u != "USD"}
    if _f:
        _mu, _mn = max(_f.items(), key=lambda kv: kv[1])
        if _mn >= _ccy.get("USD", 0):
            rec["stop"] = "foreign"
            rec["ccy"] = (_mu, _mn, _ccy.get("USD", 0))
            return rec
    years, notes, meta = load(t)
    _fnote = next((n for n in notes
                   if ("Foreign private issuers are "
                       + "not supported") in str(n)), "")
    if _fnote:
        rec["stop"] = "foreign_note"
        rec["fnote"] = str(_fnote)
        return rec

    win = [y.fy for y in years]
    fy_head = win[-1]
    ser_full, tag, dups, timing_note = div_serve(facts)
    paid_full, paid_src, paid_origin = div_paid(facts)
    family = div_family(facts)

    # The tag panel's rows are built before any stop that renders it
    # (the page-12 check-29 lesson: a stop that promises a panel
    # renders it).
    panel = []
    for tg in DIV_DPS_TAGS:
        s_i, _ = div_per_share(facts, tg)
        panel.append({"Element": tg, "Full years": len(s_i),
                      "Latest": f"FY{max(s_i)}" if s_i else "not found",
                      "Role": ("serves the per-share record"
                               if tg == tag and s_i else
                               "read, not serving" if s_i
                               else "not found")})
    for tg in DIV_PAID_TAGS:
        s_i = _annual(facts, [tg], [], [], False, False, None)
        yrs = sorted(fy for fy, tg2 in paid_origin.items() if tg2 == tg)
        panel.append({"Element": tg, "Full years": len(s_i),
                      "Latest": f"FY{max(s_i)}" if s_i else "not found",
                      "Role": (("answers " + div_fy_span(yrs)) if yrs
                               else "read, not serving" if s_i
                               else "not found")})
    rec.update({"fy_head": fy_head, "win": win, "tag": tag,
                "timing_note": timing_note, "paid_src": paid_src,
                "paid_origin": paid_origin, "family": family,
                "panel": panel, "engine_tags": meta.get("tags", []),
                "dps_all": ser_full, "paid_all": paid_full})

    if all(v is None for v in family.values()):
        rec["stop"] = "zero"
        rec["zero"] = div_zero_sentence(resolved)
        return rec

    win_set = set(win)
    ser_w = {fy: v for fy, v in ser_full.items() if fy in win_set}
    ...
    for fy in suspects & set(ser_w):
        ...
        basis_why[fy] = (f"FY{fy} is split-basis-suspect from the "
                         "cross-check (paid / DPS lands near an integer "
                         "multiple of the reader's count), so the year "
                         "refuses from streak and growth")
    sk = div_streaks(ser_w, dups, basis_ok, basis_why)
    cagr = None
    if sk["paid"] >= 2 and sk["first_fy"] is not None:
        latest = max(ser_w)
        cagr = div_cagr(ser_w[sk["first_fy"]][2], sk["first_fy"],
                        ser_w[latest][2], latest)

    n_dollars = round(years[-1].N * 1e6)
    paid_head = paid_full[fy_head][2] if fy_head in paid_full else None
    try:
        pooled = pool(years)
    except ValueError:
        pooled = None
    try:
        recent = (pool_recent(years, 3) if len(years) >= 3
                  else pool(years))
    except ValueError:
        recent = None
    no_counts = meta.get("sh_cov", (1, 1))[0] == 0
    cov_win = [y for y in years[-3:] if not y.excluded]
    cov = div_coverage(n_dollars, paid_head, recent, pooled, no_counts,
                       cov_win)

    dps_last = max(ser_full) if ser_full else None
    paid_last = max(paid_full) if paid_full else None
    dps_absent = ""
    if not ser_full and paid_full:
        dps_absent = ("No per-share dividend element reads for this "
                      "filer; the dollar line does. Paid totals move "
                      "with the share count, so no raise streak is "
                      "claimed on them — the record below is dollars "
                      "only, and coverage runs as usual.")
    rec.update({"ser_w": ser_w, "paid_w": paid_w, "dups": dups,
                "basis_ok": basis_ok, "basis_why": basis_why,
                "xnotes": xnotes, "suspects": suspects, "sk": sk,
...
# ── pinned fixtures — shapes from the 25 Sep 2026 pastes ─────────────


def _div_row(start, end, val, form="10-K", filed="", accn=""):
    return {"start": start, "end": end, "val": val, "form": form,
            "filed": filed, "accn": accn, "fy": int(end[:4]), "fp": "FY"}


def _div_wrap(tag_rows: dict) -> dict:
    return {"facts": {"us-gaap": tag_rows}}


def _div_fx_adp() -> dict:
    """ADP's FY2016 10-K mis-dated 273-day slide beside the genuine
    FY2015 full year at the same 1.95, plus FY2016; accession numbers
    real (0000008670-16-000053)."""
    return _div_wrap({DIV_DPS_TAGS[0]: {"units": {"USD/shares": [
        _div_row("2014-07-01", "2015-03-31", 1.95, filed="2016-08-05",
                 accn="0000008670-16-000053"),
        _div_row("2014-07-01", "2015-06-30", 1.95, filed="2015-08-07"),
        _div_row("2014-07-01", "2015-06-30", 1.95, filed="2016-08-05"),
        _div_row("2015-07-01", "2016-06-30", 2.12, filed="2016-08-05"),
    ]}}})


def _div_fx_msft() -> dict:
    """The held pair (FY2009→FY2010 at 0.52) plus an 8-K-form full-year
    fact that ANNUAL_FORMS must exclude (accn 0000950170-24-132722)."""
    return _div_wrap({DIV_DPS_TAGS[0]: {"units": {"USD/shares": [
        _div_row("2008-07-01", "2009-06-30", 0.52, filed="2009-07-30"),
        _div_row("2009-07-01", "2010-06-30", 0.52, filed="2010-07-30"),
        _div_row("2010-07-01", "2011-06-30", 0.64, filed="2011-07-28"),
        _div_row("2023-07-01", "2024-06-30", 3.00, form="8-K",
                 filed="2024-07-30", accn="0000950170-24-132722"),
    ]}}})


def _div_fx_qcom_dps() -> dict:
    """QCOM's real 52/53-week dates: the 371-day pair (2017-09-24 →
    2018-09-30) beside 364-day pairs — all inside the window, no
    straddle, the first such payer on the record."""
    return _div_wrap({DIV_DPS_TAGS[0]: {"units": {"USD/shares": [
        _div_row("2015-09-28", "2016-09-25", 2.02, filed="2016-11-02"),
        _div_row("2016-09-26", "2017-09-24", 2.20, filed="2017-11-01"),
        _div_row("2017-09-25", "2018-09-30", 2.38, filed="2018-11-07"),
        _div_row("2018-10-01", "2019-09-29", 2.48, filed="2019-11-06"),
    ]}}})


def _div_fx_qcom_paid() -> dict:
    """The multi-rung dollar line: PaymentsOfDividends answers FY2012-13
    and goes stale (values from the paste: 1,583M / 2,055M);
    PaymentsOfOrdinaryDividends carries identical overlap values and
    continues — the stale rung fills the past, never forward."""
    return _div_wrap({
        DIV_PAID_TAGS[1]: {"units": {"USD": [
            _div_row("2011-09-26", "2012-09-30", 1583e6,
                     filed="2013-11-06"),
            _div_row("2012-10-01", "2013-09-29", 2055e6,
                     filed="2013-11-06"),
        ]}},
        DIV_PAID_TAGS[2]: {"units": {"USD": [
            _div_row("2011-09-26", "2012-09-30", 1583e6,
                     filed="2014-11-05"),
            _div_row("2012-10-01", "2013-09-29", 2055e6,
                     filed="2014-11-05"),
            _div_row("2013-09-30", "2014-09-28", 2586e6,
                     filed="2014-11-05"),
            _div_row("2014-09-29", "2015-09-27", 2880e6,
                     filed="2015-11-04"),
        ]}},
    })


def _div_fx_hrb() -> dict:
    """The 426-day fiscal-year-end change shape (Apr 2021 → Jun 2022,
    page 13's banked evidence) as a refusing pair."""
    return _div_wrap({DIV_DPS_TAGS[0]: {"units": {"USD/shares": [
        _div_row("2020-05-01", "2021-04-30", 1.04, filed="2021-06-15"),
        _div_row("2021-07-01", "2022-06-30", 1.08, filed="2022-08-15"),
    ]}}})


def _div_fx_straddle() -> dict:
    """The SFM shape: two genuine adjacent 52/53-week year-ends sharing
    calendar 2023, 364 days apart."""
    return _div_wrap({DIV_DPS_TAGS[0]: {"units": {"USD/shares": [
        _div_row("2022-01-03", "2023-01-01", 1.00, filed="2023-03-01"),
        _div_row("2023-01-02", "2023-12-31", 1.10, filed="2024-02-25"),
    ]}}})


def _div_fx_pooled(dE: float, n: float = 100.0, years: int = 3):
    return Pooled(dE=dE, sum_N=n, sum_OE=dE * n, sum_omega=0.0,
                  sum_G=0.0, years=years)


# ── the self-test ────────────────────────────────────────────────────


def div_self_test() -> list[tuple[str, bool, str]]:
    out: list[tuple[str, bool, str]] = []
    _src = Path(__file__).read_text(encoding="utf-8")

    # DIV 1: the span carries Deploy 0's meta keys
    _load_src = inspect.getsource(load)
    out.append(("DIV 1: load() serves shares_by_fy AND "
                "shares_asfiled_by_fy (Deploy 0, 25 Sep 2026)",
                '"shares_by_fy"' in _load_src
                and '"shares_asfiled_by_fy"' in _load_src,
                "the split-basis evidence is engine-served, never "
                "re-read page-locally"))

    # DIV 2: the reader phrases this page keys on still exist
    out.append(("DIV 2: the span still carries the foreign-issuer "
                "phrase and _annual's 330-400 day window",
                (("Foreign private issuers are " + "not supported")
                 in _src)
                and "330 <=" in inspect.getsource(_annual),
                "route suffix and duration filter both keyed on the "
                "span's own text"))

    # DIV 3: ADP's mis-dated slide is excluded structurally
    _ser3, _dup3 = div_per_share(_div_fx_adp(), DIV_DPS_TAGS[0])
    out.append(("DIV 3: ADP's 273-day mis-dated slide never enters — "
                "the genuine FY2015 full year serves, no phantom dup",
                _ser3.get(2015, ("", "", 0))[1] == "2015-06-30"
                and abs(_ser3.get(2015, ("", "", 0))[2] - 1.95) < 1e-9
                and abs(_ser3.get(2016, ("", "", 0))[2] - 2.12) < 1e-9
                and not _dup3,
                "duration filter before dedup; accn "
                "0000008670-16-000053 excluded by shape"))

    # DIV 4: latest-filed-wins on a restated value
    _fx4 = _div_wrap({DIV_DPS_TAGS[0]: {"units": {"USD/shares": [
        _div_row("2014-07-01", "2015-06-30", 1.90, filed="2015-08-07"),
        _div_row("2014-07-01", "2015-06-30", 1.95, filed="2016-08-05"),
    ]}}})
    _ser4, _ = div_per_share(_fx4, DIV_DPS_TAGS[0])
    out.append(("DIV 4: latest-filed-wins per fiscal year",
                abs(_ser4[2015][2] - 1.95) < 1e-9,
                "the restating filing outranks the original"))

    # DIV 5: annual forms only — the 8-K full-year fact is excluded
    _ser5, _ = div_per_share(_div_fx_msft(), DIV_DPS_TAGS[0])
    out.append(("DIV 5: a full-year DPS fact under form 8-K is excluded "
                "(ANNUAL_FORMS)",
                2024 not in _ser5 and 2011 in _ser5,
                "accn 0000950170-24-132722's shape; annual reports "
                "only"))

    # DIV 6: one tag serves the whole series; tie goes to Declared
    _fx6 = _div_wrap({
        DIV_DPS_TAGS[0]: {"units": {"USD/shares": [
            _div_row("2023-07-01", "2024-06-30", 3.00,
                     filed="2024-07-30"),
            _div_row("2024-07-01", "2025-06-30", 3.32,
                     filed="2025-07-30"),
        ]}},
        DIV_DPS_TAGS[1]: {"units": {"USD/shares": [
            _div_row("2022-07-01", "2023-06-30", 2.72,
                     filed="2023-07-27"),
            _div_row("2023-07-01", "2024-06-30", 2.93,
                     filed="2024-07-30"),
            _div_row("2024-07-01", "2025-06-30", 3.24,
                     filed="2025-07-30"),
        ]}}})
    _s6, _t6, _, _n6 = div_serve(_fx6)
    out.append(("DIV 6: one element serves the whole series (tie at the "
                "latest year goes to Declared; no fill across)",
                _t6 == DIV_DPS_TAGS[0] and sorted(_s6) == [2024, 2025]
                and 2023 not in _s6 and _n6 == "",
                "a Declared/CashPaid seam would fabricate a raise out "
                "of timing; the earlier CashPaid year is NOT stitched "
                "in"))

    # DIV 7: a CashPaid-served series carries the timing caveat
    _fx7 = _div_wrap({DIV_DPS_TAGS[1]: {"units": {"USD/shares": [
        _div_row("2023-07-01", "2024-06-30", 2.93, filed="2024-07-30"),
    ]}}})
    _s7, _t7, _, _n7 = div_serve(_fx7)
    out.append(("DIV 7: CashPaid serving adds the timing caveat; "
                "Declared serving does not",
                _t7 == DIV_DPS_TAGS[1] and "PAID in each" in _n7
                and "one year late" in _n7,
                "declared is the rate-setting fact; paid lags it"))

    # DIV 8: QCOM's 371-day and 364-day pairs both tile
    _s8, _d8 = div_per_share(_div_fx_qcom_dps(), DIV_DPS_TAGS[0])
    out.append(("DIV 8: a 52/53-week filer's 371-day and 364-day pairs "
                "pass the 330-400 window (QCOM's real dates)",
                div_pair_why(2018, 2017, _s8, _d8) == ""
                and div_pair_why(2019, 2018, _s8, _d8) == ""
                and div_pair_why(2017, 2016, _s8, _d8) == "",
                "2017-09-24 to 2018-09-30 is 371 days and tiles"))

    # DIV 9: the 426-day fiscal-year-end change refuses, dates named
    _s9, _d9 = div_per_share(_div_fx_hrb(), DIV_DPS_TAGS[0])
    _w9 = div_pair_why(2022, 2021, _s9, _d9)
    out.append(("DIV 9: a 426-day gap refuses with dates and gap named "
                "(the HRB year-end-change shape)",
                "426 days" in _w9 and "2021-04-30" in _w9
                and "2022-06-30" in _w9 and "refuses" in _w9,
                "refuse rather than bridge a year-end change"))

    # DIV 10: streak arithmetic on the QCOM tail
    _sk10 = div_streaks(_s8, _d8, set(_s8), {})
    out.append(("DIV 10: streak walk — QCOM tail pays 4, raises 3, "
                "first walk year FY2016",
                _sk10["paid"] == 4 and _sk10["raised"] == 3
                and _sk10["first_fy"] == 2016
                and _sk10["raise_stop"] is None,
                "every pair strict-rising back to the window edge"))

    # DIV 11: a held year ends the raise streak, continues the paid one
    _s11, _d11 = div_per_share(_div_fx_msft(), DIV_DPS_TAGS[0])
    _sk11 = div_streaks(_s11, _d11, set(_s11), {})
    out.append(("DIV 11: MSFT's held FY2010 ends the raise streak and "
                "continues the paid streak",
                _sk11["paid"] == 3 and _sk11["raised"] == 1
                and _sk11["raise_stop"] == ("held", 2010),
                "0.52 to 0.52 is held, not a cut and not a raise"))

    # DIV 12: the straddle shape refuses with the straddle sentence
    _s12, _d12 = div_per_share(_div_fx_straddle(), DIV_DPS_TAGS[0])
    _w12 = div_pair_why(2023, 2022, _s12, _d12)
    out.append(("DIV 12: a duplicate end-year 364 days apart refuses as "
                "the 52/53-week straddle, not a year-end change",
                "straddling New Year" in _w12
                and "stated limitation" in _w12
                and "year-end change" in _w12
                and _s12[2023][1] == "2023-12-31",
                "page 13's Deploy-2 rule inherited on day one; "
                "latest-filed kept the true 2023-12-31"))

    # DIV 13: the split-basis year refuses by name
    _meta13 = {"shares_by_fy": {2019: 400.0, 2020: 400.0},
               "shares_asfiled_by_fy": {2019: 100.0, 2020: 400.0}}
    _ok13, _why13 = div_basis([2019, 2020], _meta13, 2020)
    out.append(("DIV 13: a year at 4.00x the headline's split basis "
                "refuses from streak and growth by name",
                _ok13 == {2020} and "4.00" in _why13[2019]
                and "1.00" in _why13[2019]
                and "refuses" in _why13[2019],
                "a split halves a filed figure exactly the way a cut "
                "does; v1 refuses, never restates"))

    # DIV 14: a year absent from either count dict is unverifiable
    _meta14 = {"shares_by_fy": {2019: 100.0, 2020: 100.0},
               "shares_asfiled_by_fy": {2020: 100.0}}
    _ok14, _why14 = div_basis([2019, 2020], _meta14, 2020)
    out.append(("DIV 14: a year missing from the as-filed counts is "
                "unverifiable and refuses",
                _ok14 == {2020}
                and "cannot be verified" in _why14[2019],
                "no basis evidence means no streak claim"))

    # DIV 15: CAGR known answers from the registration
    _c_adp = div_cagr(2.24, 2017, 6.64, 2026)
    _c_qcom = div_cagr(2.02, 2016, 3.48, 2025)
    _c_msft = div_cagr(1.56, 2017, 3.64, 2026)
    out.append(("DIV 15: CAGR endpoints reproduce the registered "
                "answers — ADP 12.8%, QCOM 6.2%, MSFT 9.9%",
                abs(_c_adp["rate"] * 100 - 12.8) < 0.05
                and "$2.24 (FY2017) to $6.64 (FY2026) over 9" in
                _c_adp["sentence"]
                and abs(_c_qcom["rate"] * 100 - 6.2) < 0.05
                and abs(_c_msft["rate"] * 100 - 9.9) < 0.05
                and "endpoints only, no smoothing" in _c_adp["sentence"],
                "displayed endpoints reproduce the displayed rate"))

    # DIV 16: CAGR refusals
    out.append(("DIV 16: CAGR refuses on k <= 0 or a non-positive "
                "endpoint",
                div_cagr(2.0, 2020, 3.0, 2020) is None
                and div_cagr(0.0, 2016, 3.0, 2020) is None
                and div_cagr(2.0, 2016, -1.0, 2020) is None,
                "no rate is printed without two positive endpoints and "
                "elapsed years"))

    # DIV 17: the coverage identity, exact
    _cov17 = div_coverage(1000, 300.0, _div_fx_pooled(0.75), None,
                          False, [])
    out.append(("DIV 17: leg 2 = leg 1 / dE, printed as arithmetic "
                "(30.0% / 75.0% = 40.0%)",
    ...
    # DIV 18: above 100%, leg 2 prints below leg 1, cause named both
    # ways by the engine's own buyback test
    _win_buy = [SimpleNamespace(G=10.0, omega=6.0, T=50.0, dS=-5.0)]
    _win_chg = [SimpleNamespace(G=10.0, omega=2.0, T=0.0, dS=0.0)]
    _cov18a = div_coverage(1000, 300.0, _div_fx_pooled(1.10), None,
                           False, _win_buy)
    _cov18b = div_coverage(1000, 300.0, _div_fx_pooled(1.10), None,
                           False, _win_chg)
    out.append(("DIV 18: dE above 100% prints leg 2 below leg 1, with "
                "the buyback vs GAAP-charge cause split (the MSFT "
                "exhibit)",
                _cov18a["leg2"] < _cov18a["leg1"]
                and any("shrank the count" in ln
                        for ln in _cov18a["lines"])
                and any("GAAP stock-comp charge" in ln
                        for ln in _cov18b["lines"])
                and any("Measured, not capped" in ln
                        for ln in _cov18a["lines"]),
                "buybacks_shrank_count decides which sentence is true"))

    # DIV 19: every dE refusal branch, reason named
    _cov19a = div_coverage(1000, 300.0,
                           Pooled(dE=1.073, sum_N=-9078.0,
                                  sum_OE=-9743.0, sum_omega=0.0,
                                  sum_G=0.0, years=3), None, False, [])
    _cov19b = div_coverage(1000, 300.0, _div_fx_pooled(1.30), None,
                           False, [])
    _cov19c = div_coverage(1000, 300.0, _div_fx_pooled(0.9), None,
                           True, [])
    _cov19d = div_coverage(1000, 300.0, None, None, False, [])
    out.append(("DIV 19: leg 2 refuses on a negative denominator "
                "(Rivian), above 125%, no counts, and an unpoolable "
                "window — each named",
                any("Rivian" in r for r in _cov19a["refusals"])
                and _cov19a["leg2"] is None
                and any("125%]" in r for r in _cov19b["refusals"])
                and any("no share counts" in r
                        for r in _cov19c["refusals"])
                and any("cannot be pooled" in r
                        for r in _cov19d["refusals"])
                and _cov19a["leg1"] is not None,
                "leg 1 still prints; only the owners'-earnings leg "
                "refuses"))

    # DIV 20: the whole pair refuses on a loss or a missing numerator
    _cov20a = div_coverage(-5, 300.0, _div_fx_pooled(0.9), None,
                           False, [])
    _cov20b = div_coverage(1000, None, _div_fx_pooled(0.9), None,
                           False, [])
    out.append(("DIV 20: non-positive net income or a paid line missing "
                "the headline year refuses coverage cleanly",
                any("not positive" in r for r in _cov20a["refusals"])
                and _cov20a["leg1"] is None
                and any("does not read the headline year" in r
                        for r in _cov20b["refusals"]),
                "no percentage of a loss; no numerator, no ratio"))

    # DIV 21: cross-check thresholds — three witnesses, three ways
    _serX = {2020: ("2019-07-01", "2020-06-30", 2.00)}
    _mk = lambda shares: {2020: ("2019-07-01", "2020-06-30",
                                 2.00 * shares)}
    _n_silent, _s_silent = div_xcheck(_serX, _mk(412e6), {2020: 400e6})
    _n_qcom, _s_qcom = div_xcheck(
        {2019: ("2018-10-01", "2019-09-29", 2.48)},
        {2019: ("", "", 2968e6)}, {2019: 1145e6})
    _n_note, _s_note = div_xcheck(_serX, _mk(428e6), {2020: 400e6})
    _n_split, _s_split = div_xcheck(_serX, _mk(1648e6), {2020: 400e6})
    out.append(("DIV 21: cross-check — 3% silent, QCOM's FY2019 4.5% "
                "silent from above, 7% notes causes, near-4:1 refuses "
                "the year",
                not _n_silent and not _n_qcom
                and len(_n_note) == 1 and not _s_note
                and "never a gate" in _n_note[0]
                and _s_split == {2020}
                and "near 4:1" in _n_split[0]
                and "split-basis" in _n_split[0],
                "silent under 5%; named at 5%; split-suspect at 40% "
                "with an integer shape"))

    # DIV 22: the cross-check is never a gate — the suspect year
    # refuses from the streak walk, everything else stands
    _ser22 = {2019: ("2018-07-01", "2019-06-30", 2.00),
              2020: ("2019-07-01", "2020-06-30", 2.10)}
    _ok22 = {2020}
    _why22 = {2019: "FY2019 is split-basis-suspect from the cross-check "
                    "(paid / DPS lands near an integer multiple of the "
                    "reader's count), so the year refuses from streak "
                    "and growth"}
    _sk22 = div_streaks(_ser22, {}, _ok22, _why22)
    out.append(("DIV 22: a split-suspect year stops the walk with its "
                "reason; the latest year still stands",
                _sk22["paid"] == 1 and _sk22["raised"] == 0
                and any("split-basis-suspect" in n
                        for n in _sk22["notes"]),
                "note-driven refusal reaches exactly the streak, "
                "nothing else"))

    # DIV 23: the zero-state is a clean answer
    _z23 = div_zero_sentence("PDEX")
    _fam23 = div_family({"facts": {"us-gaap": {}}})
    out.append(("DIV 23: five-element absence prints the clean-answer "
                "sentence (the PDEX registration)",
                all(v is None for v in _fam23.values())
                and len(_fam23) == 5
                and "no dividend record in its filings" in _z23
                and "clean answer, not a failure" in _z23,
                "absence of the elements is evidence of absence of the "
                "thing"))
    ...
    _p25, _src25, _org25 = div_paid(_div_fx_qcom_paid())
    _note25 = div_paid_note(_org25)
    out.append(("DIV 25: QCOM's dollar line fills across rungs — "
                "PaymentsOfDividends answers FY2012-FY2013, "
                "PaymentsOfOrdinaryDividends the rest; both named, "
                "scope caveat stated",
                _org25[2012] == DIV_PAID_TAGS[1]
                and _org25[2013] == DIV_PAID_TAGS[1]
                and _org25[2014] == DIV_PAID_TAGS[2]
                and _org25[2015] == DIV_PAID_TAGS[2]
                and abs(_p25[2013][2] - 2055e6) < 1
                and "FY2012-FY2013" in _note25
                and "FY2014-FY2015" in _note25
                and "vacuous but stated" in _note25
                and "never carried forward" in _note25,
                "a stale element fills the past; the broad element's "
                "scope is named"))

    # DIV 26: sentence discipline + the registered pin
    _sent26 = [div_method_text(), _z23, _st_both, _st_dps, _st_paid,
               _w9, _w12, _why13[2019], _why14[2019],
               div_financial_stop("KNSL", "insurer",
                                  "The kit's gate classifies it so."),
               div_foreign_stop("EVD", "EUR", 900, 12),
               _note25] + _cov18a["lines"] + _cov18b["lines"] \
        + _cov19a["refusals"] + _cov20a["refusals"] + _n_note \
        + _n_split + _cov17["lines"]
    _pin26 = (DIV_REGISTERED_AS_OF == "2026-09-25"
              and DIV_DPS_TAGS == ["CommonStockDividendsPerShareDeclared",
                                   "CommonStockDividendsPerShareCashPaid"]
              and DIV_PAID_TAGS == ["PaymentsOfDividendsCommonStock",
                                    "PaymentsOfDividends",
                                    "PaymentsOfOrdinaryDividends"]
              and (DIV_PAIR_MIN_DAYS, DIV_PAIR_MAX_DAYS) == (330, 400)
              and abs(DIV_XCHECK_NOTE - 0.05) < 1e-12
              and abs(DIV_XCHECK_SPLIT - 0.40) < 1e-12
              and abs(DIV_BASIS_TOL - 0.01) < 1e-12)
    out.append(("DIV 26: no banned word outside a disowning "
                "never-phrase on any output sentence, and the 25 Sep "
                "2026 registration pin holds",
                not any(_div_banned_hit(s) for s in _sent26)
                and _pin26,
...
st.set_page_config(
    page_title="Dividends — Investor Toolkit",
    page_icon="🪙",
    layout="centered",
...
st.title("🪙 Dividends")
st.caption("The filed dividend record — per-share history, in-window "
           "streaks, and the coverage pair in dollars — for one ticker "
           "at a time")
st.info(div_method_text())


def _div_tag_panel(rec: dict) -> None:
    """Every element this page read or looked for — rendered on the
    success path and on the zero-state stop (a stop that promises a
    panel renders it)."""
    with st.expander("Tag panel — every element this page read or "
                     "looked for"):
        st.dataframe(pd.DataFrame(rec.get("panel", [])),
                     width='stretch', hide_index=True)
        st.caption("Per-share lines are full-year USD/shares facts from "
                   "annual forms only; dollar lines are the span's own "
                   "fiscal-year reads. One per-share element serves the "
                   "whole record; the dollar ladder fills across with "
                   "each year's origin named. The reader's full panel "
                   "is below where a run reached it.")
        if rec.get("engine_tags"):
            st.dataframe(pd.DataFrame(rec["engine_tags"]),
                         width='stretch', hide_index=True)
...
def _page_footer() -> None:
    ...
    with _r1:
        with st.expander("What this page reads", expanded=False):
            st.markdown(
                "**The record** — dividends per share for each readable "
                "year of the engine window, from ONE serving element "
                "(declared preferred, paid as fallback with its timing "
                "named). Raises count only between fiscal years whose "
                "end dates sit 330-400 days apart and whose split basis "
                "is verified against the headline year; a year that "
                "cannot prove its basis refuses from streak and growth "
                "by name.\n\n"
                "**Coverage** — for the headline year, in dollars: cash "
                "dividends paid over reported net income, then the same "
                "cash over owners' earnings via the kit's measured "
                "recent ΔE. Leg 2 = leg 1 / ΔE, an identity printed as "
                "arithmetic; the gap between the legs is the cost stock "
                "compensation hides.\n\n"
                "**What a refusal means** — an input the page needs "
                "does not read from the filings, or cannot be verified. "
                "A refused year or leg is not a failed one, and every "
                "refusal names its reason."
            )
    with _r2:
        with st.expander("Verify the engine"):
            st.caption(
                "Two suites. The engine checks are the Tragic Algebra "
                "Analyzer's own — this page carries its reader and must "
                "prove it unchanged. The Dividends checks pin this "
                "page's arithmetic: the one-element serving rule, the "
                "330-400 day pair window on the live QCOM and HRB "
                "shapes, the split-basis refusal, the coverage identity "
                "with every refusal branch, the cross-check thresholds, "
                "and the ADP/MSFT/QCOM/PDEX figures the 25 Sep 2026 "
...
with st.form("div_form"):
    _tk = st.text_input("Stock ticker",
                        placeholder="ADP · MSFT · QCOM — press Enter"
                        ).strip().upper()
    _div_go = st.form_submit_button("Read the record", type="primary")

if _div_go and not _tk:
    st.warning("Enter a ticker first.")
if _div_go and _tk:
    try:
        _rec = div_record(_tk)
    except ValueError as _e:
        st.error(str(_e) + div_foreign_route_suffix(str(_e)))
        _page_footer()
        st.stop()

    if _rec["stop"] == "financial":
        st.error(div_financial_stop(_tk, _rec["fin_class"],
                                    _rec.get("fin_reason")
                                    or "The kit's gate classifies it "
                                    "so."))
        _page_footer()
        st.stop()
    if _rec["stop"] == "foreign":
        _cu, _cn, _cus = _rec["ccy"]
        st.error(div_foreign_stop(_tk, _cu, _cn, _cus))
        _page_footer()
        st.stop()
    if _rec["stop"] == "foreign_note":
        st.error(_rec["fnote"] + div_foreign_route_suffix(_rec["fnote"]))
        _page_footer()
        st.stop()
    if _rec["stop"] == "zero":
        st.info(_rec["zero"])
    ...
    _F = _rec["fy_head"]
    _serW, _paidW = _rec["ser_w"], _rec["paid_w"]
    _latest_dps = (_serW[max(_serW)][2] if _serW else None)
    _c1, _c2, _c3 = st.columns(3)
    with _c1:
        st.metric("Headline year", f"FY{_F}")
    with _c2:
        st.metric("DPS, latest readable",
                  f"${_latest_dps:,.2f}" if _latest_dps else "n/a")
    with _c3:
        st.metric("Paid, headline year",
                  f"${_rec['paid_head'] / 1e6:,.1f}M"
                  if _rec["paid_head"] is not None else "n/a")

    if _rec["stale"]:
        st.warning(_rec["stale"])
    if _rec["dps_absent"]:
        st.caption(_rec["dps_absent"])
    if _rec["timing_note"]:
        st.caption(_rec["timing_note"])

    # the record table — window years only
    _rows = []
    _basis_ok = _rec["basis_ok"]
    for _fy in sorted(set(_serW) | set(_paidW)):
        _dps = _serW.get(_fy)
        _chg = "—"
        if _dps is not None:
            _prev = _serW.get(_fy - 1)
            if _fy not in _basis_ok:
                _chg = "basis refused"
            elif _prev is None:
                _chg = "—"
            elif (_fy - 1) not in _basis_ok:
                _chg = "prior year refused"
            elif div_pair_why(_fy, _fy - 1, _serW, _rec["dups"]):
                _chg = "pair refused"
            else:
                _d = round(_dps[2] - _prev[2], 9)
                _chg = ("held" if _d == 0 else
                        f"+${_d:,.2f}" if _d > 0 else f"-${-_d:,.2f}")
        _pd_v = _paidW.get(_fy)
        _rows.append({
            "FY": f"FY{_fy}",
            "Year end": _dps[1] if _dps else (_pd_v[1] if _pd_v else ""),
            "DPS ($)": f"{_dps[2]:,.2f}" if _dps else "—",
            "Change": _chg,
            "Paid ($M)": (f"{_pd_v[2] / 1e6:,.1f}" if _pd_v else "—"),
        })
    if _rows:
        st.dataframe(pd.DataFrame(_rows), width='stretch',
                     hide_index=True)
        st.caption("Window years only — the years the reader loads and "
                   "can basis-verify; not the company's full history. "
                   "Filed figures include any special dividends; a "
                   "special shows as a one-year jump, and no detection "
                   "heuristics are applied.")

    _sk = _rec["sk"]
    if _sk["paid"] > 0:
        st.markdown(f"**Paid in each of the last {_sk['paid']} readable "
                    f"years** (FY{_sk['first_fy']} to FY{max(_serW)}).")
    if _sk["raised"] > 0:
        _r0 = max(_serW) - _sk["raised"] + 1
        st.markdown(f"**Raised in each of the last {_sk['raised']} "
                    f"readable years (FY{_r0} to FY{max(_serW)})** — "
                    "the window this page reads, not the company's full "
                    "history.")
    if _sk["raise_stop"]:
        _kind, _kfy = _sk["raise_stop"]
        st.caption(f"The raise streak ends at FY{_kfy}: the dividend "
                   f"was {_kind} there; the paid streak continues "
                   "through it.")
    for _n in _sk["notes"]:
        st.caption("• " + _n)
    ...
    st.markdown("#### Coverage — the headline year, in dollars")
    _cov = _rec["cov"]
    if _cov["leg1"] is not None:
        _cc1, _cc2 = st.columns(2)
        with _cc1:
            st.metric("Of reported net income", div_pct(_cov["leg1"]))
        with _cc2:
            st.metric("Of owners' earnings (at ΔE)",
                      div_pct(_cov["leg2"])
                      if _cov["leg2"] is not None else "refused")
    if _cov["identity"]:
        st.code(_cov["identity"], language="text")
    for _ln in _cov["lines"]:
        st.caption(_ln)
    for _rf in _cov["refusals"]:
        st.warning(_rf)

    for _n in _rec["xnotes"]:
        st.caption("• " + _n)
    if _rec["paid_note"]:
        st.caption(_rec["paid_note"])

    with st.expander("Assumptions used — paste this if a figure looks "
                     "wrong"):
        st.code("\n".join([
            f"ticker {_rec['resolved']}   headline FY{_F}   serving "
            f"element {_rec['tag'] or 'none'}",
            f"window FY{_rec['win'][0]}-FY{_rec['win'][-1]}   paid "
            f"streak {_sk['paid']}   raise streak {_sk['raised']}",
            f"pair window {DIV_PAIR_MIN_DAYS}-{DIV_PAIR_MAX_DAYS} days   "
            f"basis tol {DIV_BASIS_TOL:.0%}   xcheck note "
            f"{DIV_XCHECK_NOTE:.0%} / split {DIV_XCHECK_SPLIT:.0%}",
            "coverage: dollars; leg2 = leg1 / dE on the recent pool, "
            "measured, uncapped, refused above 125%",
            f"registration as-of {DIV_REGISTERED_AS_OF}",
        ]), language="text")
