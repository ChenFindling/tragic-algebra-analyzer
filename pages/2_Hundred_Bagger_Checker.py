...
def load(ticker: str, n_years: int = 10):
    ...
    if fin_class in ("bank", "insurer", "reit", "refused"):
        notes.append(
            f"{sic_desc or 'Financial company'} (SIC {sic}). {fin_reason} Leverage is the "
            "product for banks, insurers and REITs rather than a financing decision, so equity "
            "plus borrowings does not describe capital at work and cash is not free. ROIC is "
            "not shown; the Financials Checker page prices this class.")
...
if years and ticker and st.session_state.get("hb_tk") == ticker:
    ...
    # Up-C stop (queue G, 9 Sep 2026): the XBRL route summed real per-class
    # counts, so the measurements above are genuine — and Mayer's per-share
    # arithmetic still cannot run: the net income read is the parent's slice
    # while the summed count spans everything (Carvana roughly a third
    # outside the parent, Ryan Specialty over half). Lifts with HANDOVER
    # §5.5 G's NCI fix, not before. Guarded on shares > 0 like tool 1's.
    if any(e.up_c for e in XBRL_REGISTRY.get(tk, ())) and shares > 0:
        st.error(up_c_sentence(tk, shares))
        with st.expander("Notes and detail — the measurements are real", expanded=True):
            for kind_, msg in alerts:
                getattr(st, kind_)(msg)
        _page_footer()
        st.stop()
