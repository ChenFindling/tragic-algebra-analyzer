...
if years and ticker and st.session_state.get("hb_tk") == ticker:
    ...
    # verified consolidated income → the basis banner and Mayer's per-share
    # arithmetic on one basis; unverified → the surviving stop. Guarded on
    # shares > 0 like tool 1's.
    _upcb = pre.get("up_c_basis")
    if any(e.up_c for e in XBRL_REGISTRY.get(tk, ())) and shares > 0:
        if _upcb and _upcb.get("ok"):
            st.info(up_c_basis_banner(tk, shares, _upcb))
        else:
