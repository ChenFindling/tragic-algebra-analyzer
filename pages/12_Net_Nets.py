"""
Net-Nets — Benjamin Graham's NCAV test
======================================
Net current asset value: what the market pays against current assets
alone, with EVERY liability and every tagged claim ahead of the common
deducted — the classic deep-value floor, computed honestly for ONE
ticker at a time from audited SEC annual filings.

THE HONESTY THE PAGE IS BUILT ON
--------------------------------
A true Graham net-net is nearly extinct among filers large enough to
file readable XBRL. Most names will print NCAV far below the price,
many negative, and the page says plainly that this is the NORMAL
answer, not a failure. Graham's two-thirds buying threshold is stated
as HIS criterion wherever it prints — never a verdict. NCAV is a
balance-sheet floor, not a going-concern value, and a real wind-down
realizes assets below book, so the true floor is lower than the
printed one, never higher.

Total liabilities is read as filed; where a filer tags only the
classified halves at the same balance date, their sum is derived with
the addition printed as arithmetic on the page. No Assets-minus-equity
rung exists, by decision of record. Financials refuse with NCAV's own
reason and route to the Financials Checker; foreign filers route to
the Non-US Checker; an unclassified balance sheet refuses naming it.
Annual balance sheets only. No income statement is read anywhere.

The reader below is the Tragic Algebra Analyzer's, per the kit's
per-page copy pattern, byte-verbatim except ONE licensed divergence:
the _annual docstring carries FLEET-HANDOVER §1's cross-tag-fill
paragraph verbatim (decision of record, 20 Sep 2026) — the first
reader copy made after that decision, deliberately ahead of the fleet
until the queued docstring ride lands in the other carriers, so this
file's _annual hash intentionally differs from the fleet's until then.
This page adds no concept lines — every balance element it needs is
read page-locally, keyed by exact year-end date (see the page-local
block for why the calendar-year keying the span's _instant uses is not
safe for this page's lines). Reader fixes land in every carrier file,
this one now included.

THE MENU MAP OF RECORD FOR THIS FILE (20 Sep 2026)
--------------------------------------------------
    Home (entrypoint)
    ...
    6   NonUS Checker                11  Magic Formula
                                     12  Net-Nets (this file)
...
# ══════════════════════════════════════════════════════════════════════
#  NET-NETS — Graham's NCAV test (this page's own code; the reader above
#  is the Tragic Algebra Analyzer's)
# ══════════════════════════════════════════════════════════════════════
#
# Everything from here down is this page's own. The span above is
# byte-verbatim to the reader of record except ONE licensed divergence:
# the _annual docstring carries FLEET-HANDOVER §1's cross-tag-fill
# paragraph (decision of record, 20 Sep 2026) — the first reader copy
# made after that decision, deliberately ahead of the fleet until the
# queued docstring ride lands in the other carriers. This page adds NO
# concept lines to the reader: every balance element it needs is read
# page-locally, by EXACT year-end date, through nn_by_date below.
#
# WHY EXACT DATES AND NOT THE SPAN'S _instant (decision on paste
# evidence, 20 Sep 2026): _instant keys instants by calendar year of
# the end date. PDEX tags Liabilities at 2019-07-01/02 — the ASC 842
# lease-adoption instant, the first day of FY2020 — in five filings,
# and the newest of them (the FY2021 10-K, filed 2021-09-09) outranks
# the genuine 2019-06-30 fact's newest vintage under calendar-year
# keying: fy 2019 would serve 3,300 instead of 8,512. A page whose one
# claim is a coherent balance sheet keys every instant by its exact
# date, and a fiscal year-end is discovered as the LATEST date an
# annual filing tags — each 10-K's own balance-sheet date — so
# adoption-date and re-tagged quarterly facts are excluded
# structurally, never by luck of filing order.
#
# Whose method: Benjamin Graham (Security Analysis; The Intelligent
# Investor) — NCAV = current assets − total liabilities − preferred
# claims, per share, against the price, with his two-thirds buying
# threshold stated as HIS criterion. What is ours: every refusal and
# route, the normal-answer honesty (a true net-net is nearly extinct
# among readable filers, and the page says so), the liquidation-frame
# caveat, the derived-liabilities arithmetic printed where only the
# classified halves are filed, and the deduction of tagged minority
# interest and temporary equity, labelled as the kit's own addition.

NN_HISTORY_CEILING = 10   # annual balance dates shown, newest first
NN_STALE_CEILING = 3      # years the headline may trail the latest date
NN_XCHECK_TOL = 2.0       # dollars: filed total vs current+noncurrent

NN_TAGS = {
    # The current-asset subtotal. Absent in EVERY annual filing means an
    # unclassified balance sheet, and the page refuses naming it.
    "CA": ["AssetsCurrent"],
    # The liabilities ladder: the filed total wins; only where no total
    # is tagged at that exact date do the two filed halves derive it,
    # with the addition printed. NO Assets-minus-equity rung exists, by
    # decision of record (20 Sep 2026): the equity element choice
    # changes the answer for every NCI filer, temporary equity sits
    # between liabilities and equity in the tagging, and a derived L
    # that swallowed redeemable preferred would double-deduct it below.
    "L1": ["Liabilities"],
    "L2": ["LiabilitiesCurrent", "LiabilitiesNoncurrent"],
    # Claims ranking ahead of the common, each deducted where tagged at
    # the year-end date, zero with a note where no element is tagged —
    # an untagged preferred element is evidence of no preferred, which
    # a missing liabilities total is not.
    "PREF": ["PreferredStockValue"],
    # One, never both: the specific redeemable-preferred element
    # outranks the general temporary-equity total, so a redeemable
    # preferred tagged under both can never deduct twice.
    "MEZZ": ["RedeemablePreferredStockCarryingAmount",
             "TemporaryEquityCarryingAmount"],
    "NCI": ["MinorityInterest"],
}

# The registration of record: the tag lists the 20 Sep 2026 paste
# evidence and the known-answer bands were committed against. The
# self-test pins the page's own NN_TAGS to these, so the page cannot
# drift from what the bands were registered on.
NN_REGISTERED_AS_OF = "2026-09-20"
NN_TAGS_REGISTERED = {
    "CA": ["AssetsCurrent"],
    "L1": ["Liabilities"],
    "L2": ["LiabilitiesCurrent", "LiabilitiesNoncurrent"],
...
def _nn_rows(facts: dict, concept: str) -> list[dict]:
...
def nn_by_date(facts: dict, concept: str) -> dict[str, float]:
    ...
    for row in _nn_rows(facts, concept):
...
def nn_yearend_dates(facts: dict, concepts: list[str]) -> list[str]:
    ...
    instants (PDEX's 2019-07-01/02 ASC 842 facts) and re-tagged
    quarterly comparatives are never the latest date of their filing
    and are excluded structurally."""
...
# ── the liabilities ladder and the deduction slots ───────────────────

def nn_arith(a: float, b: float) -> str:
    """a + b printed at the coarsest precision where the DISPLAYED
    terms reproduce the DISPLAYED result exactly — arithmetic whose
    whole point is that the reader can check it by hand must check by
    hand (the page-11 formatter's discipline, addition form)."""
    for nd in range(0, 7):
        fa, fb, fs = f"{a:,.{nd}f}", f"{b:,.{nd}f}", f"{a + b:,.{nd}f}"
...
def nn_liab_at(date: str, l1: dict, lc: dict, ln: dict
               ) -> tuple[float | None, str, str, str]:
    """(value, rung, arithmetic, why) — total liabilities at one exact
    date. The filed total wins (L1); where a filer files the total AND
    both halves at this date they must agree within $2 or the year
    refuses naming both figures, never picking; where only the halves
    are filed at this same date, their sum is derived with the addition
    printed (L2); anything less refuses naming the missing lines."""
    t, c, n = l1.get(date), lc.get(date), ln.get(date)
    if t is not None:
        if c is not None and n is not None and abs((c + n) - t) > NN_XCHECK_TOL:
            return (None, "xfail", "",
                    f"the filed total {t:,.0f} disagrees with current + "
                    f"noncurrent {c:,.0f} + {n:,.0f} = {c + n:,.0f} beyond "
                    "the $2 cross-check — the year refuses rather than pick")
        return t, "L1", "", ""
    if c is not None and n is not None:
        return c + n, "L2", nn_arith(c, n), ""
    missing = [name for name, v in (("Liabilities", t),
                                    ("LiabilitiesCurrent", c),
                                    ("LiabilitiesNoncurrent", n)) if v is None]
    return (None, "none", "",
            "no readable liabilities total: "
            + ", ".join(missing) + " not tagged at this date")


def nn_mezz_at(date: str, rp: dict, te: dict) -> tuple[float, str, str]:
    """(value, tag, note) — the mezzanine slot at one date. One, never
    both: the specific redeemable-preferred element outranks the
    general temporary-equity total, and a passed-over general element
    is named so a double-tag can never deduct twice silently."""
    if date in rp:
        note = ""
        if date in te:
            note = (f"TemporaryEquityCarryingAmount also tagged "
                    f"({te[date]:,.0f}) — passed over: one slot, never a sum")
        return rp[date], "RedeemablePreferredStockCarryingAmount", note
    if date in te:
        return te[date], "TemporaryEquityCarryingAmount", ""
    return 0.0, "", ""


def nn_maps(facts: dict) -> dict:
    return {
        "ca": nn_by_date(facts, NN_TAGS["CA"][0]),
        "l1": nn_by_date(facts, NN_TAGS["L1"][0]),
        "lc": nn_by_date(facts, NN_TAGS["L2"][0]),
        "ln": nn_by_date(facts, NN_TAGS["L2"][1]),
        "pref": nn_by_date(facts, NN_TAGS["PREF"][0]),
        "rp": nn_by_date(facts, NN_TAGS["MEZZ"][0]),
        "te": nn_by_date(facts, NN_TAGS["MEZZ"][1]),
        "nci": nn_by_date(facts, NN_TAGS["NCI"][0]),
        "dates": nn_yearend_dates(facts, _NN_DISCOVERY),
    }


def nn_rows_from_maps(m: dict) -> list[dict]:
    """One row per fiscal year-end date, newest first, capped at the
    history ceiling. A missing required line dashes the year naming it;
    nothing fills, nothing carries across dates. Values are raw
    dollars as filed."""
    rows = []
    for date in m["dates"][:NN_HISTORY_CEILING]:
        liab, rung, arith, lwhy = nn_liab_at(date, m["l1"], m["lc"], m["ln"])
        mezz, mezz_tag, mezz_note = nn_mezz_at(date, m["rp"], m["te"])
        row = {"date": date, "fy": int(date[:4]), "ca": m["ca"].get(date),
               "l": liab, "l_rung": rung, "l_arith": arith,
               "pref": m["pref"].get(date, 0.0),
               "mezz": mezz, "mezz_tag": mezz_tag, "mezz_note": mezz_note,
               "nci": m["nci"].get(date, 0.0), "ncav": None, "why": ""}
        whys = []
        if row["ca"] is None:
            whys.append("AssetsCurrent not tagged at this date")
        if liab is None:
            whys.append(lwhy)
        if whys:
            row["why"] = "; ".join(whys)
        else:
            row["ncav"] = (row["ca"] - liab - row["pref"]
                           - row["mezz"] - row["nci"])
        rows.append(row)
    return rows


def nn_headline(rows: list[dict]) -> tuple[dict | None, str]:
    """(headline row, refusal why). The latest COMPLETE year is the
    page; a dashed latest year falls back to the newest complete one;
    a headline more than the ceiling behind the latest filed balance
    date refuses out loud — a dash is not licensed to carry arbitrary
    staleness."""
    if not rows:
...
def nn_scan(facts: dict) -> dict:
    """The page's structural read, pure and testable: maps, rows,
    headline, and the two structural stops (unclassified; no usable
    headline). Gates and the count route live in nn_record."""
    m = nn_maps(facts)
    out = {"stop": "", "why": "", "maps": m, "rows": [], "head": None}
    if not m["ca"]:
        out["stop"] = "unclassified"
        out["l_years"] = len(m["l1"])
        out["l_latest"] = max((int(d[:4]) for d in m["l1"]), default=0)
        return out
    out["rows"] = nn_rows_from_maps(m)
    out["head"], out["why"] = nn_headline(out["rows"])
    if out["head"] is None:
        out["stop"] = "no_headline"
    return out


def nn_table_dp(rows: list[dict]) -> int:
    """The history table's one precision ($M): the smallest decimal
    count at which EVERY complete row's displayed terms reproduce its
    displayed NCAV by hand — the hand-check discipline; a table the
    reader cannot reconcile is worse than none."""
    comp = [r for r in rows if r["ncav"] is not None]
    for nd in range(0, 4):
        ok = True
        for r in comp:
            t = [round(v / 1e6, nd) for v in (r["ca"], r["l"], r["pref"],
                                              r["mezz"], r["nci"], r["ncav"])]
            if round(t[0] - t[1] - t[2] - t[3] - t[4], nd) != t[5]:
                ok = False
                break
        if ok:
            return nd
...
# ── the page's sentences, every one a testable builder ───────────────

def money(x) -> str:
    return f"\\${x:,.2f}" if abs(x) < 100 else f"\\${x:,.0f}"


def nn_metric_ps(v: float) -> str:
    """Per-share metric string — st.metric takes plain text, sign
    passed through (negative NCAV renders as itself, never hidden)."""
    return f"${v:,.2f}"


def nn_method_text() -> str:
    return (
        "**Whose method.** Benjamin Graham's net-current-asset-value test "
        "(*Security Analysis*; *The Intelligent Investor*): NCAV = current "
        "assets − total liabilities − claims ahead of the common, per share, "
        "against the price; his two-thirds-of-NCAV buying threshold is stated "
        "as HIS criterion wherever it prints, never a verdict of this "
        "page's. **What is ours:** every refusal and route; the honest "
        "context that a true net-net is nearly extinct among filers large "
        "enough to file readable XBRL — most names print NCAV far below the "
        "price, many negative, and the page says that is the normal answer, "
        "not a failure; the liquidation-frame caveat; the derived-liabilities "
        "arithmetic printed where only the classified halves are filed; and "
        "the deduction of tagged minority interest and temporary equity, "
        "labelled as this kit's own addition where it fires. **Annual "
        "balance sheets only**, said plainly: every figure is an annual "
        "report's own year-end instant, quarterly balance sheets are "
        "excluded by date, and a year enters the history only through its "
        "own annual report's balance date. No income statement is read "
        "anywhere on this page.")


def nn_threshold_sentence(ncav_ps: float) -> str:
    return (f"**Graham's buying criterion: two-thirds of NCAV = "
            f"{money(ncav_ps * 2.0 / 3.0)} per share.** The two-thirds "
            "threshold is his (*The Intelligent Investor*), stated here as "
            "his criterion — never a verdict, and never this page's advice.")


def nn_negative_threshold_sentence() -> str:
    return ("Graham's two-thirds criterion has no purchase here: NCAV is at "
            "or below zero, so there is no floor for a threshold to sit "
            "under.")


def nn_normal_answer(ncav_ps, price) -> str:
    """Fires whenever NCAV per share sits below the price — which is
...
def nn_caveat() -> str:
    return ("NCAV is a balance-sheet floor, not a going-concern value: it "
            "takes current assets at book, and a real wind-down realizes "
            "inventory and receivables below book, so the true floor is "
            "lower than the figure shown, never higher.")


def nn_addition_fires(row: dict) -> bool:
    """The kit's-own-addition label fires exactly where the deduction
    goes beyond Graham's named claims: tagged minority interest, or a
    mezzanine slot answered by the temporary-equity TOTAL (which may
    carry redeemable NCI); a slot answered by the specific
    redeemable-preferred element is Graham's own preferred."""
    return (row.get("nci", 0.0) > 0.0
            or (row.get("mezz", 0.0) > 0.0
                and row.get("mezz_tag") == "TemporaryEquityCarryingAmount"))


def nn_addition_label() -> str:
    return ("**Deducting tagged minority interest and temporary equity is "
            "this kit's own addition to Graham's test.** His formula names "
            "total liabilities and preferred stock; today's consolidated "
            "balance sheets carry claims his era's statements did not "
            "present — noncontrolling interests and mezzanine (temporary) "
            "equity — and every claim ranking ahead of the common comes out "
            "of a liquidation floor. The test is Graham's; this deduction "
            "is ours, and it is labelled so here.")


def nn_deduction_caption(row: dict) -> str:
    parts = []
    if row["pref"] > 0:
        parts.append(f"preferred stock [PreferredStockValue] "
                     f"{money(row['pref'] / 1e6)}M deducted")
    else:
        parts.append("no preferred equity element tagged at this date — "
                     "deducted at zero")
    if row["mezz"] > 0:
        p = (f"mezzanine [{row['mezz_tag']}] {money(row['mezz'] / 1e6)}M "
             "deducted")
        if row["mezz_note"]:
            p += " (" + row["mezz_note"] + ")"
        parts.append(p)
    else:
        parts.append("no redeemable-preferred or temporary-equity element "
                     "tagged — deducted at zero")
    if row["nci"] > 0:
        parts.append(f"minority interest [MinorityInterest] "
                     f"{money(row['nci'] / 1e6)}M deducted")
    else:
        parts.append("no minority-interest element tagged — deducted at zero")
    return (f"Deductions at FY{row['fy']}: " + "; ".join(parts)
            + ". Carrying amounts, not liquidation preferences: where a "
            "preference exceeds its carrying value, the floor is lower than "
            "shown.")


def nn_history_note() -> str:
    return ("NCAV per share for up to ten annual balance dates, every year "
            "at TODAY'S share count (the reader's standard route), labelled "
            "as such: the column shows the balance sheet's trend, not "
            "buybacks — mixing count vintages would make the trend "
            "unreadable. A dashed year names the line missing at its exact "
            "date; nothing fills, and nothing carries across years.")


def nn_financial_stop(tk: str, fin_class: str, fin_reason: str) -> str:
    desc = {"bank": "a bank", "insurer": "an insurer", "reit": "a REIT",
            "broker": "a client-asset broker"}.get(fin_class)
    if desc:
        return (f"**{tk} is {desc}.** {fin_reason} NCAV means nothing on "
                "this balance sheet: deposits and policy reserves are its "
                "\"total liabilities\", the asset side is not working "
                "capital, and Graham's test assumes an operating company's "
                "current assets. The Financials Checker prices banks, "
                "insurers, REITs and client-asset brokers on a filed base "
                "per share — use that page.")
    return (f"**{tk} is in a financial class this kit refuses.** "
            f"{fin_reason} NCAV means nothing on this balance sheet, and "
            "no page here prices this class honestly either.")


def nn_foreign_stop(tk: str, ccy: str, n_f: int, n_usd: int) -> str:
    return (f"**{tk} tags its balance sheet in {ccy}, not US dollars** — "
            f"{n_f} annual balance facts in {ccy} against {n_usd} in USD, "
            "and the few USD facts a foreign issuer tags are usually "
            "convenience translations for a year or two. The Non-US Checker "
            "prices 20-F and 40-F filers in the filing currency, with a "
            "paste-your-own-figures mode for companies EDGAR has never "
            "heard of — use that page.")


def nn_unclassified_stop(tk: str, l_years: int, l_latest: int) -> str:
    tail = (f"its total liabilities line reads fine ({l_years} annual "
            f"balance dates through FY{l_latest}), so this is the "
            "statement's presentation, not a dead filer"
            if l_years else
            "no liabilities total reads either — nothing on this balance "
            "sheet answers in the form the test needs")
    return (f"**{tk} files an unclassified balance sheet.** No AssetsCurrent "
            "fact exists in any of its annual filings — the statement "
            "presents assets by nature, without the current/noncurrent "
            "split, and NCAV needs the filed current-asset subtotal; "
            + tail + ". Graham's test has no honest answer on a balance "
            "sheet without a current split, and this page refuses rather "
            "than derive one.")


def nn_foreign_route_suffix(msg: str) -> str:
    ...
    if ("Foreign private issuers are " + "not supported") not in msg:
        return ""
    return (" The Non-US Checker prices 20-F and 40-F filers in the filing "
            "currency, with a paste-your-own-figures mode for companies "
            "EDGAR has never heard of — use that page.")


NN_COUNT_LABEL = "today's count, the reader's standard route"


# ── the record (network side) ────────────────────────────────────────

def nn_record(t: str) -> dict:
    """One name's full Net-Nets record. Firing order is the design's:
    financial gate → foreign → unclassified → ladder/headline → the
    reader's standard count route (load raises its own sentences) →
    price, for the comparison only."""
    cmap = _ticker_map()
    ...
    rec = {"ticker": t, "resolved": resolved, "sic": sic,
           "sic_desc": sic_desc, "fin_class": fin_class,
           "fin_reason": fin_reason, "stop": "", "why": ""}
    if fin_class != "ordinary":
        rec["stop"] = "financial"
        return rec
    _ccy = currency_facts(facts, _NN_DISCOVERY)
    ...
    if _f:
        ...
        if _mn >= _ccy.get("USD", 0):
            ...
            return rec
    rec.update(nn_scan(facts))
    if rec["stop"]:
        return rec
    _yrs, _notes, meta = load(t)
    _fnote = next((n for n in _notes
    ...
    if _fnote:
        ...
        return rec
    rec["shares"] = meta.get("shares") or 0.0
    try:
        rec["price"] = current_price(resolved)
    except Exception:
        rec["price"] = None
    head = rec["head"]
...
def _nn_fx(concept_rows: dict) -> dict:
    return {"facts": {"us-gaap": {
        c: {"units": {"USD": [
            {"end": e, "val": v, "accn": a, "filed": f, "form": fm}
            for (e, v, a, f, fm) in rows]}}
        for c, rows in concept_rows.items()}}}


_NN_PDEX_FIX = _nn_fx({
    "AssetsCurrent": [
        ("2019-06-30", 21783000, "0001553350-19-000942", "2019-09-12", "10-K"),
        ("2019-06-30", 20845000, "0001553350-20-000854", "2020-09-10", "10-K"),
        ("2020-06-30", 22674000, "0001553350-20-000854", "2020-09-10", "10-K"),
        ("2020-06-30", 22674000, "0001553350-21-000795", "2021-09-09", "10-K"),
        ("2021-06-30", 25013000, "0001553350-21-000795", "2021-09-09", "10-K"),
        ("2025-06-30", 47295000, "0001079973-25-001426", "2025-09-04", "10-K"),
        ("2025-06-30", 47295000, "0001079973-26-001215", "2026-09-03", "10-K"),
        ("2026-06-30", 53248000, "0001079973-26-001215", "2026-09-03", "10-K"),
        ("2026-03-31", 53710000, "0001079973-26-000580", "2026-04-30", "10-Q"),
    ],
    "Liabilities": [
        ("2019-06-30", 8512000, "0001553350-19-000942", "2019-09-12", "10-K"),
        ("2019-06-30", 8512000, "0001553350-20-000854", "2020-09-10", "10-K"),
        ("2019-07-02", 3300000, "0001553350-20-000854", "2020-09-10", "10-K"),
        ("2020-06-30", 12064000, "0001553350-20-000854", "2020-09-10", "10-K"),
        ("2019-07-02", 3300000, "0001553350-21-000795", "2021-09-09", "10-K"),
        ("2021-06-30", 20236000, "0001553350-21-000795", "2021-09-09", "10-K"),
        ("2025-06-30", 24560000, "0001079973-25-001426", "2025-09-04", "10-K"),
        ("2025-06-30", 24560000, "0001079973-26-001215", "2026-09-03", "10-K"),
        ("2026-06-30", 26876000, "0001079973-26-001215", "2026-09-03", "10-K"),
    ],
})

_NN_HRMY_FIX = _nn_fx({
    "AssetsCurrent": [
        ("2019-12-31", 31497000, "0001564590-21-015274", "2021-03-25", "10-K"),
        ("2020-12-31", 262891000, "0001564590-21-015274", "2021-03-25", "10-K"),
        ("2020-12-31", 262891000, "0001558370-22-002257", "2022-02-28", "10-K"),
        ("2021-12-31", 284439000, "0001558370-22-002257", "2022-02-28", "10-K"),
        ("2024-12-31", 579252000, "0001104659-26-018859", "2026-02-24", "10-K"),
        ("2025-12-31", 907014000, "0001104659-26-018859", "2026-02-24", "10-K"),
    ],
    "Liabilities": [
        ("2019-12-31", 118288000, "0001564590-21-015274", "2021-03-25", "10-K"),
        ("2020-12-31", 329894000, "0001564590-21-015274", "2021-03-25", "10-K"),
        ("2020-12-31", 329894000, "0001558370-22-002257", "2022-02-28", "10-K"),
        ("2021-12-31", 246936000, "0001558370-22-002257", "2022-02-28", "10-K"),
        ("2024-12-31", 340045000, "0001104659-26-018859", "2026-02-24", "10-K"),
        ("2025-12-31", 401413000, "0001104659-26-018859", "2026-02-24", "10-K"),
    ],
})

_NN_JOE_FIX = _nn_fx({
    "Liabilities": [
        ("2023-12-31", 825040000, "0001558370-25-001603", "2025-02-26", "10-K"),
        ("2024-12-31", 801839000, "0001558370-25-001603", "2025-02-26", "10-K"),
        ("2024-12-31", 801839000, "0001104659-26-019676", "2026-02-25", "10-K"),
        ("2025-12-31", 742801000, "0001104659-26-019676", "2026-02-25", "10-K"),
    ],
})


# ══════════════════════════════════════════════════════════════════════
#  SELF-TESTS — the Net-Nets suite (checks numbered per the committed
#  list of 20 Sep 2026; 28 is the flagged addition, the page-11
#  footer-adjacency structural check)
# ══════════════════════════════════════════════════════════════════════

def nn_self_test() -> list[tuple[str, bool, str]]:
    out = []

    # 1 — the registration drift pin
    out.append(("NN 1: page tags equal the registration of record",
                NN_TAGS == NN_TAGS_REGISTERED
                and NN_REGISTERED_AS_OF == "2026-09-20",
                "bands were committed on these lists"))

    # synthetic maps for the ladder checks
    _l1 = {"2025-12-31": 100.0}
    _lc = {"2025-12-31": 60.0, "2024-12-31": 60.0}
    _ln = {"2025-12-31": 41.5, "2023-12-31": 40.0}
    # 2 — L1 outranks L2
    out.append(("NN 2: ladder order — the filed total outranks the derived sum",
                nn_liab_at("2025-12-31", _l1, _lc, _ln)[0:2] == (100.0, "L1"),
                "L1 first"))
    # 3 — L2 needs both halves at the SAME date
    _r3a = nn_liab_at("2024-12-31", {}, _lc, _ln)
    _r3b = nn_liab_at("2023-12-31", {}, _lc, _ln)
    out.append(("NN 3: the derived sum needs both halves at the same exact "
                "date — a lone half refuses naming the missing lines",
                _r3a[0] is None and "LiabilitiesNoncurrent" in _r3a[3]
                and _r3b[0] is None and "LiabilitiesCurrent" in _r3b[3]
                and "Liabilities" in _r3a[3],
                "no cross-date sum ever"))
    # 4 — the addition formatter hand-checks
    _a4 = nn_arith(60.4, 40.4)
    out.append(("NN 4: derived arithmetic hand-checks at its own precision",
                nn_arith(60.0, 40.0) == "60 + 40 = 100"
                and _a4 == "60.4 + 40.4 = 100.8",
                _a4))
    # 5 — overlap cross-check within $2 passes, total served
    out.append(("NN 5: total vs halves within $2 passes and the total serves",
                nn_liab_at("2025-12-31", {"2025-12-31": 100.0},
                           {"2025-12-31": 60.0}, {"2025-12-31": 41.5})
                == (100.0, "L1", "", "")
                and nn_liab_at("2025-12-31", {"2025-12-31": 100.0},
                               {"2025-12-31": 60.0}, {"2025-12-31": 40.0})
                == (100.0, "L1", "", ""),
                "tolerance $2"))
    # 6 — overlap disagreement refuses naming both figures
    _r6 = nn_liab_at("2025-12-31", {"2025-12-31": 100.0},
                     {"2025-12-31": 60.0}, {"2025-12-31": 45.0})
    out.append(("NN 6: total vs halves beyond $2 refuses naming both figures "
                "— the poisoned figure never serves",
                _r6[0] is None and _r6[1] == "xfail"
                and "100" in _r6[3] and "105" in _r6[3]
                and "refuses" in _r6[3],
                _r6[3][:70]))
    # 7 — no equity element anywhere: the structural no-A−E pin
    _all7 = [c for v in NN_TAGS.values() for c in v]
    out.append(("NN 7: no Assets-minus-equity rung — no equity or total-assets "
                "element exists anywhere in the page's tag lists",
                set(_all7) == {"AssetsCurrent", "Liabilities",
                               "LiabilitiesCurrent", "LiabilitiesNoncurrent",
                               "PreferredStockValue",
                               "RedeemablePreferredStockCarryingAmount",
                               "TemporaryEquityCarryingAmount",
                               "MinorityInterest"}
                and not any("StockholdersEquity" in c or c == "Assets"
                            for c in _all7),
                "eight elements, frozen"))
    # 8 / 9 — a missing required line dashes the year naming it
    _m8 = {"ca": {}, "l1": {"2025-12-31": 50.0}, "lc": {}, "ln": {},
           "pref": {}, "rp": {}, "te": {}, "nci": {},
           "dates": ["2025-12-31"]}
    _r8 = nn_rows_from_maps(_m8)[0]
    out.append(("NN 8: missing AssetsCurrent dashes the year naming it — "
                "never zero-filled, never carried",
                _r8["ncav"] is None and "AssetsCurrent not tagged" in _r8["why"],
                _r8["why"]))
    _m9 = {"ca": {"2025-12-31": 80.0}, "l1": {}, "lc": {}, "ln": {},
           "pref": {}, "rp": {}, "te": {}, "nci": {},
           "dates": ["2025-12-31"]}
    _r9 = nn_rows_from_maps(_m9)[0]
    out.append(("NN 9: missing liabilities dashes the year naming every "
                "missing element",
                _r9["ncav"] is None and "Liabilities" in _r9["why"]
                and "LiabilitiesCurrent" in _r9["why"]
                and "LiabilitiesNoncurrent" in _r9["why"],
                _r9["why"][:70]))
    # 10 — headline falls back to the latest complete year
    _rows10 = [{"fy": 2026, "date": "2026-12-31", "ncav": None, "why": "x"},
               {"fy": 2025, "date": "2025-12-31", "ncav": 5.0, "why": ""}]
    _h10, _ = nn_headline(_rows10)
    out.append(("NN 10: a dashed latest year falls back to the newest "
                "complete year as headline",
                _h10 is not None and _h10["fy"] == 2025,
                "FY2025 heads"))
    # 11 — the staleness ceiling refuses beyond 3
    _rows11a = [{"fy": 2026, "date": "d", "ncav": None, "why": "x"},
                {"fy": 2022, "date": "d", "ncav": 5.0, "why": ""}]
    _rows11b = [{"fy": 2025, "date": "d", "ncav": None, "why": "x"},
                {"fy": 2022, "date": "d", "ncav": 5.0, "why": ""}]
    _h11a, _w11a = nn_headline(_rows11a)
    _h11b, _ = nn_headline(_rows11b)
    out.append(("NN 11: a headline more than 3 years behind the latest filed "
                "date refuses with the staleness sentence; 3 exactly passes",
                _h11a is None and "beyond 3 years" in _w11a
                and _h11b is not None and _h11b["fy"] == 2022,
                "ceiling 3"))
    # 12 — permanent preferred deducts where tagged
    _m12 = {"ca": {"2025-12-31": 100.0}, "l1": {"2025-12-31": 40.0},
            "lc": {}, "ln": {}, "pref": {"2025-12-31": 5.0},
            "rp": {}, "te": {}, "nci": {}, "dates": ["2025-12-31"]}
    out.append(("NN 12: tagged preferred stock deducts from the floor",
                nn_rows_from_maps(_m12)[0]["ncav"] == 55.0,
                "100 − 40 − 5"))
    # 13 — mezzanine one-not-both, the passed-over element named
    _v13, _t13, _n13 = nn_mezz_at("2025-12-31", {"2025-12-31": 10.0},
                                  {"2025-12-31": 25.0})
    _v13b, _t13b, _ = nn_mezz_at("2025-12-31", {}, {"2025-12-31": 25.0})
    out.append(("NN 13: mezzanine slot takes one element, never both — the "
                "specific outranks the general and the passed-over is named",
                _v13 == 10.0
                and _t13 == "RedeemablePreferredStockCarryingAmount"
                and "passed over" in _n13 and "25" in _n13
                and _v13b == 25.0
                and _t13b == "TemporaryEquityCarryingAmount",
                "one, never a sum"))
    # 14 — NCI deducts, and the addition label's trigger is exact
    _m14 = {"ca": {"2025-12-31": 100.0}, "l1": {"2025-12-31": 40.0},
            "lc": {}, "ln": {}, "pref": {}, "rp": {},
            "te": {}, "nci": {"2025-12-31": 7.0}, "dates": ["2025-12-31"]}
    _r14 = nn_rows_from_maps(_m14)[0]
    out.append(("NN 14: tagged minority interest deducts, and the kit's-own-"
                "addition trigger fires on NCI or the temporary-equity total, "
                "never on redeemable preferred alone",
                _r14["ncav"] == 53.0 and nn_addition_fires(_r14)
                and nn_addition_fires({"nci": 0.0, "mezz": 5.0,
                                       "mezz_tag":
                                       "TemporaryEquityCarryingAmount"})
                and not nn_addition_fires(
                    {"nci": 0.0, "mezz": 5.0,
                     "mezz_tag": "RedeemablePreferredStockCarryingAmount"})
                and not nn_addition_fires({"nci": 0.0, "mezz": 0.0,
                                           "mezz_tag": ""}),
                "trigger exact"))
    # 15 — absent slots read zero with the absence named
    _m15 = {"ca": {"2025-12-31": 100.0}, "l1": {"2025-12-31": 40.0},
            "lc": {}, "ln": {}, "pref": {}, "rp": {}, "te": {},
            "nci": {}, "dates": ["2025-12-31"]}
    _r15 = nn_rows_from_maps(_m15)[0]
    _c15 = nn_deduction_caption(_r15)
    out.append(("NN 15: absent deduction slots read zero with the absence "
                "named — an untagged element is evidence, a missing total "
                "is not",
                _r15["ncav"] == 60.0
                and _c15.count("deducted at zero") == 3
                and "preferred equity" in _c15
                and "minority-interest" in _c15,
                "zero with the note"))
    # 16 — the addition label owns the design
    out.append(("NN 16: the kit's-own-addition label names Graham, the claim "
                "class, and owns the deduction as ours",
                "kit's own addition" in nn_addition_label()
                and "Graham" in nn_addition_label()
                and "ranking ahead of the common" in nn_addition_label()
                and "this deduction is ours" in nn_addition_label(),
                "labelled ours"))
    # 17 — the two-thirds wording pin
    _s17 = nn_threshold_sentence(9.0)
    out.append(("NN 17: the two-thirds line computes and is attributed as "
                "Graham's criterion, never a verdict; the negative variant "
                "states no purchase",
                "two-thirds" in _s17 and "6.00" in _s17
                and "his criterion" in _s17 and "Graham" in _s17
                and nn_disowned(_s17)
                and "no purchase" in nn_negative_threshold_sentence(),
                "criterion, attributed"))
    # 18 — the banned-words scan over every output sentence
    _fixtures = [nn_method_text(), nn_threshold_sentence(9.0),
                 nn_negative_threshold_sentence(),
                 nn_normal_answer(2.0, 50.0), nn_caveat(),
                 nn_addition_label(), nn_deduction_caption(_r15),
                 nn_history_note(),
                 nn_unclassified_stop("JOE", 3, 2025),
                 nn_financial_stop("KNSL", "insurer", "r."),
                 nn_foreign_stop("T", "EUR", 12, 0)]
    out.append(("NN 18: no output sentence uses a banned or verdict word — "
                "and the disowned rule rejects a bare verdict",
                all(not any(b in s.lower() for b in NN_BANNED)
                    for s in _fixtures)
                and all(nn_disowned(s) for s in _fixtures)
                and not nn_disowned("the verdict is clear"),
                "banned scan"))
    ...
    out.append(("NN 19: the normal-answer sentence fires whenever NCAV sits "
                ...
                "fires below only"))
    # 20 — the liquidation caveat is directioned
    out.append(("NN 20: the liquidation caveat directions the error — the "
                "true floor is lower, never higher",
                "lower than the figure shown" in nn_caveat()
                and "never higher" in nn_caveat()
                and "going-concern" in nn_caveat()
                and "wind-down" in nn_caveat(),
                "directioned"))
    # 21 — PDEX, the arithmetic control, from the pinned paste rows
    _sp = nn_scan(_NN_PDEX_FIX)
    _pd_rows = {r["fy"]: r for r in _sp["rows"]}
    _dp21 = nn_table_dp(_sp["rows"])
    out.append(("NN 21: PDEX control — FY2026 NCAV exactly 26,372,000 from "
                "the pinned filed facts, every rung L1, the graded column "
                "reproduced, the table hand-checks at 3dp ($M)",
                _sp["head"] is not None and _sp["head"]["fy"] == 2026
                and _sp["head"]["ncav"] == 26372000.0
                and _pd_rows[2025]["ncav"] == 22735000.0
                and _pd_rows[2021]["ncav"] == 4777000.0
                and _pd_rows[2020]["ncav"] == 10610000.0
                and all(r["l_rung"] == "L1" for r in _sp["rows"])
                and _dp21 == 3
                and round(53.248 - 26.876, 3) == round(26372000.0 / 1e6, 3),
                "to the dollar"))
    # 22 — HRMY: the crossing, the headline band, and the discovery rule
    _sh = nn_scan(_NN_HRMY_FIX)
    _h_rows = {r["fy"]: r for r in _sh["rows"]}
    out.append(("NN 22: HRMY — FY2025 NCAV exactly 505,601,000, the negative-"
                "to-positive crossing at FY2021, and a comparative-only year "
                "never enters (a year needs its own annual report)",
                _sh["head"] is not None
                and _sh["head"]["ncav"] == 505601000.0
                and _h_rows[2020]["ncav"] == -67003000.0
                and _h_rows[2021]["ncav"] == 37503000.0
                and "2019-12-31" not in _sh["maps"]["dates"]
                and "2024-12-31" not in _sh["maps"]["dates"],
                "crossing + discovery"))
    # 23 — JOE: the unclassified refusal, CA-driven on a live filer
    _sj = nn_scan(_NN_JOE_FIX)
    _sj_sent = nn_unclassified_stop("JOE", _sj.get("l_years", 0),
                                    _sj.get("l_latest", 0))
    out.append(("NN 23: JOE shape — no AssetsCurrent in any filing refuses "
                "naming the unclassified balance sheet while the liabilities "
                "line is shown reading fine",
                _sj["stop"] == "unclassified"
                and _sj["l_years"] == 3 and _sj["l_latest"] == 2025
                and "unclassified balance sheet" in _sj_sent
                and "AssetsCurrent" in _sj_sent
                and "3 annual" in _sj_sent and "FY2025" in _sj_sent
                and "not a dead filer" in _sj_sent,
                "CA-driven"))
    # 24 — negative NCAV renders as itself
    out.append(("NN 24: negative NCAV renders with its sign — metric and "
                "table cell alike, never hidden or floored",
                nn_metric_ps(-1.5) == "$-1.50"
                and f"{round(_h_rows[2020]['ncav'] / 1e6, 3):,.3f}"
                == "-67.003",
                "sign passes through"))
    # 25 — the history ceiling and the today's-count label
    _m25 = {"ca": {f"{y}-12-31": 100.0 for y in range(2014, 2026)},
            "l1": {f"{y}-12-31": 40.0 for y in range(2014, 2026)},
            "lc": {}, "ln": {}, "pref": {}, "rp": {}, "te": {}, "nci": {},
            "dates": sorted([f"{y}-12-31" for y in range(2014, 2026)],
                            reverse=True)}
    _r25 = nn_rows_from_maps(_m25)
    out.append(("NN 25: the history caps at ten dates, newest kept, and the "
                "note pins the today's-count label",
                len(_r25) == 10 and _r25[0]["fy"] == 2025
                and _r25[-1]["fy"] == 2016
                and "TODAY'S share count" in nn_history_note()
                and "ten annual balance dates" in nn_history_note(),
                "ten, newest first"))
    # 26 — routes: the gate, the foreign stop, and the reader's phrase
    _self26 = Path(__file__).read_text(encoding="utf-8")
    out.append(("NN 26: the financial stop routes the four priced classes to "
                "the Financials Checker with NCAV's own reason, the refused "
                "class gets the no-page sentence, the foreign stop and "
                "suffix route to the Non-US Checker, and the reader's "
                "foreign phrase still exists in the span",
                all(_route_ok(nn_financial_stop("T", c, "r."))
                    and "NCAV means nothing" in nn_financial_stop("T", c, "r.")
                    for c in ("bank", "insurer", "reit", "broker"))
                and "no page here prices" in nn_financial_stop("T", "refused",
                                                               "r.")
                and _route_ok(nn_foreign_stop("T", "EUR", 12, 0))
                and _route_ok(nn_foreign_route_suffix(
                    "Foreign private issuers are " + "not supported"))
                and nn_foreign_route_suffix("something else") == ""
                and ("Foreign private issuers are " + "not supported")
                in _self26,
                "routed by menu name"))
    # 27 — the exact-date keying, on PDEX's own poisoned facts
    out.append(("NN 27: exact-date keying — PDEX's 2019-07-01/02 ASC 842 "
                "adoption facts (3,300, newest-filed 2021) never enter the "
                "date set, FY2019 serves the restated CA 20,845,000 and the "
                "true L 8,512,000, and the quarterly 10-Q instant is "
                "form-filtered",
                "2019-07-02" not in _sp["maps"]["dates"]
                and "2019-07-01" not in _sp["maps"]["dates"]
                and "2026-03-31" not in _sp["maps"]["dates"]
                and _pd_rows[2019]["ca"] == 20845000.0
                and _pd_rows[2019]["l"] == 8512000.0
                and _pd_rows[2019]["ncav"] == 12333000.0
                and set(_sp["maps"]["dates"]) == {"2019-06-30", "2020-06-30",
                                                  "2021-06-30", "2025-06-30",
                                                  "2026-06-30"},
                "the seam that forced the design"))
    # 28 — (flagged addition) footer adjacency, the page-11 structural check
    _stops, _covered = 0, 0
    ...
    for _i, _l in enumerate(_slines):
        if "st.stop()" in _l and "adjacency" not in _l:
            ...
            if any("_page_footer()" in p for p in _prev):
                _covered += 1
    out.append(("NN 28: every stop renders the footer first — source-level "
                "adjacency",
...
# literal dollar amounts in markdown surfaces are escaped via money().
...
def _page_footer() -> None:
    ...
    with _r1:
        with st.expander("What the numbers mean", expanded=False):
            st.markdown(
                "**NCAV** — current assets minus total liabilities minus "
                "every tagged claim ahead of the common (preferred stock, "
                "temporary equity, minority interest): Graham's liquidation "
                "floor, from the annual balance sheet alone.\\n\\n"
                "**NCAV per share** — the floor on the reader's standard "
                "count route, today's count for every year shown.\\n\\n"
                "**Two-thirds line** — Graham's own buying threshold, "
                "stated as his criterion wherever it prints.\\n\\n"
                + nn_caveat()
            )
    with _r2:
        with st.expander("Verify the engine"):
            st.caption(
                ...
                "prove it unchanged. The Net-Nets checks pin this page's "
                "arithmetic: the liabilities ladder and its cross-check, "
                "the exact-date keying, every deduction slot and guard, "
                "every refusal and route, and the known-answer facts the "
                "20 Sep 2026 registration committed."
            )
            if st.button("Run checks"):
                for _label, _suite in (("Engine", self_test()),
                                       ("Net-Nets", nn_self_test())):
    ...
    st.caption(
        "Research aid, not financial advice. The method follows Benjamin "
        "Graham's published writing (Security Analysis; The Intelligent "
        "Investor); the refusals, the normal-answer context, the derived-"
        "liabilities arithmetic and the minority-interest and temporary-"
        "equity deduction are this project's own. Independent; not "
        "affiliated with or endorsed by any estate or publisher."
    )


st.set_page_config(
    page_title="Net-Nets — Investor Toolkit",
    page_icon="🧊",
    layout="centered",
...
st.title("🧊 Net-Nets")
st.caption("Benjamin Graham's net-current-asset-value test — the deep-value "
           "floor, from annual balance sheets alone, for one ticker at a time")
st.info(nn_method_text())

with st.form("nn_form"):
    _tk = st.text_input("Stock ticker",
                        placeholder="PDEX · HRMY · MSFT — press Enter"
                        ).strip().upper()
    _nn_go = st.form_submit_button("Compute NCAV", type="primary")

if _nn_go and not _tk:
    st.warning("Enter a ticker first.")
if _nn_go and _tk:
    try:
        _rec = nn_record(_tk)
    except ValueError as _e:
        st.error(str(_e) + nn_foreign_route_suffix(str(_e)))
        _page_footer()
        st.stop()

    if _rec["stop"] == "financial":
        st.error(nn_financial_stop(_tk, _rec["fin_class"],
                                   _rec.get("fin_reason")
    ...
    if _rec["stop"] == "foreign":
        ...
        st.error(nn_foreign_stop(_tk, _cu, _cn, _cus))
        _page_footer()
        st.stop()
    if _rec["stop"] == "foreign_note":
        st.error(_rec["fnote"] + nn_foreign_route_suffix(_rec["fnote"]))
        _page_footer()
        st.stop()
    ...
    if not _rec.get("shares"):
        st.error("The reader's count route returned no share count, so the "
                 "per-share figure cannot be computed honestly.")
        _page_footer()
        st.stop()
    ...
    if _rows and _rows[0]["ncav"] is None:
        st.warning(f"FY{_rows[0]['fy']} ({_rows[0]['date']}) is incomplete — "
                   f"{_rows[0]['why']} — so the headline is the latest "
                   f"complete year, FY{_head['fy']}.")

    _c1, _c2, _c3, _c4 = st.columns(4)
    with _c1:
        st.metric(f"NCAV, FY{_head['fy']}",
                  f"${_head['ncav'] / 1e6:,.1f}M")
    with _c2:
        st.metric("NCAV per share", nn_metric_ps(_ps))
    with _c3:
        st.metric("Price today", f"${_px:,.2f}" if _px else "—")
    with _c4:
        st.metric("Price ÷ NCAV/share",
                  f"{_px / _ps:,.2f}×" if (_px and _ps and _ps > 0) else "—")

    if _head["l_rung"] == "L2":
        st.info("**Total liabilities (derived)** — no filed total is tagged "
                "at this date; the figure is the two filed halves with the "
                f"addition shown: [{_head['l_arith']}].")
    if _ps is not None and _ps > 0:
        st.markdown(nn_threshold_sentence(_ps))
    else:
        st.markdown(nn_negative_threshold_sentence())
    if _px is None:
        st.warning("No current price could be fetched — the comparison and "
                   "the normal-answer sentence need it; NCAV itself is "
                   "price-free and stands above.")
    _na = nn_normal_answer(_ps, _px)
    if _na:
        st.info(_na)
    if nn_addition_fires(_head):
        st.info(nn_addition_label())
    st.caption(nn_deduction_caption(_head))
    st.caption(nn_caveat())

    _dp = nn_table_dp(_rows)
    _fys = [r["fy"] for r in _rows]
    _dups = {fy for fy in _fys if _fys.count(fy) > 1}
    st.dataframe(pd.DataFrame([{
        "FY": (str(r["fy"]) if r["fy"] not in _dups else r["date"]),
        "Balance date": r["date"],
        "Current assets $M": (None if r["ca"] is None
                              else round(r["ca"] / 1e6, _dp)),
        "Total liabilities $M": (None if r["l"] is None
                                 else round(r["l"] / 1e6, _dp)),
        "Deductions $M": round((r["pref"] + r["mezz"] + r["nci"]) / 1e6, _dp),
        "NCAV $M": (None if r["ncav"] is None
                    else round(r["ncav"] / 1e6, _dp)),
        "NCAV/share (today's count)": (
            None if r["ncav"] is None
            else round((r["ncav"] / 1e6) / _rec["shares"], 2)),
        "Note": (r["why"] if r["ncav"] is None
                 else ("derived: " + r["l_arith"] if r["l_rung"] == "L2"
                       else "")),
    } for r in _rows]), width='stretch', hide_index=True)
    st.caption(nn_history_note())

    with st.expander("Assumptions used — paste this if a figure looks wrong"):
        st.code("\n".join([
            f"ticker {_rec['resolved']}   headline FY{_head['fy']} "
            f"({_head['date']})   rung {_head['l_rung']}"
            + (f"   [{_head['l_arith']}]" if _head["l_rung"] == "L2" else ""),
            f"CA {_head['ca']:,.0f}   L {_head['l']:,.0f}   "
            f"pref {_head['pref']:,.0f}   mezz {_head['mezz']:,.0f}"
            ...
            f"({_rec['maps']['dates'][-1]} .. {_rec['maps']['dates'][0]})",
