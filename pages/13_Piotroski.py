"""
Piotroski F-Score — Joseph Piotroski's nine binary tests
========================================================
Nine financial-strength binaries from "Value Investing: The Use of
Historical Financial Statement Information" (2000), each computed from
filed data with HIS definitions and scaling, each shown with its
inputs, for ONE ticker at a time from audited SEC annual filings.

THE HONESTY THE PAGE IS BUILT ON
--------------------------------
TESTS PASSED, EACH SHOWN — evidence, never a verdict, never a buy
signal. A test whose inputs do not read for the needed years REFUSES
by name, and the summary prints "N of M readable tests passed" with M
stated plainly: a 6 of 8 is honest where a 6 of 9 with a silent dash
is not. Fewer than five readable tests refuses the page, because a
score on a minority of the tests is not the F-Score. A stale
balance-sheet line refuses its test naming the last year it was
filed, never serving a zero into a deterioration test, and every
change test demands consecutive fiscal years (330-400 days between
year-end dates, the same window the reader demands of annual flows),
refusing across a fiscal year-end change rather than bridging it.
Financials route to the Financials Checker with the kit's own reason;
foreign filers route to the Non-US Checker.
...
21 Sep 2026 with ZERO divergence: it already carries the FLEET
cross-tag-fill _annual docstring and load()'s shares_by_fy meta key
this page's issuance test consumes. (The span's own menu-map comment
stays at its 12-Sep vintage by decision of record; THIS docstring is
the file's map of record.)
...
# ══════════════════════════════════════════════════════════════════════
#  PIOTROSKI F-SCORE — nine binary tests (this page's own code; the
#  reader above is the Tragic Algebra Analyzer's)
# ══════════════════════════════════════════════════════════════════════
#
# Everything from here down is this page's own. The span above is the
# reader of record byte-verbatim, copied from tool 1 AFTER Deploy 0
# (21 Sep 2026), so it carries the FLEET cross-tag-fill _annual
# docstring and load()'s shares_by_fy meta key with ZERO divergence —
# the first page whose span needed no licensed exception.
#
# Whose method: Joseph Piotroski, "Value Investing: The Use of
# Historical Financial Statement Information" (2000) — nine binary
# financial-strength tests, HIS definitions including his scaling:
# ROA, CFO and asset turnover scale by BEGINNING-of-year total assets;
# the leverage ratio scales by AVERAGE total assets, so tests 3, 5 and
# 9 need three consecutive year-end dates and the page says so. What
# is ours: every refusal and route; the per-test refusal with the
# summary printing "N of M readable tests passed" with M stated
# plainly; the floor (fewer than five readable tests is not the
# F-Score); the never-tagged-vs-stale doctrine on long-term debt; the
# count-route share-issuance test with its decomposition context; and
# the "≤" reading of test 5 (his wording is a strict fall; strict-<
# fails every filer that has no debt and keeps none, which inverts a
# deterioration test, so unchanged-at-zero scores as strength — stated
# as the kit's reading of his intent). One score, nine binaries, no
# adjusted variant: the F-Score is nine binaries by definition, and
# the kit's extra evidence rides as context lines, not as a fork.
#
# WHY EXACT DATES for this page's instants (page 12's doctrine,
# carried verbatim): calendar-year keying can serve an adoption-day
# fact (PDEX's ASC 842 3,300 where 8,512 is true). Every page-local
# instant is keyed by its exact end date, newest-filed per date, and a
# fiscal year-end is discovered as the LATEST instant an annual filing
# tags. Consecutive fiscal years are two discovered dates 330–400 days
# apart — the SAME duration window _annual applies to annual flows —
# so a fiscal year-end change (HRB's 2021-04-30 → 2022-06-30, 426
# days) refuses structurally, never bridges. The one stated asymmetry:
# test 7's counts are the ENGINE's fy-keyed route via load()'s
# shares_by_fy, because agreement with the engine outranks the date
# doctrine for that line.

PIO_REGISTERED_AS_OF = "2026-09-21"

PIO_FLOOR = 5            # readable tests below this: not the F-Score
PIO_PAIR_MIN_DAYS = 330  # consecutive-year window, both bounds shared
PIO_PAIR_MAX_DAYS = 400  # with _annual's own annual-duration filter

PIO_TAGS = {
    # New annual flow lines, page-local _annual calls. CFO fills
    # across the pair on the "N" precedent: the continuing-operations
    # element is definitionally narrower (excludes discontinued ops),
    # and the fill note names the filled years and both tags. PDEX
    # itself is the live case: FY2015 exists ONLY under the continuing
    # element (registration pastes, 21 Sep 2026).
    "CFO": ["NetCashProvidedByUsedInOperatingActivities",
            "NetCashProvidedByUsedInOperatingActivitiesContinuingOperations"],
    # GrossProfit only, no derivation rung in v1: the cost-of-revenue
    # family is a minefield and the kit adds rungs on evidence (the MF
    # ladder precedent). A filer without the subtotal refuses test 8
    # by name — HRB is the registered exhibit (NoSuchKey, never
    # tagged).
    "GP": ["GrossProfit"],
    # New instant lines, exact-date keyed.
    "TA": ["Assets"],
    # Long-term debt: preference is FAITHFULNESS — LongTermDebt is his
    # "total long-term debt" (current portion included);
    # LongTermDebtNoncurrent serves only where the total does not
    # cover the test's dates, with the narrower definition named.
    # DebtLongtermAndShorttermCombinedAmount is deliberately EXCLUDED:
    # it contains short-term borrowings, which are not long-term debt.
    # ONE TAG PER TEST — mixing the total at t with the noncurrent
    # portion at t-1 manufactures a fake delta out of a maturity
    # reclass. NOTE the deliberate difference from the engine's
    # net-cash ladder (narrow-first): different purposes, both shown
    # in the panel so neither surprises.
    "LTD": ["LongTermDebt", "LongTermDebtNoncurrent"],
    "CA": ["AssetsCurrent"],
    "CL": ["LiabilitiesCurrent"],
}

# The registration of record: the lists the 21 Sep 2026 paste evidence
# and committed figures were registered against; the self-test pins
# the page to them so it cannot drift from what the bands stand on.
PIO_TAGS_REGISTERED = {
    "CFO": ["NetCashProvidedByUsedInOperatingActivities",
            "NetCashProvidedByUsedInOperatingActivitiesContinuingOperations"],
    "GP": ["GrossProfit"],
...
_PIO_DISCOVERY = ["Assets", "AssetsCurrent", "LiabilitiesCurrent"]

PIO_TEST_NAMES = {
    1: "ROA positive", 2: "Operating cash flow positive",
    3: "ROA improving", 4: "Cash flow above net income",
    5: "Leverage not rising", 6: "Current ratio improving",
    7: "No new shares issued", 8: "Gross margin improving",
    9: "Asset turnover improving",
}

PIO_BANNED = (" buy ", " sell ", " cheap", "expensive",
              "undervalued", "overvalued", "bargain",
              " strong ", " weak ", " healthy ", " distressed ")


def pio_disowned(s: str) -> bool:
    """"verdict" may appear in an output sentence ONLY inside a
    disowning phrase ("never a verdict") — a refusal must be allowed
    to name the thing it refuses (page 12's rule, verbatim)."""
    t = s.lower()
    i = t.find("verdict")
    while i != -1:
        if "never" not in t[max(0, i - 25):i]:
            return False
        i = t.find("verdict", i + 1)
    return True


# ── page-local instant reads, keyed by exact date ────────────────────
# pio_rows / pio_by_date / pio_yearend_dates are page 12's _nn_rows /
# nn_by_date / nn_yearend_dates with the pio_ prefix — bodies verbatim
# (the date-keying doctrine carrier), renamed only.

def pio_rows(facts: dict, concept: str) -> list[dict]:
    """Annual-form USD instant facts for one concept, unfiltered by
    date — the raw material for both discovery and lookup."""
...
def pio_by_date(facts: dict, concept: str) -> dict[str, float]:
    """{exact end date: value}, newest-filed per date across annual
    filings — a 10-K restates a comparative and the restatement wins,
    at its own date, never at a neighbouring one."""
    got: dict[str, tuple[str, float]] = {}
    for row in pio_rows(facts, concept):
        end, filed = str(row["end"]), str(row.get("filed", ""))
        if end not in got or filed > got[end][0]:
            got[end] = (filed, float(row.get("val", 0.0)))
    return {k: v[1] for k, v in got.items()}


def pio_yearend_dates(facts: dict, concepts: list[str]) -> list[str]:
    """Fiscal year-end dates, newest first: a date qualifies only when
    it is the LATEST instant some annual filing tags for one of these
    concepts — each annual report's own balance-sheet date. So a year
    enters the history only through its own annual report; adoption-day
    instants and re-tagged quarterly comparatives are never the latest
    date of their filing and are excluded structurally."""
    per_acc: dict[str, str] = {}
    for concept in concepts:
        for row in pio_rows(facts, concept):
            acc, end = str(row.get("accn", "")), str(row["end"])
            if acc not in per_acc or end > per_acc[acc]:
                per_acc[acc] = end
    return sorted(set(per_acc.values()), reverse=True)
...
def pio_gap_days(d_new: str, d_old: str) -> int:
    return (dt.date.fromisoformat(d_new) - dt.date.fromisoformat(d_old)).days


def pio_consecutive(d_new: str, d_old: str) -> bool:
    """Two year-end dates are consecutive fiscal years iff they are
    330–400 days apart — the same window _annual demands of an annual
    flow, so instants and flows define "a year apart" identically.
    HRB's 2021-04-30 → 2022-06-30 (426 days, the year-end change) is
    the live filer this refuses; a 52/53-week retail year (371 days)
    passes."""
    return PIO_PAIR_MIN_DAYS <= pio_gap_days(d_new, d_old) <= PIO_PAIR_MAX_DAYS


def pio_pair_why(f_new: int, f_old: int, fy2d: dict, dups: set) -> str:
    """The one refusal sentence for a broken delta pair: names the
    duplicate end-year (a fiscal year-end change), the missing year,
    ...
    if f_new not in fy2d or f_old not in fy2d:
        ...
        return (f"no annual balance date is discovered for FY{missing} — "
                "the year has no annual report of its own")
    d_new, d_old = fy2d[f_new], fy2d[f_old]
    if not pio_consecutive(d_new, d_old):
        return (f"the two year-end dates ({d_old} → {d_new}) are "
                f"{pio_gap_days(d_new, d_old)} days apart — outside "
                f"{PIO_PAIR_MIN_DAYS}–{PIO_PAIR_MAX_DAYS}, not consecutive "
                "fiscal years (a fiscal year-end change); refused rather "
                "than bridged")
    return ""


# ── the long-term-debt family rules ──────────────────────────────────

def pio_ltd_serve(ltd_t: dict, ltd_n: dict, need: list[str]
                  ) -> tuple[dict[str, float] | None, str, str, str]:
    """(values at the needed dates, serving tag, note, refusal why).

    Rule 1, faithfulness: LongTermDebt (his total) outranks the
    noncurrent portion. Rule 2, one tag per test: the serving tag must
    read at EVERY needed date. Rule 3, never-vs-stale: a family never
    tagged in any annual filing is evidence of no long-term debt and
    serves zero with the note (the untagged-preferred logic); a family
    tagged elsewhere but absent at a needed date REFUSES naming the
    tag and its last date — "retired to zero" and "stale with billions
    outstanding" are indistinguishable at the concept level, and
    Progressive's $6.9B is what serving zero costs (ADBE/PGR lesson,
    applied before the defect)."""
    if not ltd_t and not ltd_n:
        return ({d: 0.0 for d in need}, "",
                "never tagged: no long-term-debt element in any annual "
                "filing — read as no long-term debt", "")
    if all(d in ltd_t for d in need):
        return {d: ltd_t[d] for d in need}, "LongTermDebt", "", ""
    if all(d in ltd_n for d in need):
        return ({d: ltd_n[d] for d in need}, "LongTermDebtNoncurrent",
                "read from LongTermDebtNoncurrent — the noncurrent "
                "portion only; current maturities sit outside it", "")
    cov = []
    for tag, m in (("LongTermDebt", ltd_t),
                   ("LongTermDebtNoncurrent", ltd_n)):
        have = sum(1 for d in need if d in m)
        last = max(m) if m else "never at a year-end"
        cov.append(f"{tag} covers {have} of {len(need)} needed dates "
                   f"(last {last})")
    return (None, "", "",
            "no single long-term-debt element reads at every date this "
            "test needs — " + "; ".join(cov) + "; a stale debt line is "
            "refused, never served as zero")


# ── hand-check formatters ────────────────────────────────────────────

def pio_pct_arith(num: float, den: float) -> str:
    """num / den printed at the coarsest percentage precision where
    the DISPLAYED terms reproduce the DISPLAYED percentage exactly —
    the page-11 formatter's discipline, ratio form."""
    for nd in range(2, 7):
        fn, fd = f"{num:,.0f}", f"{den:,.0f}"
        pct = f"{num / den * 100:,.{nd}f}%"
        redo = (float(fn.replace(",", "")) / float(fd.replace(",", ""))
                * 100)
        if f"{redo:,.{nd}f}%" == pct:
            return f"{fn} / {fd} = {pct}"
    return f"{num:,.0f} / {den:,.0f} = {num / den * 100:,.6f}%"


def pio_pct(x: float, nd: int = 2) -> str:
    return f"{x * 100:,.{nd}f}%"


def pio_delta_line(r_new: float, r_old: float, cmp_word: str) -> str:
    """The delta stated FROM THE DISPLAYED ratios, so the reader's own
    subtraction of the two printed figures reproduces the printed
    delta exactly."""
    a, b = round(r_new * 100, 2), round(r_old * 100, 2)
    return (f"{a:,.2f}% vs {b:,.2f}% — change {a - b:+,.2f} pts "
            f"({cmp_word})")


# ── the nine tests ───────────────────────────────────────────────────

def _t(n: int, status: str, detail: str, why: str = "") -> dict:
    return {"n": n, "name": PIO_TEST_NAMES[n], "status": status,
            "detail": detail, "why": why}


def pio_tests(rec: dict) -> list[dict]:
    """The nine binaries at the headline year, each PASS / FAIL /
    REFUSED with its inputs in the detail and every refusal named.
    Same-source-year discipline: every figure is keyed to the headline
    pair's own fiscal years and exact dates; nothing fills across a
    test, nothing bridges a year-end change."""
    F = rec["fy"]
    fy2d, dups = rec["fy2d"], rec["dups"]
    dF, d1, d2 = fy2d.get(F), fy2d.get(F - 1), fy2d.get(F - 2)
    ta, ca, cl = rec["ta"], rec["ca"], rec["cl"]
    ni, cfo, gp, rev = rec["ni"], rec["cfo"], rec["gp"], rec["rev"]
    counts = rec["counts"]
    out: list[dict] = []
    pair10 = pio_pair_why(F, F - 1, fy2d, dups)
    pair21 = pio_pair_why(F - 1, F - 2, fy2d, dups)

    def beg_ratio(flow_val: float) -> str:
        """The paper's displayed ratio for tests 1/2: the flow over
        BEGINNING-of-year assets. The BINARY is the flow's sign
        (beginning assets are positive by construction); the ratio is
        display, and an unreadable denominator is named, never faked."""
        if pair10 == "" and d1 in ta and ta[d1] > 0:
            return " — " + pio_pct_arith(flow_val, ta[d1]) + \
                   f" of beginning assets ({d1})"
        return (" — beginning assets not readable at the prior year-end, "
                "so only the sign is shown (the binary needs only the "
                "sign)")

    # 1 — ROA > 0
    if F not in ni:
        out.append(_t(1, "REFUSED", "",
                      f"net income not read at FY{F} by the reader"))
    else:
        st1 = "PASS" if ni[F] > 0 else "FAIL"
        out.append(_t(1, st1, f"net income {ni[F]:,.0f}" + beg_ratio(ni[F])))

    # 2 — CFO > 0
    if F not in cfo:
        out.append(_t(2, "REFUSED", "",
                      f"operating cash flow not read at FY{F} "
                      "(no element in the CFO pair answers the year)"))
    else:
        st2 = "PASS" if cfo[F] > 0 else "FAIL"
        out.append(_t(2, st2, f"CFO {cfo[F]:,.0f}" + beg_ratio(cfo[F])))

    # 3 — delta ROA > 0 (beginning-of-year scaling, HIS definition:
    #     needs assets at the two PRIOR year-ends, both pairs sound)
    why3 = ""
    if F not in ni or (F - 1) not in ni:
        why3 = f"net income not read at FY{F} and FY{F - 1} both"
    elif pair10:
        why3 = pair10
    elif pair21:
        why3 = pair21
    elif d1 not in ta or d2 not in ta:
        miss = d1 if d1 not in ta else d2
        why3 = f"Assets not tagged at {miss} — the beginning-assets " \
               "denominator his scaling needs"
    if why3:
        out.append(_t(3, "REFUSED", "", why3))
    else:
        r_t, r_p = ni[F] / ta[d1], ni[F - 1] / ta[d2]
        st3 = "PASS" if r_t > r_p else "FAIL"
        out.append(_t(3, st3,
                      f"ROA FY{F}: " + pio_pct_arith(ni[F], ta[d1])
                      + f"; ROA FY{F - 1}: " + pio_pct_arith(ni[F - 1],
                                                             ta[d2])
                      + "; " + pio_delta_line(r_t, r_p,
                                              "beginning-assets scaling, "
                                              "his definition")))

    # 4 — CFO > net income, strict (both scaled by the same beginning
    #     assets in the paper, so the scaling cancels)
    if F not in cfo or F not in ni:
        out.append(_t(4, "REFUSED", "",
                      f"CFO and net income both needed at FY{F}; "
                      + ("CFO" if F not in cfo else "net income")
                      + " not read"))
    else:
        st4 = "PASS" if cfo[F] > ni[F] else "FAIL"
        out.append(_t(4, st4,
                      f"CFO {cfo[F]:,.0f} vs net income {ni[F]:,.0f} "
                      f"(strict >; the paper's common scaling cancels)"))

    # 5 — delta leverage <= 0 on AVERAGE total assets (three dates)
    why5 = ""
    ltdv = tag5 = note5 = None
    if pair10:
        why5 = pair10
    elif pair21:
        why5 = pair21
    else:
        missing_ta = [d for d in (dF, d1, d2) if d not in ta]
        if missing_ta:
            why5 = ("Assets not tagged at " + ", ".join(missing_ta)
                    + " — his AVERAGE-assets scaling needs all three "
                    "year-end dates")
        else:
            ltdv, tag5, note5, why5 = pio_ltd_serve(
                rec["ltd_t"], rec["ltd_n"], [dF, d1, d2])
    if why5:
        out.append(_t(5, "REFUSED", "", why5))
    else:
        avg_t = (ta[dF] + ta[d1]) / 2.0
        avg_p = (ta[d1] + ta[d2]) / 2.0
        r_t, r_p = ltdv[dF] / avg_t, ltdv[d1] / avg_p
        st5 = "PASS" if r_t <= r_p else "FAIL"
        det = (f"LTD FY{F}: " + pio_pct_arith(ltdv[dF], avg_t)
               + f" of average assets; FY{F - 1}: "
               + pio_pct_arith(ltdv[d1], avg_p) + "; "
               + pio_delta_line(r_t, r_p, "<= 0 passes — the kit's "
                                "stated reading of his intent"))
        if tag5:
            det += f" [element: {tag5}]"
        if note5:
            det += " — " + note5
        out.append(_t(5, st5, det))
    ...
    if pair10:
        ...
        for d in (dF, d1):
            if d not in ca:
                why6 = f"AssetsCurrent not tagged at {d}"
                break
            if d not in cl:
                why6 = f"LiabilitiesCurrent not tagged at {d}"
                break
            if cl[d] == 0:
                why6 = (f"LiabilitiesCurrent reads zero at {d} — the "
                        "ratio is undefined")
                break
    if why6:
        out.append(_t(6, "REFUSED", "", why6))
    else:
        r_t, r_p = ca[dF] / cl[dF], ca[d1] / cl[d1]
        st6 = "PASS" if r_t > r_p else "FAIL"
        out.append(_t(6, st6,
                      f"FY{F}: {ca[dF]:,.0f} / {cl[dF]:,.0f} = "
                      f"{r_t:,.3f}; FY{F - 1}: {ca[d1]:,.0f} / "
                      f"{cl[d1]:,.0f} = {r_p:,.3f}; change "
                      f"{r_t - r_p:+,.3f} (strict >)"))

    # 7 — no new shares issued: the ENGINE's count route, raw
    if F not in counts or (F - 1) not in counts:
        miss = F if F not in counts else F - 1
        out.append(_t(7, "REFUSED", "",
                      f"the reader's count route reads no year-end "
                      f"count for FY{miss}"))
    elif pair10:
        out.append(_t(7, "REFUSED", "", pair10))
    else:
        s0, s1 = counts[F - 1], counts[F]
        raw = s1 - s0
        st7 = "PASS" if raw <= 0 else "FAIL"
        ds_engine = rec["ds"].get(F)
        nonsbc = None
        if ds_engine is not None:
            nonsbc = round(raw / 1e6 - ds_engine, 6)
        det = (f"{s0:,.0f} → {s1:,.0f} shares (raw change {raw:+,.0f}; "
               "split-restated, the reader's standard route)")
        if nonsbc is not None:
            det += (f"; the engine's compensation net change is "
                    f"{ds_engine:+,.3f}M and tagged corporate issuance "
                    "the engine excluded (acquisitions, offerings, "
                    f"conversions) is {max(nonsbc, 0.0):,.3f}M")
            if st7 == "PASS" and nonsbc > 0:
                det += (" — the count fell only because buybacks "
                        "exceeded that tagged issuance; his own test "
                        "reads the count")
        out.append(_t(7, st7, det))

    # 8 — delta gross margin > 0
    why8 = ""
    if not gp and not rec["gp_src"]:
        why8 = "GrossProfit never tagged in any annual filing"
    elif F not in gp or (F - 1) not in gp:
        last8 = max(gp) if gp else 0
        why8 = (f"GrossProfit reads {len(gp)} year(s) to FY{last8} — "
                f"not at FY{F} and FY{F - 1} both")
    elif pair10:
        why8 = pair10
    elif F not in rev or (F - 1) not in rev or rev[F] <= 0 \
            or rev[F - 1] <= 0:
        why8 = f"revenue not read positive at FY{F} and FY{F - 1} both"
    if why8:
        out.append(_t(8, "REFUSED", "", why8))
    else:
        r_t, r_p = gp[F] / rev[F], gp[F - 1] / rev[F - 1]
        st8 = "PASS" if r_t > r_p else "FAIL"
        out.append(_t(8, st8,
                      f"margin FY{F}: " + pio_pct_arith(gp[F], rev[F])
                      + f"; FY{F - 1}: " + pio_pct_arith(gp[F - 1],
                                                         rev[F - 1])
                      + "; " + pio_delta_line(r_t, r_p, "strict >")))

    # 9 — delta asset turnover > 0 (beginning-assets scaling)
    why9 = ""
    if F not in rev or (F - 1) not in rev:
        why9 = f"revenue not read at FY{F} and FY{F - 1} both"
    elif pair10:
        why9 = pair10
    elif pair21:
        why9 = pair21
        ...
        miss = d1 if d1 not in ta else d2
        why9 = f"Assets not tagged at {miss} — the beginning-assets " \
               "denominator his scaling needs"
    if why9:
        out.append(_t(9, "REFUSED", "", why9))
    else:
        r_t, r_p = rev[F] / ta[d1], rev[F - 1] / ta[d2]
        st9 = "PASS" if r_t > r_p else "FAIL"
        out.append(_t(9, st9,
                      f"turnover FY{F}: " + pio_pct_arith(rev[F], ta[d1])
                      + f"; FY{F - 1}: " + pio_pct_arith(rev[F - 1],
                                                         ta[d2])
                      + "; " + pio_delta_line(r_t, r_p,
                                              "beginning-assets scaling")))
    return out
...
def pio_summary(tests: list[dict]) -> tuple[int, int, str]:
    """(passed, readable, the summary sentence). M is stated plainly
    and every refused test is named with its reason — a 6 of 8 is
    honest where a 6 of 9 with a silent dash is not."""
    readable = [t for t in tests if t["status"] != "REFUSED"]
    passed = [t for t in readable if t["status"] == "PASS"]
    n, m = len(passed), len(readable)
    s = f"**{n} of {m} readable tests passed.**"
    refused = [t for t in tests if t["status"] == "REFUSED"]
    if refused:
        s += (" Refused: "
              + "; ".join(f"test {t['n']} ({t['name']}) — {t['why']}"
                          for t in refused) + ".")
    return n, m, s


def pio_floor_stop(tk: str, m: int) -> str:
    return (f"**Only {m} of the nine tests are readable for {tk}, and a "
            "score on a minority of the tests is not the F-Score.** "
            f"The floor is {PIO_FLOOR} readable tests; the tag panel "
            "below names every element read or looked for, and each "
            "refused test's reason is in the table above it.")


# ── the page's sentences ─────────────────────────────────────────────

def pio_method_text() -> str:
    return (
        "**Whose method.** Joseph Piotroski's nine binary "
        "financial-strength tests (\"Value Investing: The Use of "
        "Historical Financial Statement Information\", 2000), his "
        "definitions and his scaling: ROA, cash flow and asset turnover "
        "scale by beginning-of-year total assets, the leverage ratio by "
        "average total assets — so tests 3, 5 and 9 need three "
        "consecutive year-end dates, and a filer's third year is the "
        "first that can answer them in full. **What is ours:** every "
        "refusal and route; the per-test refusal, named, with the "
        "summary printing tests passed of tests READABLE, M stated "
        "plainly; the floor (fewer than five readable tests is not the "
        "F-Score); the share-issuance test read from the same count "
        "route the engine grades, with the year's change decomposed "
        "beside it; and the \"unchanged or lower\" reading of the "
        "leverage test, stated as this kit's reading of his intent, "
        "because a strict fall would fail every filer that has no debt "
        "and keeps none. TESTS PASSED, EACH SHOWN — evidence, never a "
        "verdict, never a buy signal. One score, nine binaries, no "
        "adjusted variant.")


def pio_financial_stop(tk: str, fin_class: str, fin_reason: str) -> str:
    desc = {"bank": "a bank", "insurer": "an insurer", "reit": "a REIT",
            "broker": "a client-asset broker"}.get(fin_class)
    if desc:
        return (f"**{tk} is {desc}.** {fin_reason} The leverage and "
                "current-ratio tests presume a classified nonfinancial "
                "balance sheet — a bank's deposits are not current "
                "liabilities and its leverage is its business model — "
                "so the F-Score's binaries are the wrong frame here. "
                "The Financials Checker prices banks, insurers, REITs "
                "and client-asset brokers on a filed base per share — "
                "use that page.")
    return (f"**{tk} is in a financial class this kit refuses.** "
            f"{fin_reason} The F-Score's balance-sheet tests are the "
            "wrong frame here, and no page in this kit prices this "
            "class honestly either.")


def pio_foreign_stop(tk: str, ccy: str, n_f: int, n_usd: int) -> str:
...
def pio_foreign_route_suffix(msg: str) -> str:
...
def pio_no_dates_stop(tk: str) -> str:
    return (f"**No annual balance date could be discovered for {tk}.** "
            "No Assets, AssetsCurrent or LiabilitiesCurrent instant "
            "exists in any of its annual filings, so no fiscal year-end "
            "can be established and no test has a date to stand on. The "
            "tag panel below names every element this page looked for.")


def pio_cfo_fill_note(origin: dict[int, str]) -> str:
    """Fires when the CFO pair's two elements both answered: names the
    filled years and the definitional gap, the "N" fill precedent."""
    ...
    return ("Operating cash flow was filled across two elements: the "
            ...
            + " — the continuing-operations element excludes "
            "discontinued operations, a definitional gap named rather "
            "than hidden.")


# ── the record ───────────────────────────────────────────────────────

def pio_flow(facts: dict, tags: list[str], fill: bool,
             recent: bool = False
             ) -> tuple[dict[int, float], list[str], dict[int, str]]:
    """One page-local annual flow through the span's own _annual, raw
    dollars per fiscal year, with sources and per-year origin."""
    src: list[str] = []
    origin: dict[int, str] = {}
    ser = _annual(facts, tags, [], src, fill, recent, origin)
    return {fy: float(v[2]) for fy, v in ser.items()}, src, origin


def pio_record(t: str) -> dict:
    """One name's full F-Score record. Firing order is page 12's:
    financial gate → foreign → discovery → the reader (load raises its
    own sentences, inherited verbatim) → page-local reads → tests."""
    cmap = _ticker_map()
    ...
    if fin_class != "ordinary":
        rec["stop"] = "financial"
        return rec
    _ccy = currency_facts(facts, _PIO_DISCOVERY)
    ...
    if _f:
        ...
        if _mn >= _ccy.get("USD", 0):
            ...
            return rec
    rec["maps"] = {
        "ta": pio_by_date(facts, PIO_TAGS["TA"][0]),
        "ltd_t": pio_by_date(facts, PIO_TAGS["LTD"][0]),
        "ltd_n": pio_by_date(facts, PIO_TAGS["LTD"][1]),
        "ca": pio_by_date(facts, PIO_TAGS["CA"][0]),
        "cl": pio_by_date(facts, PIO_TAGS["CL"][0]),
        "dates": pio_yearend_dates(facts, _PIO_DISCOVERY),
    }
    if not rec["maps"]["dates"]:
        rec["stop"] = "no_dates"
        return rec
    ...
    if _fnote:
        ...
        return rec
    fy2d, dups = pio_fy_map(rec["maps"]["dates"])
    cfo, cfo_src, cfo_origin = pio_flow(facts, PIO_TAGS["CFO"], True)
    gp, gp_src, _ = pio_flow(facts, PIO_TAGS["GP"], False)
    # Revenue page-locally with the ENGINE'S OWN lists and flags (in
    # RECENCY_KEYS, not FILL_KEYS) — page 11's deployed _mf_rev
    # precedent; safe because no registry route mutates REV.
    rev, rev_src, _ = pio_flow(facts, CONCEPTS["REV"][0], False, True)
    rec.update({
        "fy": int(rec["maps"]["dates"][0][:4]),
        "fy2d": fy2d, "dups": dups,
        "ta": rec["maps"]["ta"], "ca": rec["maps"]["ca"],
        "cl": rec["maps"]["cl"],
        "ltd_t": rec["maps"]["ltd_t"], "ltd_n": rec["maps"]["ltd_n"],
        # Filed dollars back from the Year records (the engine's basis,
        # Up-C rebase included); round() recovers the exact filed
        # dollar from the /1e6 round-trip for any real filer size.
        "ni": {y.fy: float(round(y.N * 1e6)) for y in years},
        "ds": {y.fy: y.dS for y in years},
        "g": {y.fy: y.G for y in years},
        "t_bb": {y.fy: y.T for y in years},
        "cfo": cfo, "cfo_src": cfo_src, "cfo_origin": cfo_origin,
        "gp": gp, "gp_src": gp_src, "rev": rev, "rev_src": rev_src,
        "counts": dict(meta.get("shares_by_fy", {})),
        "engine_tags": meta.get("tags", []),
    })
    rec["tests"] = pio_tests(rec)
    rec["n"], rec["m"], rec["summary"] = pio_summary(rec["tests"])
    return rec


# ── the tag panel ────────────────────────────────────────────────────

def _pio_tag_panel(rec: dict) -> None:
    """Every element this page read or looked for — rendered on the
    success path and on BOTH structural stops (the page-12 check-29
    lesson, inherited on day one: a stop that promises a panel renders
    it)."""
    with st.expander("Tag panel — every element this page read or "
        ...
        def _irow(label, m):
            return {"Element": label, "Dates": len(m),
                    "Latest": max(m) if m else "not found"}

        def _frow(label, ser, src):
            return {"Element": label + (f"  [{' + '.join(src)}]"
                                        if src else ""),
                    "Dates": len(ser),
                    "Latest": (f"FY{max(ser)}" if ser else "not found")}

        st.dataframe(pd.DataFrame([
            _irow("Assets", maps.get("ta", {})),
            _irow("LongTermDebt", maps.get("ltd_t", {})),
            _irow("LongTermDebtNoncurrent", maps.get("ltd_n", {})),
            _irow("AssetsCurrent", maps.get("ca", {})),
            _irow("LiabilitiesCurrent", maps.get("cl", {})),
            _frow("Operating cash flow", rec.get("cfo", {}),
                  rec.get("cfo_src", [])),
            _frow("GrossProfit", rec.get("gp", {}),
                  rec.get("gp_src", [])),
            _frow("Revenue", rec.get("rev", {}),
                  rec.get("rev_src", [])),
        ]), width='stretch', hide_index=True)
        st.caption("Instant lines keyed by exact year-end date, annual "
                   "forms only — a year enters only through its own "
                   "annual report's balance date; flow lines are the "
                   "span's fiscal-year reads. Net income and the share "
                   "counts come from the reader's standard route; its "
                   "full panel is below where a run reached it.")
        if rec.get("engine_tags"):
            st.dataframe(pd.DataFrame(rec["engine_tags"]),
                         width='stretch', hide_index=True)


# ── pinned fixtures — verbatim rows from the 21 Sep 2026 pastes ──────

def _pio_fx(concept_rows: dict) -> dict:
...
def _pi(end, val, accn, filed, form="10-K"):
    return {"end": end, "val": val, "accn": accn, "filed": filed,
            "form": form}


_PIO_PDEX_FIX = _pio_fx({
    "Assets": [
        # 2011-06-30 is comparative-only: it appears in the FY2012
        # 10-K (whose own latest instant is 2012-06-30) and in 10-Qs.
        _pi("2011-06-30", 15561000, "0001193125-12-389350", "2012-09-12"),
        _pi("2011-06-30", 15561000, "0001193125-11-304375", "2011-11-09",
            "10-Q"),
        _pi("2012-06-30", 12822000, "0001193125-12-389350", "2012-09-12"),
        # The FY2022 restatement seam: the FY2023 10-K (filed late,
        # 2023-10-13) refiled 47,326,000 as 49,089,000 — newest-filed
        # per date serves the restatement.
        _pi("2022-06-30", 47326000, "0001553350-22-000745", "2022-09-08"),
        _pi("2022-06-30", 49089000, "0001079973-23-001429", "2023-10-13"),
        # The FY2023 10-K also tags QUARTERLY instants inside the
        # annual form — never that accession's latest date, excluded
        # structurally by the discovery rule.
        _pi("2022-12-31", 51876000, "0001079973-23-001429", "2023-10-13"),
        _pi("2023-06-30", 51823000, "0001079973-23-001429", "2023-10-13"),
        _pi("2024-06-30", 52477000, "0001553350-24-000079", "2024-09-05"),
        _pi("2025-06-30", 61192000, "0001079973-25-001426", "2025-09-04"),
        _pi("2026-06-30", 74509000, "0001079973-26-001215", "2026-09-03"),
        _pi("2026-03-31", 74977000, "0001079973-26-000580", "2026-04-30",
            "10-Q"),
    ],
})

# The committed control figures (registration of record, 21 Sep 2026):
_PDEX_TA = {"2026-06-30": 74509000.0, "2025-06-30": 61192000.0,
            "2024-06-30": 52477000.0}
_PDEX_LTD = {"2026-06-30": 17513000.0, "2025-06-30": 15431000.0,
             "2024-06-30": 11956000.0}
_PDEX_CFO_2026 = 7242000.0
_PDEX_GP = {2026: 24335000.0, 2025: 19510000.0}


def _pio_fixture_rec(**over) -> dict:
    """A minimal healthy three-date record for the test fixtures; every
    field overridable so each check isolates one behaviour."""
    dates = ["2026-06-30", "2025-06-30", "2024-06-30"]
    fy2d, dups = pio_fy_map(dates)
    rec = {"fy": 2026, "fy2d": fy2d, "dups": dups,
           "ta": {d: 1000.0 for d in dates},
           "ca": {d: 500.0 for d in dates},
           "cl": {d: 250.0 - 10 * i for i, d in enumerate(dates)},
           "ltd_t": {d: 100.0 for d in dates}, "ltd_n": {},
           "ni": {2026: 120.0, 2025: 100.0},
           "cfo": {2026: 150.0, 2025: 130.0},
           "gp": {2026: 300.0, 2025: 250.0}, "gp_src": ["GrossProfit"],
           "rev": {2026: 900.0, 2025: 800.0},
           "counts": {2026: 1000.0, 2025: 1000.0},
           "ds": {2026: 0.0}, "g": {}, "t_bb": {}}
    rec.update(over)
    return rec
...
# ══════════════════════════════════════════════════════════════════════
#  SELF-TESTS — the Piotroski suite (30 checks, the committed list of
#  21 Sep 2026)
# ══════════════════════════════════════════════════════════════════════

def pio_self_test() -> list[tuple[str, bool, str]]:
    out = []

    # 1 — registration drift pin
    out.append(("PIO 1: page tags equal the registration of record",
                PIO_TAGS == PIO_TAGS_REGISTERED
                and PIO_REGISTERED_AS_OF == "2026-09-21",
                "committed figures stand on these lists"))

    # 2 — the discovery rule on PDEX's own facts
    _d2 = pio_yearend_dates(_PIO_PDEX_FIX, _PIO_DISCOVERY)
    out.append(("PIO 2: discovery — comparative-only 2011-06-30, the "
                "annual form's quarterly instants and 10-Q dates never "
                "enter; each year only through its own annual report",
                "2011-06-30" not in _d2 and "2022-12-31" not in _d2
                and "2026-03-31" not in _d2
                and _d2[0] == "2026-06-30" and "2012-06-30" in _d2,
                "the page-12 doctrine, live on Assets"))

    # 3 — newest-filed per exact date (the FY2022 restatement seam)
    _m3 = pio_by_date(_PIO_PDEX_FIX, "Assets")
    out.append(("PIO 3: newest-filed-per-date — PDEX FY2022 Assets "
                "serves the FY2023 10-K's restated 49,089,000, at its "
                "own date only",
                _m3["2022-06-30"] == 49089000.0
    ...
    _f5, _du5 = pio_fy_map(["2022-06-30", "2021-04-30"])
    _w5 = pio_pair_why(2022, 2021, _f5, _du5)
    out.append(("PIO 5: 365 and 371-day (52/53-week) pairs pass; HRB's "
                "426-day year-end-change pair and a 300-day pair refuse "
                "naming the gap — the window is _annual's own 330-400",
                pio_consecutive("2026-06-30", "2025-06-30")
                and pio_consecutive("2025-02-01", "2024-01-27")
                and not pio_consecutive("2022-06-30", "2021-04-30")
                and not pio_consecutive("2022-06-30", "2021-09-04")
                and "426 days" in _w5 and "not consecutive" in _w5
                and (PIO_PAIR_MIN_DAYS, PIO_PAIR_MAX_DAYS) == (330, 400),
                "the HRB shape banked from paste 7"))

    # 6 — test 1: binary is the flow's sign; ratio shows his
    #     beginning-assets scaling
    _r6 = _pio_fixture_rec()
    _t6 = pio_tests(_r6)
    _t1 = _t6[0]
    out.append(("PIO 6: test 1 binary from net income's sign, ratio on "
                "beginning assets at the prior year-end, named",
                _t1["status"] == "PASS" and "beginning assets" in
                _t1["detail"] and "2025-06-30" in _t1["detail"]
                and pio_tests(_pio_fixture_rec(
                    ni={2026: -5.0, 2025: 3.0}))[0]["status"] == "FAIL",
                "sign decides; assets display"))

    # 7 — test 2 on the committed PDEX figures (display arithmetic
    #     hand-checks; the ratio is 11.83%, the registered 11.84% was
    #     a recorded display miss)
    _a7 = pio_pct_arith(_PDEX_CFO_2026, _PDEX_TA["2025-06-30"])
    out.append(("PIO 7: test 2's PDEX control — CFO 7,242,000 over "
                "beginning assets 61,192,000 displays 11.83% and the "
                "displayed terms reproduce it",
                _a7 == "7,242,000 / 61,192,000 = 11.83%",
                _a7))

    # 8 — the scaling pin: a fixture where END-year scaling would flip
    #     test 3's sign; the paper's beginning scaling must FAIL it
    _r8 = _pio_fixture_rec(
        ni={2026: 110.0, 2025: 100.0},
        ta={"2026-06-30": 2100.0, "2025-06-30": 2000.0,
            "2024-06-30": 1000.0})
    _t8 = pio_tests(_r8)[2]
    out.append(("PIO 8: beginning-assets scaling pinned — NI grew 10% "
                "but beginning assets doubled, so his ROA falls and "
                "test 3 FAILS where end-year scaling would pass it",
                _t8["status"] == "FAIL" and "5.50%" in _t8["detail"]
                and "10.00%" in _t8["detail"],
                "end-scaling would read 5.24% vs 5.00% and pass"))

    # 9 — test 4 strict: a tie fails
    _r9 = _pio_fixture_rec(cfo={2026: 120.0, 2025: 1.0})
    out.append(("PIO 9: test 4 is strict — CFO equal to net income "
                "fails; above passes",
                pio_tests(_r9)[3]["status"] == "FAIL"
                and pio_tests(_pio_fixture_rec())[3]["status"] == "PASS",
                "accruals test, his inequality"))

    # 10 — test 5 on the committed PDEX dollars, to the displayed digit
    _r10 = _pio_fixture_rec(ta=dict(_PDEX_TA), ltd_t=dict(_PDEX_LTD))
    _t10 = pio_tests(_r10)[4]
    out.append(("PIO 10: test 5's PDEX control — 25.81% vs 27.15% of "
                "average assets, change -1.34 pts, PASS with dollars UP "
                "and the ratio DOWN, element named",
                _t10["status"] == "PASS" and "25.81%" in _t10["detail"]
                and "27.15%" in _t10["detail"]
                and "-1.34 pts" in _t10["detail"]
                and "[element: LongTermDebt]" in _t10["detail"],
                "his definition is the ratio, not the balance"))

    # 11 — test 5's <= boundary and the never-tagged zero
    _r11a = _pio_fixture_rec(ltd_t={d: 100.0 for d in
                                    ("2026-06-30", "2025-06-30",
                                     "2024-06-30")},
                             ta={d: 1000.0 for d in
                                 ("2026-06-30", "2025-06-30",
                                  "2024-06-30")})
    _r11b = _pio_fixture_rec(ltd_t={}, ltd_n={})
    _t11a, _t11b = pio_tests(_r11a)[4], pio_tests(_r11b)[4]
    out.append(("PIO 11: unchanged leverage passes (<=, the kit's stated "
                "reading); a family never tagged serves zero with the "
                "no-long-term-debt note and passes",
                _t11a["status"] == "PASS" and _t11b["status"] == "PASS"
                and "never tagged" in _t11b["detail"]
                and "no long-term debt" in _t11b["detail"],
                "strict-< would fail every debt-free filer"))

    # 12 — never-tagged vs stale: different zeros
    _sv12 = pio_ltd_serve({"2020-06-30": 5.0}, {},
                          ["2026-06-30", "2025-06-30", "2024-06-30"])
    out.append(("PIO 12: a family tagged elsewhere but absent at a "
                "needed date REFUSES naming the tag and its last date — "
                "a stale debt line is never served as zero",
                _sv12[0] is None and "LongTermDebt covers 0" in _sv12[3]
                and "2020-06-30" in _sv12[3]
                and "never served as zero" in _sv12[3],
                "the ADBE/PGR lesson, applied before the defect"))

    # 13 — one tag per test; the narrow-definition note; assets'
    #      three-date requirement
    _need13 = ["2026-06-30", "2025-06-30", "2024-06-30"]
    _svn = pio_ltd_serve({}, {d: 7.0 for d in _need13}, _need13)
    _svm = pio_ltd_serve({"2026-06-30": 9.0, "2025-06-30": 9.0},
                         {"2024-06-30": 7.0}, _need13)
    _r13 = _pio_fixture_rec(ta={"2026-06-30": 1000.0,
                                "2025-06-30": 1000.0})
    _t13 = pio_tests(_r13)[4]
    out.append(("PIO 13: only the noncurrent element covering all dates "
                "serves WITH the narrower-definition note; mixed "
                "coverage refuses per tag; assets missing at the third "
                "date refuse test 5 naming his average-assets need",
                _svn[1] == "LongTermDebtNoncurrent"
                and "noncurrent portion only" in _svn[2]
                and _svm[0] is None
                and "covers 2 of 3" in _svm[3]
                and "covers 1 of 3" in _svm[3]
                and _t13["status"] == "REFUSED"
                and "2024-06-30" in _t13["why"]
                and "AVERAGE-assets" in _t13["why"],
                "no cross-tag delta, ever"))

    # 14 — test 6: strict >, and the refusals name the element and the
    #      undefined-ratio case
    _r14 = _pio_fixture_rec(cl={"2026-06-30": 250.0, "2025-06-30": 250.0,
                                "2024-06-30": 250.0})
    _r14b = _pio_fixture_rec(cl={"2026-06-30": 0.0, "2025-06-30": 250.0,
                                 "2024-06-30": 250.0})
    _r14c = _pio_fixture_rec(cl={"2025-06-30": 250.0,
                                 "2024-06-30": 250.0})
    out.append(("PIO 14: flat current ratio fails (strict >); a zero CL "
                "refuses as undefined; a missing CL refuses naming "
                "LiabilitiesCurrent at its date",
                pio_tests(_r14)[5]["status"] == "FAIL"
                and "undefined" in pio_tests(_r14b)[5]["why"]
                and "LiabilitiesCurrent not tagged at 2026-06-30"
                in pio_tests(_r14c)[5]["why"],
                "his liquidity delta"))

    # 15 — test 7: raw count decides; the offering fixture where the
    #      engine's dS is compensation-only
    _r15 = _pio_fixture_rec(counts={2026: 1100.0e6, 2025: 1000.0e6},
                            ds={2026: 2.0})
    _t15 = pio_tests(_r15)[6]
    out.append(("PIO 15: test 7 FAILS on a raw count rise even where "
                "the engine's compensation dS is small — the excluded "
                "corporate issuance is named in the context (raw minus "
                "dS), the disagreement-is-wrong pin",
                _t15["status"] == "FAIL"
                and "+100,000,000" in _t15["detail"]
                and "98.000M" in _t15["detail"]
                and "excluded" in _t15["detail"],
                "his test reads issuance; the engine's split is context"))

    # 16 — test 7's pass-with-issuance caveat
    _r16 = _pio_fixture_rec(counts={2026: 990.0e6, 2025: 1000.0e6},
                            ds={2026: -15.0})
    _t16 = pio_tests(_r16)[6]
    out.append(("PIO 16: a count that fell past a tagged issuance "
                "passes WITH the buybacks-exceeded-it caveat printed",
                _t16["status"] == "PASS"
                and "buybacks" in _t16["detail"]
                and "5.000M" in _t16["detail"],
                "passed, and honest about why"))

    # 17 — test 8 on the committed PDEX GP dollars: the 24.73% ceiling,
    #      both sides
    _below = _pio_fixture_rec(gp={2026: _PDEX_GP[2026],
                                  2025: _PDEX_GP[2025]},
                              rev={2026: 120.0e6, 2025: 100.0e6})
    _above = _pio_fixture_rec(gp={2026: _PDEX_GP[2026],
                                  2025: _PDEX_GP[2025]},
                              rev={2026: 130.0e6, 2025: 100.0e6})
    out.append(("PIO 17: test 8's PDEX pin — with GP 24,335,000 / "
                "19,510,000, revenue growth below the committed 24.73% "
                "ceiling passes and above it fails",
                pio_tests(_below)[7]["status"] == "PASS"
                and pio_tests(_above)[7]["status"] == "FAIL",
                "the registered inequality, both sides"))

    # 18 — test 8's never-tagged refusal, the HRB wording
    _r18 = _pio_fixture_rec(gp={}, gp_src=[])
    _t18 = pio_tests(_r18)[7]
    out.append(("PIO 18: GrossProfit never tagged refuses test 8 with "
                "the exact registered sentence — the HRB exhibit",
                _t18["status"] == "REFUSED"
                and _t18["why"] == "GrossProfit never tagged in any "
                                   "annual filing",
                "banked NoSuchKey, paste 6"))

    # 19 — test 9 on the committed PDEX assets: the 16.61% bar, both
    #      sides
    _lo = _pio_fixture_rec(ta=dict(_PDEX_TA),
                           rev={2026: 110.0e6, 2025: 100.0e6})
    _hi = _pio_fixture_rec(ta=dict(_PDEX_TA),
                           rev={2026: 120.0e6, 2025: 100.0e6})
    out.append(("PIO 19: test 9's PDEX pin — revenue growth below the "
                "committed 16.61% beginning-assets bar fails and above "
                "it passes",
                pio_tests(_lo)[8]["status"] == "FAIL"
                and pio_tests(_hi)[8]["status"] == "PASS",
                "the registered inequality, both sides"))

    # 20 — the summary: M stated, refused tests named, no silent dash
    _r20 = _pio_fixture_rec(gp={}, gp_src=[])
    _n20, _m20, _s20 = pio_summary(pio_tests(_r20))
    out.append(("PIO 20: the summary prints N of M readable with every "
                "refusal named by number, name and reason",
                _m20 == 8 and "of 8 readable tests passed" in _s20
                and "test 8 (Gross margin improving)" in _s20
                and "never tagged" in _s20,
                _s20[:80]))

    # 21 — the floor, both sides
    _r21 = _pio_fixture_rec(ni={}, cfo={}, gp={}, gp_src=[],
                            rev={2026: 1.0, 2025: 1.0})
    _t21 = pio_tests(_r21)
    _m21 = sum(1 for t in _t21 if t["status"] != "REFUSED")
    _s21 = pio_floor_stop("XX", _m21)
    _r21b = _pio_fixture_rec(gp={}, gp_src=[], ni={2026: 5.0},
                             rev={}, counts={})
    _m21b = sum(1 for t in pio_tests(_r21b)
                if t["status"] != "REFUSED")
    out.append(("PIO 21: below five readable tests the page refuses "
                "with the minority sentence; at five it computes",
                _m21 < PIO_FLOOR
                and "not the F-Score" in _s21 and str(_m21) in _s21
                and _m21b == 5,
                f"M={_m21} refuses, M={_m21b} computes"))

    # 22 — invariants across the fixtures
    _ok22 = True
    for _r in (_pio_fixture_rec(), _r20, _r21, _r15):
        _tt = pio_tests(_r)
        _nn, _mm, _ = pio_summary(_tt)
        _ok22 = _ok22 and len(_tt) == 9 and 0 <= _nn <= _mm <= 9
    out.append(("PIO 22: nine rows always; 0 <= passed <= readable <= 9",
                _ok22, "the shape can never lie about its own size"))

    # 23 — the banned-word and disowned-verdict scan over every
    #      sentence surface
    _sent23 = [pio_method_text(), pio_floor_stop("T", 3),
               pio_no_dates_stop("T"),
               pio_financial_stop("T", "bank", "r."),
               pio_financial_stop("T", "refused", "r."),
               pio_foreign_stop("T", "EUR", 9, 1),
               pio_summary(pio_tests(_pio_fixture_rec()))[2]]
    _sent23 += [t["detail"] + " " + t["why"]
                for t in pio_tests(_pio_fixture_rec())]
    def _banned_hit(s: str) -> bool:
        t = " " + s.lower() + " "
        for b in PIO_BANNED:
            i = t.find(b)
            while i != -1:
                ...
                i = t.find(b, i + 1)
        return False
    _ok23 = all(pio_disowned(s) for s in _sent23) and not any(
        _banned_hit(s) for s in _sent23)
    out.append(("PIO 23: no verdict or banned word outside a disowning "
                "never-phrase on any output sentence",
                _ok23, "evidence, never a verdict"))

    # 24 — the year-end-change refusal reaches the delta tests
    _dates24 = ["2022-06-30", "2021-04-30", "2020-04-30"]
    _f24, _du24 = pio_fy_map(_dates24)
    _r24 = _pio_fixture_rec(
        fy=2022, fy2d=_f24, dups=_du24,
        ta={d: 1000.0 for d in _dates24},
        ca={d: 500.0 for d in _dates24},
        cl={d: 250.0 for d in _dates24},
        ltd_t={d: 100.0 for d in _dates24},
        ni={2022: 120.0, 2021: 100.0},
        cfo={2022: 150.0}, gp={2022: 1.0, 2021: 1.0},
        rev={2022: 2.0, 2021: 2.0},
        counts={2022: 1.0, 2021: 1.0})
    _t24 = pio_tests(_r24)
    _dd = [t for t in _t24 if t["n"] in (3, 5, 6, 7, 8, 9)]
    out.append(("PIO 24: HRB's 426-day change-spanning pair refuses "
                "every delta test with the gap named, while tests 1, 2 "
                "and 4 still read",
                all(t["status"] == "REFUSED" and "426 days" in t["why"]
                    for t in _dd)
                and all(t["status"] != "REFUSED"
                        for t in _t24 if t["n"] in (1, 2, 4)),
                "the live filer from paste 7"))

    # 25 — the ratio formatter's hand-check discipline, escalation
    #      included
    _a25 = pio_pct_arith(1.0, 3.0)
    _re25 = float(_a25.split(" = ")[1].rstrip("%"))
    out.append(("PIO 25: every printed ratio's displayed terms "
                "reproduce its displayed percentage; a non-terminating "
                "ratio escalates precision instead of lying",
                "7,242,000 / 61,192,000 = 11.83%"
                == pio_pct_arith(7242000, 61192000)
                and abs(_re25 - 1.0 / 3.0 * 100) < 10 ** (-2),
                _a25))

    # 26 — source-level: the span's load() returns shares_by_fy (the
    #      Deploy 0 key this page consumes); needles split so the
    #      check never matches itself
    _self26 = Path(__file__).read_text(encoding="utf-8")
    _n26 = '"shares_by' + '_fy": dict(shares_out)'
    out.append(("PIO 26: the span's load() carries the shares_by_fy "
                "meta key exactly once — test 7's supply line, pinned "
                "at source",
                _self26.count(_n26) == 1,
                "Deploy 0's key, consumed here"))

    # 27 — source-level: no price is read anywhere in the page-local
    #      block (the method needs none)
    _local27 = _self26.split("PIOTROSKI F-SCORE — nine binary", 1)[1]
    _n27a = "_avg_" + "price("
    _n27b = "current_" + "price("
    _n27c = "_monthly_" + "closes("
    out.append(("PIO 27: the page-local block reads no price — no "
                "average-price, spot-price or closes call below the "
                "span",
                all(n not in _local27 for n in (_n27a, _n27b, _n27c)),
                "nine tests, zero prices"))

    # 28 — every stop renders the footer first (source-level adjacency,
    #      the fleet's structural check)
    _stops, _covered = 0, 0
    _slines = _self26.split("\n")
    for _i, _l in enumerate(_slines):
        if "st.stop()" in _l and "adjacency" not in _l:
            ...
            if any("_page_footer()" in p for p in _prev):
                _covered += 1
    out.append(("PIO 28: every stop renders the footer first — "
                "source-level adjacency",
                _stops > 0 and _stops == _covered,
                f"{_covered} of {_stops} stops covered"))

    # 29 — both structural stops render the tag panel they promise
    #      (page 12's check-29 lesson, inherited on day one)
    _n29a = "pio_no_dates_stop(" + "_tk"
    _n29b = "pio_floor_stop(" + "_tk"

    def _panel_follows(needle: str) -> bool:
        for _j, _m in enumerate(_slines):
            if needle in _m:
                _nxt = [q for q in _slines[_j + 1:_j + 6] if q.strip()]
                return any("_pio_tag_panel(" in q for q in _nxt)
        return False
    out.append(("PIO 29: the no-dates and floor stops render the tag "
                "panel they summarize — the promise kept in source",
                _panel_follows(_n29a) and _panel_follows(_n29b),
                "the AZO catch, inherited"))

    # 30 — the CFO fill note names the filled years and both tags, and
    #      routes name menu pages (the reader's foreign phrase intact)
    _note30 = pio_cfo_fill_note({2016: PIO_TAGS["CFO"][0],
                                 2015: PIO_TAGS["CFO"][1],
                                 2014: PIO_TAGS["CFO"][0]})
    out.append(("PIO 30: the mixed-tag CFO note names FY2015 and the "
                "definitional gap (the PDEX shape); a single-tag read "
                "stays silent; financial and foreign stops route by "
                "menu name and the reader's phrase survives in the span",
                "FY2015" in _note30 and "FY2014" not in _note30
                and "definitional gap" in _note30
                and pio_cfo_fill_note({2026: PIO_TAGS["CFO"][0]}) == ""
                and _route_ok(pio_financial_stop("T", "bank", "r."))
                and _route_ok(pio_foreign_stop("T", "EUR", 9, 1))
                and _route_ok(pio_foreign_route_suffix(
                    "Foreign private issuers are " + "not supported"))
                and ("Foreign private issuers are " + "not supported")
                in _self26,
...
# ══════════════════════════════════════════════════════════════════════
#
# NOTE ON DOLLAR SIGNS: Streamlit markdown parses $...$ as LaTeX;
# every literal dollar amount on a markdown surface here is either
# comma-formatted without the sign or lives in st.code / st.dataframe.
...
def _page_footer() -> None:
    ...
    with _r1:
        with st.expander("What the tests mean", expanded=False):
            st.markdown(
                "**The F-Score** — Piotroski's nine binary tests: "
                "profitability (1, 2, 3, 4), capital structure and "
                "liquidity (5, 6, 7), operating efficiency (8, 9). One "
                "point per passed test in his paper; here each test "
                "prints its own inputs and arithmetic instead, and the "
                "summary counts passes over READABLE tests only, with "
                "every refusal named.\n\n"
                "**His scaling** — ROA, cash flow and turnover on "
                "beginning-of-year assets; leverage on average assets. "
                "That is why tests 3, 5 and 9 need three consecutive "
                "year-end dates.\n\n"
                "**What a refusal means** — an input his definition "
                "needs does not read from the filings for the needed "
                "years. A refused test is not a failed test, and the "
                "score's denominator says so."
            )
    with _r2:
        with st.expander("Verify the engine"):
            st.caption(
                ...
                "prove it unchanged. The Piotroski checks pin this "
                "page's arithmetic: his scaling, the consecutive-year "
                "window on the live HRB shape, the long-term-debt "
                "never-vs-stale split both ways, every refusal wording, "
                "and the PDEX control figures the 21 Sep 2026 "
                "registration committed."
            )
            if st.button("Run checks"):
                for _label, _suite in (("Engine", self_test()),
                                       ("Piotroski", pio_self_test())):
    ...
    st.caption(
        "Research aid, not financial advice. The method follows Joseph "
        "Piotroski's published paper (2000); the refusals, the readable-"
        "tests summary, the floor, the count-route issuance test and "
        "its context lines are this project's own. Independent; not "
        "affiliated with or endorsed by the author or any publisher."
    )


st.set_page_config(
    page_title="Piotroski F-Score — Investor Toolkit",
    page_icon="🩺",
    layout="centered",
...
st.caption("Joseph Piotroski's nine financial-strength tests — computed "
           "from filed data, each shown with its inputs, for one ticker "
           "at a time")
st.info(pio_method_text())

with st.form("pio_form"):
    _tk = st.text_input("Stock ticker",
                        placeholder="PDEX · MSFT · HRB — press Enter"
                        ).strip().upper()
    _pio_go = st.form_submit_button("Run the nine tests", type="primary")

if _pio_go and not _tk:
    st.warning("Enter a ticker first.")
if _pio_go and _tk:
    try:
        _rec = pio_record(_tk)
    except ValueError as _e:
        st.error(str(_e) + pio_foreign_route_suffix(str(_e)))
        _page_footer()
        st.stop()

    if _rec["stop"] == "financial":
        st.error(pio_financial_stop(_tk, _rec["fin_class"],
                                    _rec.get("fin_reason")
                                    or "The kit's gate classifies it so."))
        _page_footer()
        st.stop()
    if _rec["stop"] == "foreign":
        _cu, _cn, _cus = _rec["ccy"]
        st.error(pio_foreign_stop(_tk, _cu, _cn, _cus))
        _page_footer()
        st.stop()
    if _rec["stop"] == "foreign_note":
        st.error(_rec["fnote"] + pio_foreign_route_suffix(_rec["fnote"]))
        _page_footer()
        st.stop()
    if _rec["stop"] == "no_dates":
        st.error(pio_no_dates_stop(_tk))
        _pio_tag_panel(_rec)
        _page_footer()
        st.stop()
    if _rec["m"] < PIO_FLOOR:
        st.error(pio_floor_stop(_tk, _rec["m"]))
        _pio_tag_panel(_rec)
        _page_footer()
        st.stop()
    ...
    with _c1:
        st.metric("Tests passed", f"{_rec['n']} of {_rec['m']}")
    with _c2:
    ...
    st.dataframe(pd.DataFrame([{
        ...
        "Test": t["name"],
        "Result": t["status"],
        "Inputs and arithmetic": (t["detail"] if t["status"] != "REFUSED"
                                  else t["why"]),
    } for t in _rec["tests"]]), width='stretch', hide_index=True)
    st.caption("Every figure is a filed one at the headline pair's own "
               "fiscal years and exact year-end dates; a refused test "
               "names the element and date it could not read. Nothing "
               "fills across a test, and no year-end change is bridged.")

    _t7 = _rec["tests"][6]
    if _t7["status"] != "REFUSED":
        _gF = _rec["g"].get(_F)
        _tF = _rec["t_bb"].get(_F)
        if _gF is not None or _tF is not None:
            st.caption(
                f"Test 7 context, FY{_F}: filed buybacks "
                f"{(_tF or 0.0):,.1f}M and the GAAP stock-comp charge "
                f"{(_gF or 0.0):,.1f}M, from the reader's own year "
                "record — the decomposition above is the kit's "
                "evidence; the binary is his test on the count.")
    _fn = pio_cfo_fill_note(_rec["cfo_origin"])
    if _fn:
        st.caption(_fn)
    ...
    with st.expander("Assumptions used — paste this if a figure looks "
                     "wrong"):
        st.code("\n".join([
            f"ticker {_rec['resolved']}   headline FY{_F} "
            f"({_rec['fy2d'][_F]})   score {_rec['n']} of {_rec['m']} "
            "readable",
            "tests " + " ".join(f"{t['n']}:{t['status'][0]}"
                                for t in _rec["tests"]),
            f"dates read {len(_rec['maps']['dates'])} "
            f"({_rec['maps']['dates'][-1]} .. "
            f"{_rec['maps']['dates'][0]})   consecutive window "
            f"{PIO_PAIR_MIN_DAYS}-{PIO_PAIR_MAX_DAYS} days",
            "scaling: beginning assets (tests 1/2/3/9), average assets "
            "(test 5); test 5 passes on <= (the kit's stated reading)",
            f"floor {PIO_FLOOR} readable tests   registration as-of "
            f"{PIO_REGISTERED_AS_OF}",
        ]), language="text")
