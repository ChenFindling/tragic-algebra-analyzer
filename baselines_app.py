...
Layout of this file:
  lines up to the BASELINES banner — tool 1's engine, reader and 152-check
  self-test, copied VERBATIM from the deployed 1_Tragic_Algebra_Analyzer.py
  (its lines 1-4600; only this docstring replaced, tool 1's UI dropped).
...
#  (lines 1–4600 of the deployed 1_Tragic_Algebra_Analyzer.py, 152
#  checks; only the module docstring was replaced). Re-copied 12 Sep
#  2026 for F4, the treasury equality-reject (DECOMP §4): a
#  treasury-origin Cw candidate equal to the year's own buyback is the
#  buyback, rejected before the size gate, with the house-style note.
#  Previous recopy 9 Sep 2026 for the per-filing XBRL route (queue G):
#  registry-gated instance reads for GOOGL's custom withholding tag and
#  the dual-class per-class share counts, verification-gated. The
