...
Layout of this file:
  lines up to the BASELINES banner — tool 1's engine, reader and 147-check
  self-test, copied VERBATIM from the deployed 1_Tragic_Algebra_Analyzer.py
  (its lines 1-4448; only this docstring replaced, tool 1's UI dropped).
...
#  (lines 1–4448 of the deployed 1_Tragic_Algebra_Analyzer.py, 147
#  checks; only the module docstring was replaced). Re-copied 9 Sep
#  2026 for the per-filing XBRL route (queue G): registry-gated
#  instance reads for GOOGL's custom withholding tag and the
#  dual-class per-class share counts, verification-gated. The
